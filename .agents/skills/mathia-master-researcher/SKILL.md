...
Assess each current line from evidence, not from activity volume or age. The program's objective is to resolve RH. Treat proportion improvements, finite certificates, and standalone theory as potential instruments for that objective, with honest independent mathematical value; do not silently substitute a percentage, formal-coverage target, or publication output for resolving the hypothesis.

Follow each consequential route from its strongest established input to its intended conclusion and locate the first unsupported implication. Distinguish a new estimate or reusable theorem from a reformulation that leaves that implication unchanged. Use this analysis inside the existing snapshot and clue formats; no extra scoring system, campaign ledger, or reporting structure is needed.

Challenge the direction and quantifiers of the claimed bridge: a lower bound on the Newman constant is not an upper bound, a density-one statement does not eliminate all exceptional zeros, and RH does not require simplicity. Favor work that can remove or clarify such gaps while allowing exploratory learning without an already complete proof plan.

Read clue outcomes rather than counting dispositions as success. Acceptance licenses investigation; resolution may support, narrow, refute, or merely recognize an existing classical answer. Identify what the outcome enables or rules out and whether the remaining question is materially different. Neither a clue for every line nor many local results imply that every line deserves equal attention. Preserve room for exploratory and standalone mathematical value without labelling it progress toward RH.
...
When a route has exhausted its current inputs, state the mathematical condition that would justify reopening it, such as a stronger estimate, a source-derived coupling, or a controlled limit outside the obstruction's hypotheses. Difficulty, a small finding count, or failure to finish within one invocation does not establish exhaustion. Use the existing recommendation fields and keep task rotation subject to its separate gates.
