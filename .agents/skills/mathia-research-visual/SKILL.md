...
A retained PNG must be a **fully decodable image**, not merely a file with a PNG signature, plausible dimensions, or non-zero size. A truncated renderer output is a workflow failure and must never be published as a research artifact.
...
4. require positive dimensions and a complete decode through the terminal PNG structure; checking only the eight-byte signature or `IHDR` is insufficient;
...
6. before any commit, enumerate **every added or modified `.png`** in the complete changed-path set and run the same full decode on each one.

If any retained PNG fails validation, abort publication of the whole visual-artifact change. Regenerate it from the mathematical construction and recorded parameters; do not commit a partial placeholder and do not rely on GitHub or a browser preview as validation. If an already tracked PNG is discovered to be corrupt, repairing it is a substantive artifact correction: reproduce it from the documented construction where possible, validate the regenerated bytes, and replace the broken blob. If faithful regeneration is not possible, do not fabricate a substitute image; report the workflow error instead.

Temporary files left by an interrupted rendering attempt are disposable and must not be committed. The final tracked PNG/Markdown pair remains subject to the normal path and no-churn gates.
