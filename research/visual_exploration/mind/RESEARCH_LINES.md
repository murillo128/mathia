...
VIS-270 now shows that leading-functional cancellation and arithmetic access can be separated by changing support geometry rather than by increasing moment order. A fixed smooth two-band packet places one band at the first prime frequency `xi_2=log 2/(2pi)` and a compensating band in a source-empty region. The combination has `g(0)=0` and cancels Wang's cusp functional exactly, while its Rodgers quadratic coefficient is a nonzero pure first-prime term. The arithmetic access threshold is therefore genuinely the support crossing at `xi_2`; diagonal/cusp nulling need not make the packet source-blind.

The representation-side question is no longer whether one can build a leading-null packet that still sees arithmetic data: VIS-270 does so at first-prime level with fixed conditioning. The live question is whether the short-interval theorem can expose that lower-order coefficient with a remainder below it, and more generally how theorem-transfer errors scale for fixed translated or multiband arithmetic support.
...
VIS-269 shows that this generic shrinking-packet interface becomes source-blind once its support falls below the first prime-power frequency. VIS-270 supplies the complementary fixed-band construction: the leading Wang functional can vanish while the packet still samples the first arithmetic frequency. The remaining theorem-transfer gap is now explicit rather than representational: the available Wang error is larger than the Rodgers lower-order first-prime coefficient and the theorem does not identify that coefficient in the short-interval statistic.

What Wang still does not provide is a lower-order expansion resolving the VIS-270 arithmetic coefficient, support whose edge itself approaches `theta`, a polylogarithmic physical source window beyond the current seminorm gate, a remainder automatically below every arithmetic residual, or four-level covariance. Source-height localization and pair-separation weighting also remain distinct: a theorem controlling one cannot be transferred by identifying its window with the other.