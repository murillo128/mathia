...
VIS-421 closes the implicit source-anchoring assumption in that program. In the Wang short-interval setup the interval start `T` is a generic asymptotic parameter; the source does not supply a canonical discrete family of distinguished heights such as zero ordinates, Gram points or extrema. The fixed-shell Haar law therefore calibrates a chosen realized shell but does not by itself make the observation height source-selected. A multi-height anomaly test must specify independently both a deterministic height selector and a transport rule for the shell/window normalization as height changes. If the selector is imported from another zeta structure, the resulting claim is a cross-structure alignment statement, not intrinsic Wang anchoring.

The live source question is now whether an **independently justified, pre-specified arithmetic height selector** produces an anomaly under the exact base-prime Haar self-null, after the VIS-416 bandwidth normalization and VIS-417 beat-scale controls, and whether the required shell/window transport remains quantitative in the physical growing-shell regime. For fixed finite observables VIS-420 makes the finite-window tariff explicit. A jointly growing shell/height theorem must control both coefficient mass and the smallest relevant linear forms in prime logarithms strongly enough that the sinc remainder vanishes uniformly, while VIS-421 requires the selector and normalization transport to be fixed before the anomaly is inspected. Direct Haar tests avoid the translation-calibration step but do not solve the missing source-selection problem.

A positive result would establish only a source-sensitive or cross-structure phase-alignment channel, depending on the selector's provenance, not an RH mechanism. A null result would close another visually attractive route. Any claim must preserve the actual shell character dependencies, distinguish exact fixed-shell Haar calibration from the much stronger jointly growing shell/height limit, and not smuggle a distinguished height into a source that does not provide one.
