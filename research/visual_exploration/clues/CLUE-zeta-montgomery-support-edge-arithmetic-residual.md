...
based_on:
  ...
  - research/visual_exploration/findings/VIS-256-finite-moments-do-not-beat-sine-kernel-cusp.md
  - research/visual_exploration/findings/VIS-257-cusp-annihilation-exposes-finite-window-quadratic-conditioning.md
  - research/visual_exploration/findings/VIS-258-cusp-plus-quadratic-null-sharp-quartic-conditioning.md
...
`VIS-256` closes a tempting finite-moment escape from that linear scale. For every fixed cancellation order there are symmetric bounded-normalization signed packets on `[-b,b]` that annihilate all ordinary moments through that order but pair with the limiting sine-kernel cusp `T(q)=min(|q|,1)` at exactly `Theta(b)`. Zero mass, symmetry, and any fixed finite list of polynomial vanishing moments therefore do not force the support-safe companion from `O(b)` to `o(b)`. Any stronger shrinking-band suppression must control the cusp itself — for example a weighted `|q|` pairing or another non-polynomial mechanism — rather than merely increase a fixed polynomial moment count.

`VIS-257` resolves the existence side of that cusp-adapted escape sharply. A packet satisfying zero mass and exact `|q|` cancellation can kill the limiting sine-kernel cusp while retaining a quadratic channel, but `|m_2(nu)| <= (b^2/8)||nu||_TV` is sharp. A unit quadratic carrier therefore costs exactly `8b^-2` in total variation, and the extremizing three-radius packet still sees the finite-window CUE quadratic response. Cusp cancellation removes the limiting linear obstruction; it does not remove the order-matched finite-window calibration problem.

`VIS-258` closes the next fork. If the packet also imposes `m_2=0`, then `|m_4(nu)| <= E_* b^4 ||nu||_TV` with the sharp constant `E_*=0.0634615548319...`; equivalently a unit quartic carrier costs `K_*b^-4` with `K_*=15.7575716928...`. The optimal four-radius packet kills mass, the cusp, and the quadratic finite-window mode exactly, but any total-variation-linear uncertainty must now beat the corresponding `E_*|A_4|b^4` scale. Increasing the null order therefore moves both the first surviving carrier and its conditioning/error gate to a more expensive order; it is not by itself a generic route to a better-conditioned arithmetic residual.
...
For the asymmetric near-edge × low-frequency redesign, the packet-existence question is now resolved through quartic order. The live question is whether the actual zeta-minus-null destination supplies a **source-specific quadratic or quartic coefficient, or another explicitly derived first-surviving carrier, whose theorem/transfer/covariance error is asymptotically smaller than the sharp order-matched conditioning gate**. In the quadratic branch this means surviving exact cusp cancellation under the unavoidable `8b_L^-2` total-variation cost and an `o(b_L^2)` null/transfer calibration at the unnormalized scale. In the quartic branch it means surviving cusp-plus-quadratic cancellation under the `K_*b_L^-4` cost and an error asymptotically smaller than `E_*|A_4|b_L^4`. Merely adding more vanishing moments is not an answer unless the destination first supplies the corresponding higher-order arithmetic carrier and a compatible error law.
...
`VIS-139` makes the missing information concrete at the four-level layer. `VIS-140` shows that the classical unconditional Rudnick–Sarnak support theorem does not reach the edge-edge packet, `VIS-141` rules out a vague “just redesign it inside the support diamond” shortcut for two identical edge factors, and `VIS-142` shows that the surviving asymmetric support-safe companion is not free. `VIS-256` rules out fixed finite polynomial moment balancing as a generic way to buy another power of bandwidth, while `VIS-257` and `VIS-258` show exactly what happens when the null is redesigned to cancel the cusp itself: the quadratic and quartic carrier capacities have sharp `b^2` and `b^4` scales with unavoidable `b^-2` and `b^-4` total-variation costs.

The restricted-support branch is therefore no longer blocked by uncertainty about whether cusp-adapted packets exist. They do, with sharp conditioning constants. The unresolved issue is whether zeta contributes arithmetic structure in one of those first-surviving channels strongly and uniformly enough to beat the matching finite-window, transfer, covariance, and theorem errors. If not, moving to still higher cancellation order only makes the conditioning problem more expensive without evidence of a new source carrier.

The live fork remains: either obtain genuinely wider-support/direct four-level control for the original frozen edge covariance, or deliberately study one pre-registered cusp-adapted edge–low-frequency branch and prove from scratch that a source-specific arithmetic contribution survives its sharp conditioning budget. This prevents restricted-support compatibility or moment engineering from being mistaken for preservation of the original question.
...
If an asymmetric edge–low-frequency redesign is pursued, pre-register the branch before fitting any arithmetic amplitude or inspecting confirmation data. For the quadratic branch, use an exact cusp-annihilating packet of the `VIS-257` type with nonzero `m_2`; derive the destination's source-specific quadratic coefficient and prove that the finite-window/null/transfer uncertainty is `o(b_L^2)` before the optimal `b_L^-2` normalization, while propagating the exact `8b_L^-2` total-variation cost through stochastic variance and cross-window covariance. Kill this branch if the surviving quadratic term is wholly explained by the calibrated finite-CUE null or if its uncertainty is not asymptotically below that scale.

For the quartic branch, impose zero mass, exact cusp cancellation, and `m_2=0` using a pre-registered packet satisfying the sharp `VIS-258` gate. Derive a genuine source-specific quartic coefficient `A_4` for the actual zeta-minus-null destination and require every total-variation-linear theorem/transfer uncertainty to satisfy `epsilon_b/[E_*|A_4|b_L^4] -> 0`, with the `K_*b_L^-4` conditioning propagated through variance and dependence. Kill this branch if no such coefficient exists, if the error budget does not beat the sharp scale, or if a matched cusp-plus-quadratic CUE/smooth surrogate reproduces the apparent residual.

Do not switch from quadratic to quartic, or add further vanishing moments, merely after seeing that a pre-registered branch fails on confirmation data. A higher-order branch is admissible only when its source carrier and conditioning/error law have been derived independently beforehand. `VIS-256` remains the kill test for ordinary finite-moment balancing without cusp control; `VIS-257` and `VIS-258` are the quantitative gates once cusp control is imposed.
...
Independently bound the finite-height zeta-to-null mismatch and nuisance propagation through the same statistic at order `1/L`. The gate passes only if covariance-implied averaging error and transfer uncertainty can both be made smaller than a predeclared arithmetic amplitude without fitting window selection, effective size, normalization, packet order, or statistic to confirmation data.
...
`VIS-140` is a literature-bounded support audit: the classical Rudnick–Sarnak theorem does not include the Montgomery edge-edge four-level packet. `VIS-141` is an elementary support-budget corollary showing that an identical-factor restricted-support redesign is confined below `|q|<1/2`, while a near-edge factor forces an independent companion band toward zero frequency. `VIS-142` is an exact finite-CUE harmonic bound showing that the zero-mode-centered mean of any bounded-normalization companion packet is `O(b_L)` under a fixed source-capacity margin. `VIS-256` is an exact negative design control showing that no fixed finite collection of ordinary polynomial moment cancellations forces this universal sine-kernel cusp response to improve to `o(b_L)`.

`VIS-257` proves a sharp cusp-adapted quadratic capacity statement: exact zero-mass and `|q|` cancellation is compatible with a nonzero quadratic carrier, but a unit carrier costs exactly `8b^-2` in total variation and the finite-window CUE quadratic response remains. `VIS-258` proves the next sharp capacity statement: after also cancelling `m_2`, a unit quartic carrier costs exactly `K_*b^-4`, `K_*=15.7575716928...`, with `E_*=1/K_*`; its finite-window consequence gives the corresponding order-matched quartic transfer/error gate. These are conditioning and null-design results, not evidence that zeta supplies either a quadratic or quartic arithmetic residual.

None of these results establishes the four-level correlation law of actual zeta height windows, stationarity or mixing of that sequence, a cross-height finite-CUE transfer theorem, a uniform spectral gap, the asymptotic error of a shrinking and renormalized Rudnick–Sarnak test family, or the existence of an arithmetic residual. In particular, `VIS-257` and `VIS-258` do not prove source-specific suppression, a zeta-specific quadratic/quartic coefficient, or that moving to higher moments improves detectability. `VIS-142` does not bound a zeta-specific arithmetic correction by `O(b_L)`. The `Theta(L^2)` count remains conditional on summable covariance with uniformly controlled positive long-run variance. No fresh confirmation-zero evidence has been used.
...
The restricted-support redesign is now a sharper arithmetic-conditioning problem. Exact cusp-adapted quadratic and quartic packets exist and their optimal conditioning costs are known from `VIS-257` and `VIS-258`; packet existence and normalization are no longer the unknowns. The next admissible step is to choose one branch before confirmation data and derive a source-specific first-surviving coefficient together with an error/covariance law that beats that branch's sharp order-matched gate. Further moment cancellation is not progress by itself. Arithmetic-amplitude fitting plus untouched confirmation remain downstream.
