...
`VIS-139` makes the missing information concrete at the four-level layer. `VIS-140` shows that the classical unconditional Rudnick–Sarnak support theorem does not reach the edge-edge packet, `VIS-141` rules out a vague “just redesign it inside the support diamond” shortcut for two identical edge factors, `VIS-142` shows that the surviving asymmetric support-safe companion is not free, and `VIS-256` rules out fixed finite polynomial moment balancing as a generic way to buy another power of bandwidth. The restricted-support branch therefore needs either a direct cusp-adapted cancellation mechanism or a source-specific reformulation; it cannot obtain `o(b_L)` null suppression merely by imposing finitely many ordinary moments.
...
If an asymmetric edge–low-frequency redesign is pursued, define its companion packet `nu_(b_L)` and normalization before fitting any arithmetic amplitude. With a fixed capacity margin, `VIS-142` gives the CUE-side bound `|B_L|<=kappa_alpha^2 b_L ||nu_(b_L)||_TV`. If `||nu_(b_L)||_TV=O(1)`, derive a zeta-specific arithmetic term that remains detectable despite the vanishing null-centered scale. If stronger null suppression is claimed from cancellation constraints, prove a bound that controls the actual cusp-sensitive quantity, such as `int |q| dnu_(b_L)(q)`, or another mechanism not reducible to a fixed finite set of polynomial moments. `VIS-256` is a kill test for any argument whose only extra input is zero mass, symmetry, and finitely many ordinary vanishing moments. If the cancellation order grows with `L`, account explicitly for the accompanying total-variation/conditioning growth rather than treating it as the fixed-order case. If `||nu_(b_L)||_TV` grows like `1/b_L` or faster, propagate that exact normalization through the Rudnick–Sarnak remainder or any replacement theorem, finite-height transfer, stochastic variance, and cross-window covariance. Kill the redesign if the required amplification makes any of those errors comparable to the proposed arithmetic signal.
...
None of these results establishes the four-level correlation law of actual zeta height windows, stationarity or mixing of that sequence, a cross-height finite-CUE transfer theorem, a uniform spectral gap, the asymptotic error of a shrinking and renormalized Rudnick–Sarnak test family, or the existence of an arithmetic residual. `VIS-256` does not say that every balanced packet has linear cusp response, does not rule out a specially cusp-adapted packet, and does not settle the tradeoff when cancellation order grows with `L`; it only shows that fixed finite ordinary moments do not force the stronger suppression. In particular, `VIS-142` does not bound a zeta-specific arithmetic correction by `O(b_L)`. The `Theta(L^2)` count remains conditional on summable covariance with uniformly controlled positive long-run variance. No fresh confirmation-zero evidence has been used.
...
The restricted-support redesign is now a narrower renormalization-and-regularity problem rather than a generic escape. It cannot control two identical Montgomery-edge factors; a support-safe near-edge factor forces a shrinking low-frequency companion, `VIS-142` shows that bounded normalization makes the companion's finite-CUE centered mean vanish with that bandwidth, and `VIS-256` shows that finitely many ordinary vanishing moments do not generically improve the cusp scale. Any stronger bounded-normalization suppression must identify a cusp-adapted mechanism; any amplified companion must earn its arithmetic signal while carrying the same amplification through theorem, transfer, noise, and dependence errors. Arithmetic-amplitude fitting plus untouched confirmation remain downstream.