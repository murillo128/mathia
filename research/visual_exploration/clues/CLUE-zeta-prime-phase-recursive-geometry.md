...
`VIS-078` now writes the cancellation-sensitive quantity exactly. The long-height mean-square fluctuation is
...
## Research question
After quotient/reconstruction controls, fixed-prime vertical equidistribution, the exact finite-window resonance null, the generic safe growing-support corridor, the additive log-Euler corridor, the positive-cone holomorphic Euler-product law, the fixed-support signed `|P_X|^2` closure, and the separation between growing reciprocal-frequency certificates and actual sinc-filtered energy are accounted for, is there a representation-stable visual statistic in the hybrid hierarchy that genuinely leaves the prime-torus information class?

The signed growing-support route is now specific: it must control the **full cancellation-sensitive sinc-filtered spectrum**, or invoke a direct mean-value theorem for the exact growing observable. Growth of `l^1` or reciprocal-frequency `l^2` certificate constants, and visually dramatic nearest-neighbor beats, are no longer admissible positive evidence. Independently anchored finite windows and independently calibrated factor/residual coordinates remain separate admissible routes.
...
The next signed test must therefore control

`P_X(1,0)^2 sum_(k!=0) w_k^2 sinc^2(L lambda_k/2)`

for a frozen joint law `X=X(L)`, with any normalization and selected spectral band specified before confirmation. A positive claim must lower-bound or otherwise characterize **actual sinc-filtered mass** that survives the matched prime-torus law. A direct growing-cutoff mean-value theorem for the same observable may replace the explicit spectral sum. Do not infer signal from divergence of `B_X`, `Q_X`, a nearest-neighbor resonance count, or failure of a sufficient corridor.
...
None of these findings determines the full growing-support sinc-filtered variance, a sharp cutoff/window threshold, proof that a witness outside the sufficient corridors separates from the torus null, control of arbitrary nonlinear growing-dimensional path functionals, proof that a particular externally anchored window is arithmetic-specific, a new prime/zero independence result, an unexpected factor/residual joint law, or an RH consequence.

## Research disposition
Accepted in further narrowed form. Treat fixed-finite population separation, fixed-finite sliding-window separation, the `VIS-073` generic safe corridor, the `VIS-074` additive corridor, the `VIS-075` raw holomorphic Euler-product positive-cone route, the `VIS-076` fixed-support squared-modulus signed route, dimension-uniform `l^1`/reciprocal-frequency `l^2` certificate expectations, and nearest-neighbor small-divisor mass as closed positive routes. Continue the signed route only through the full sinc-filtered growing-support energy or a direct theorem controlling that exact observable; the independently anchored-window and independently calibrated factor/residual routes remain open.