...
Because `m/(m+1)->1`, every fixed sublinear power scale `H_n=O(g_n^alpha(log g_n)^B)` with `alpha<1` and fixed `B` is swallowed by some fixed finite-order deterministic sampling null. A residual that appears merely because a lower-order Gram correction became too weak is therefore not a separator anywhere on this whole family of local power-law scales.
...
First, a Gram/prime-phase route may move into a genuinely different sampling regime: nearly linear or nonlocal Gram blocks, a statistic whose growing-dimensional sensitivity is not controlled by coordinatewise phase collapse, or another regime requiring a separately derived null. Merely increasing a fixed sublinear power exponent or the finite Taylor order is no longer an escape route.
...
The current sequence of controls shows how easily visual structure can be generated by representation and sampling alone. The same prime field is globally Haar on fixed support, locally frozen on very short Gram blocks, organized into a deterministic arc at the first motion scale, and controlled by a hierarchy of deterministic inverse-theta clock corrections on every fixed sublinear power-law block with polynomial prime support.

A surviving statistic would therefore need a genuinely new information source or sampling regime rather than another rendering of the same local clocked prime phases. That makes the remaining directions narrower but materially cleaner.
...
For consecutive Gram windows with polynomial prime support, compare against `VIS-084`--`VIS-087`. If the block has a fixed sublinear power scale `H_n=O(g_n^alpha(log g_n)^B)` with `alpha<1`, first choose a fixed finite order `m` with `alpha<m/(m+1)` and quotient the corresponding inverse-theta Taylor clock. `VIS-087` then forces the remaining coordinatewise prime-phase clock residual to zero.

Do not treat increasing `m`, `alpha`, the number of sampled Gram points, or the prime cutoff within polynomial support as confirmation. A local Gram/prime-phase claim survives this gate only if its statistic retains information despite the appropriate finite-order clock null, for example because its sensitivity grows with dimension in a separately quantified way. A nearly linear or nonlocal Gram route requires its own sampling theorem rather than extrapolation from failure of the fixed-order estimate.
...
The signed squared-Euler-product branch is resolved negatively as a separator by `VIS-080`--`VIS-082`. Gram-point anchoring is classified at several levels: `VIS-083` gives fixed-prime population Haar behavior, `VIS-084` gives raw short-block freezing and the first deterministic local arc, `VIS-085` and `VIS-086` give first- and second-order corrected collapse, and `VIS-087` proves the fixed finite-order hierarchy that covers every fixed sublinear power scale for polynomial prime support.

None of these findings proves growing-dimensional Haar equidistribution, controls nearly linear or genuinely nonlocal Gram sampling, bounds arbitrary dimension-sensitive observables, approximates zeta strongly enough for the intended inference, proves independence of prime and zero data, or has an RH consequence. Failure of a particular coordinatewise upper bound remains only an absence of that control unless a surviving law is established independently.

## Research disposition

Accepted in further narrowed form. Continue only through a Gram/prime-phase statistic that demonstrably escapes the appropriate finite-order local sampling null, a genuinely different nearly linear/nonlocal sampling regime with its own control, an independently informative added coordinate/source law, or an independently calibrated factor/residual joint-law question. Do not reopen fixed sublinear power-law Gram blocks merely by raising the correction order, using more primes, or changing the rendering.