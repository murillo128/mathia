...
`VIS-397` removes the common-rate restriction from that shared-block control. For independent `X_i~Gamma(k_i,lambda_i)` and `C~Gamma(k_0,lambda_0)`, the simple independent Dirichlet/shrinkage factorization is lost, but the normalized simplex law remains exact:
...
`VIS-398` closes the obvious finite multi-block enlargement. For any fixed finite collection of independent shared Gamma blocks with fixed nonnegative incidence vectors, the normalized qutrit weights have an exact latent-fraction simplex density obtained by a finite-dimensional integral. Pair-specific blocks such as `(1,1,0)`, `(1,0,1)`, and `(0,1,1)` can generate independently varying positive pairwise covariances, and subset-specific/common blocks can be combined, yet conditioning on `m_1` still leaves an exactly specified one-dimensional residual law once the block architecture, shapes, rates, and loadings are fixed independently. Merely adding finitely many additive Gamma shocks therefore does not create an uncontrolled source-specific channel.

`VIS-399` adds a qualitatively different dependence control. With a fixed total packet budget, mutually exclusive sector assignment, and independently specified Gamma amplitude law in each sector, the sector energies have exact negative cross-covariance

`Cov(Y_i,Y_j)=-N p_i p_j mu_i mu_j`.

Conditional on the multinomial sector counts, the normalized qutrit weights are the same exact normalized-Gamma laws used earlier; averaging over the finite count vectors gives a finite mixture, with explicit face components when some sector receives no packet. Conditioning on `m_1` therefore leaves an exact mixed continuous/discrete one-dimensional residual law. Negative dependence, skew, concentration, and boundary mass can consequently be manufactured by a source-free mechanism too.
...
Equivalently, after quotienting compact/Vandermonde conditioning, Gram-only spectral filtering, bounded positive preconditioning, rational-expression artifacts, unavailable similarity directions, bare commutator-gap collapse, generator-coordinate gauge, tangent-amplitude gauge, the isotropic defect-energy budget, the universal Markov floor, the exact qutrit Gaussian ladder, symmetry-only vacuity, the independent replicated-Gamma count/rate ladder, the full fixed finite additive shared-Gamma incidence-network ladder, and the fixed-budget exclusive-Gamma negative-dependence ladder, is there a remaining **mechanistically restricted** nonarithmetic control under which the Wang residual is still exceptional?
...
`VIS-393` shows why rejecting that Gaussian family is not enough: full source-stabilizer invariance permits arbitrary distributions of `u`, so symmetry itself supplies no stronger discriminating null. `VIS-394` and `VIS-395` show that concrete source-free repeated-packet mechanisms already produce broad nonuniform and asymmetric residual families without appealing to arbitrary invariant laws. `VIS-396`--`VIS-398` show that fixed finite additive Gamma shock networks can produce broad positive cross-sector dependence while remaining exactly calibratable. `VIS-399` supplies the complementary finite-budget mechanism: exclusive allocation produces exact negative raw-energy dependence and a finite normalized-Gamma mixture, again with an exact conditioned residual law.

This blocks a still broader false escalation in which residual shape is called source-specific merely because it is non-Gaussian, nonuniform, skewed, concentrated, positively or negatively correlated across sectors, inconsistent with one common rate, generated by several common shocks, or carries boundary mass. All of those features can arise from independently specified source-free packet architecture.
...
For the generic qutrit rank-one test bed, compute the source cubic invariant, the support `q_1<q_2<q_3`, the moments `m_1,m_2`, and the residual coordinate `u`. Use `VIS-390` only as the isotropic one-replica baseline. Use `VIS-391` when independent sector rates are justified for one Gaussian block per sector. Use `VIS-394` when the source-free mechanism independently specifies a common replica count `k`. Use `VIS-395` when the architecture independently specifies a sectorwise count vector `(k_1,k_2,k_3)` and sector-rate ratios. Use `VIS-396` when the architecture independently specifies one additive shared packet block under a common rate. Use `VIS-397` when the same one-shared-block mechanism has independently specified sector and shared rates. Use `VIS-398` when the source-free architecture specifies any fixed finite additive Gamma incidence network, including subset-specific shared blocks. Use `VIS-399` when the architecture instead fixes a total packet budget, exclusive sector-assignment probabilities, and sector-conditional Gamma amplitude shapes/rates. In the latter case retain the exact face atoms unless the control explicitly conditions on full occupancy, and use the corresponding randomized CDF if a uniformized residual is desired. Do not fit block architecture, packet budget, assignment probabilities, shapes, rates, replica counts, shared-block strengths, or another nuisance parameter to the same candidate residual being judged.
...
The next useful extension must now change mechanism beyond both fixed finite additive Gamma shocks and fixed-budget exclusive Gamma allocation. Examples include genuinely non-Gamma packet-amplitude laws, signed linear combinations or interactions not reducible to positive exclusive allocation, continuous or infinite latent mixtures, or a Wang admissibility constraint that derives a narrower coupled law. A candidate source-specific channel must remain exceptional relative to the resulting independently constrained family across scale/truncation changes fixed without seeing the effect.

Kill the route if the residual is reproduced by the matched controls, if significance disappears after independently calibrated independent-Gamma, finite additive shared-Gamma, or fixed-budget exclusive-Gamma nuisance structure is admitted, if the null acquires enough post-hoc freedom to fit an arbitrary `u` law, if the effect depends on tuned rank/windows, or if the destination rule imports the target arithmetic estimate. A positive source-side residual remains insufficient unless its full cost propagates to the Wang destination without assuming a stronger bound for `G`, `theta`, or an equivalent unsmoothed source quantity.

## Evidence boundary

`VIS-378`--`VIS-386` establish exact finite-dimensional necessary conditions and remove coordinate/amplitude false positives. `VIS-387`--`VIS-389` classify the minimal qutrit residual-shape degree. `VIS-390` gives the exact isotropic one-replica conditional null. `VIS-391` gives the exact sector-scaled one-replica Gaussian extension. `VIS-392` proves that extension is the full source-stabilizer-invariant Gaussian covariance family. `VIS-393` proves that once Gaussianity is dropped, source-stabilizer symmetry alone leaves the sector-weight and fixed-`m_1` residual laws arbitrary. `VIS-394` proves the exact common-count replicated-Gaussian normalized-Gamma control. `VIS-395` extends that calculation to independently specified sectorwise replica counts and rates. `VIS-396` adds one additive shared Gamma block under a common rate. `VIS-397` removes that common-rate restriction. `VIS-398` extends the exact normalized-simplex construction to any fixed finite network of independent additive Gamma blocks with fixed nonnegative incidence/loadings and shows that the conditioned qutrit residual remains exactly specified by finite-dimensional integration. `VIS-399` adds a fixed-budget exclusive-allocation mechanism: it produces exact negative raw-energy cross-covariance and a finite mixture of normalized-Gamma simplex laws, with exact boundary components and fixed-`m_1` calibration.

None of these results proves that an admissible Wang family has an anomalous residual relative to a properly restricted matched control, identifies an arithmetic cause for such an anomaly, or propagates one to a Wang gain. `VIS-399` still does not exhaust genuinely non-Gamma packet laws, signed linear latent factors, interacting allocations, continuous or infinite latent mixtures, arbitrary copulas, or admissibility-induced coupling and does not license fitting the packet-budget mechanism to the candidate residual.

The clue therefore remains `accepted`, but is narrowed again: the next source-specificity test must compare the qutrit residual against mechanistically justified controls whose nuisance degrees of freedom are frozen independently. Independent replicated-Gamma architectures, every fixed finite additive shared-Gamma incidence network, and fixed-budget exclusive-Gamma packet allocation are now exactly calibratable; further extensions need genuinely different source-free structure.

## Research disposition

Outcome: **narrowed**.

For the minimal qutrit route, apply the exact source-free ladder only at the level justified by the mechanism: `VIS-390` for one isotropic block, `VIS-391` for one independently scaled Gaussian block per sector, `VIS-394` for a common independently specified replica count, `VIS-395` for independently specified sectorwise counts/rates, `VIS-396` for one additive common packet block under a common rate, `VIS-397` for the same single-common-block mechanism with independently specified unequal rates, `VIS-398` for any independently specified fixed finite additive Gamma incidence network, and `VIS-399` for a fixed total packet budget with independently specified exclusive sector assignment and Gamma amplitudes. Rejecting those controls is still not source evidence by itself. The next stronger null must earn its restrictions from genuinely non-Gamma amplitudes, signed/interacting dependence outside exclusive positive allocation, continuous or infinite latent structure, Wang admissibility, or another independently justified mechanism, and only a residual surviving that control should proceed to the destination-cost test.