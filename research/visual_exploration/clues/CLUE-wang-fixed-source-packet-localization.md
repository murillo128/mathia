...
After `VIS-403`, the decisive issue is **model restriction, not latent existence**. A useful escape should derive before seeing the candidate a non-vacuous family of allowed latent states, decoders/loadings, interaction kernels, phase relations, support/moment constraints, or continuum limits whose induced residual family has a falsifiable boundary. Merely introducing another finite latent representation is no longer informative, because an unrestricted one-dimensional latent can already encode arbitrary `u`.

## Why it may matter

The qutrit reduction leaves a deliberately small evidential target. Nonuniformity, skew, concentration, boundary mass, positive or negative dependence, unequal counts/rates, non-Gamma amplitudes, sector-local signed cancellation, finite additive common-factor coherence, and even abstract finite-latent conditional independence can all be source-free or vacuous when their nuisance structure is not independently restricted.
...
`VIS-403` does **not** say that every restricted smooth/noisy latent model is universal or that latent models are invalid controls. It shows that finite-latent conditional independence by itself is non-identifying: one scalar latent can encode any qutrit residual if the latent law/decoder are unrestricted. Thus any useful Wang null must obtain its restrictions independently of the candidate.

None of these findings proves that an admissible Wang family has an anomalous residual relative to such a properly restricted matched control, identifies an arithmetic cause for such an anomaly, derives the required Wang admissibility restriction, or propagates one to a destination gain.

## Research disposition

Outcome: **narrowed**.

The clue remains `accepted`. The nuisance frontier has moved past generic finite-latent conditional-independence language. The next coherent test must derive a pre-candidate, falsifiable Wang/source-packet restriction on the latent or interaction family and then test the source residual against that fixed boundary.
