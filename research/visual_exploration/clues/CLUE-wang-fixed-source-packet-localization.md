...
`VIS-394` supplies the first larger non-Gaussian control whose restriction comes from an explicit source-free mechanism rather than symmetry. If every qutrit root sector aggregates `k` independent Gaussian packet replicas, its sector energy is `Gamma(k,lambda_i)`. Normalizing the three independent sector energies gives an exact normalized-Gamma/scaled-Dirichlet density, and conditioning on `m_1` gives the exact residual law

`p_k(u|m_1,q,lambda) proportional_to [prod_i w_i(u)]^(k-1)/[sum_i lambda_i w_i(u)]^(3k)`.

For equal rates this is already nonuniform for every `k>1`: isotropy does not restore the uniform residual once the source-free mechanism aggregates more than one independent packet replica. Thus a nonuniform residual after `VIS-390` can be manufactured by finite packet multiplicity without any arithmetic coupling.

The remaining channel is consequently not just "non-isotropic alignment" or "non-Gaussianity". It is **a residual law that is exceptional relative to separately justified, source-free nuisance mechanisms or matched controls whose parameters are fixed independently of the candidate residual**, together with a structural explanation and a destination-level payoff.
...
Equivalently, after quotienting compact/Vandermonde conditioning, Gram-only spectral filtering, bounded positive preconditioning, rational-expression artifacts, unavailable similarity directions, bare commutator-gap collapse, generator-coordinate gauge, tangent-amplitude gauge, the isotropic defect-energy budget, the universal Markov floor, the exact qutrit one-replica Gaussian ladder, the fact that symmetry alone permits arbitrary non-Gaussian residual laws, and now the exact finite-replica normalized-Gamma ladder, is there a remaining **mechanistically restricted** nonarithmetic control under which the Wang residual is still exceptional?
...
`VIS-393` shows why rejecting that Gaussian family is not enough: full source-stabilizer invariance permits arbitrary distributions of `u`, so symmetry itself supplies no stronger discriminating null. `VIS-394` then shows that a concrete source-free repeated-packet mechanism already produces a non-Gaussian and, for `k>1`, nonuniform residual family without appealing to arbitrary invariant laws. The replica count and sector rates are legitimate nuisance parameters only when the architecture or independent controls determine them before the candidate residual is judged.

This prevents two false escalations: calling every Gaussian rejection source evidence, and calling every nonuniform isotropic residual source evidence. Both can arise from source-free mechanisms that are exactly calibratable.
...
For the generic qutrit rank-one test bed, compute the source cubic invariant, the support `q_1<q_2<q_3`, the moments `m_1,m_2`, and the residual coordinate `u`. Use `VIS-390` only as the isotropic one-replica baseline. Use `VIS-391` when independent sector rates are justified for one Gaussian block per sector. Use `VIS-394` when the source-free packet mechanism independently specifies or constrains a replica count `k` and sector-rate ratios; transform the residual through its exact conditional CDF. Do not fit `k`, `lambda_i`, or another nuisance parameter to the same candidate residual being judged.
