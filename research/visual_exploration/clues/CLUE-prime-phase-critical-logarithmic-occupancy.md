...
The live question is now purely quantitative and discriminating: whether a frozen implementation of that exact kernel realizes the exact one-block law and both high-dimensional linear benchmark families, mixes the nonlinear `bar T`-relevant geometry adequately at the actual critical parameters, and leaves a stable prime residual after that calibration.
...
`VIS-252` and `VIS-253` now give exact high-dimensional scan benchmarks without requiring numerical integration over the conditioned target: ordinary coordinate contrasts and log-coordinate contrasts must both have autocorrelation `rho_m^t` and integrated autocorrelation time `m-2`. This separates a basic implementation/scan failure from genuinely slower nonlinear conditioned geometry.
...
After those mandatory checks, at the actual high-dimensional `m` and `lambda`, test stationarity and mixing from deliberately separated admissible starting configurations using `bar T` and additional nonlinear within-level observables. `VIS-250` establishes exact irreducibility only, `VIS-251` excludes one label bottleneck only, and `VIS-252`--`VIS-253` supply exact additive-coordinate eigenmodes only; none proves the full spectral gap, conductance, practical burn-in, or protection against a slower nonlinear mode or a numerically distorted implementation.

Freeze burn-in, retained-sample count, thinning if any, convergence diagnostics, numerical CDF/root tolerances, the contrast diagnostics, and the final `bar T` comparison rule before examining the prime residual. Require Monte Carlo and numerical uncertainty to be materially below the residual scale. Kill the candidate if the exact low-dimensional or either linear benchmark fails, if different valid initializations do not converge to the same calibrated nonlinear law within the declared budget, if the residual disappears under the exact quotient, if the result depends on post-hoc tuning, or if a stronger representation-matched spacing control reproduces it.
