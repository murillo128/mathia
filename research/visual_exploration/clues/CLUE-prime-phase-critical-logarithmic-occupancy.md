...
The live question is now purely quantitative and discriminating: whether a frozen implementation of that exact kernel realizes the exact one-block law and both high-dimensional linear benchmark families, whether the target-law `delta_(bar T)` already certifies a slower nonlinear scale, whether the remaining nonlinear geometry is mixed adequately at the actual critical parameters, and whether a stable prime residual survives after that calibration.
...
If `delta_(bar T)<2/(m-1)`, treat

`2/delta_(bar T)-1`

as a mandatory lower bound on the integrated-autocorrelation budget for `bar T`; the nonlinear observable is then rigorously slower than the universal linear scan modes. If `delta_(bar T)>=2/(m-1)`, do not infer fast mixing: proceed to the separated-start and long-lag checks because a spectral mixture can retain a slower tail despite a larger one-step dissipation.

At the actual high-dimensional `m` and `lambda`, test stationarity and mixing from deliberately separated admissible starting configurations using `bar T` and additional nonlinear within-level observables. `VIS-250` establishes exact irreducibility only, `VIS-251` excludes one label bottleneck only, `VIS-252`--`VIS-253` supply exact additive-coordinate eigenmodes only, and `VIS-254` supplies a lower-bound diagnostic only; none proves the full spectral gap, conductance, practical burn-in, or an upper bound on nonlinear autocorrelation.

Freeze burn-in, retained-sample count, thinning if any, convergence diagnostics, numerical CDF/root tolerances, the contrast diagnostics, the `delta_(bar T)` estimator and decision rule, and the final `bar T` comparison rule before examining the prime residual. Require Monte Carlo and numerical uncertainty to be materially below the residual scale. Kill the candidate if the exact low-dimensional or either linear benchmark fails, if the `delta_(bar T)` estimate is not stable under the predeclared target-integration checks, if different valid initializations do not converge to the same calibrated nonlinear law within the declared budget, if the residual disappears under the exact quotient, if the result depends on post-hoc tuning, or if a stronger representation-matched spacing control reproduces it.
