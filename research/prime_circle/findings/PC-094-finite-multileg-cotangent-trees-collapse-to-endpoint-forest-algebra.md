...
For `|X|=2`, the conclusion is compatible with the stronger endpoint-displacement normal form of PC-092/PC-093. PC-094 extends the topological obstruction to arbitrary finite external valence, though it deliberately makes only the weaker endpoint-forest claim needed for that extension.
...
Accordingly PC-094 closes **finite acyclic branching**, not all finite cotangent tensor networks. The first genuinely different finite topology left to the accepted cocycle clue is a residual cycle/loopy network or a construction using a second intrinsic operator that is not reducible to the same cotangent edge.

## 7. Prime-blind matched control and RH relevance

The star identity and the tree-elimination proof hold for arbitrary finite disjoint sets of unit-circle points. Neither uses exact order, primality, cyclotomic factorization, or the common anchor. Therefore the same reduction holds on matched non-prime controls with the same graph topology and cotangent rule.

Primitive-shell arithmetic can enter the numerical values of the one-body transforms `T_Bf`, but it cannot alter the Hermite composition law. A claimed prime-specific associator, cocycle, curvature, or spectral carrier derived solely from finite acyclic cotangent branching must therefore first demonstrate information **outside** the endpoint-forest normal form; branching itself is not such information.
...
2. Eliminating hidden variables on a finite tree is standard acyclic factor-graph / sum-product reasoning, already used and audited in PC-093.
...
4. Test a tree with at least two hidden branching vertices by brute-force summation and by eliminating the hidden vertices in different orders. All elimination orders must give the same endpoint function.
5. Replace every primitive shell by matched generic disjoint unit-circle sets. The reduction must remain unchanged.
6. A counterexample in which a finite tree built solely from one copy of `K` per edge leaves an unsummed hidden-shell tensor after recursive Hermite elimination refutes the theorem.

The pairwise-disjoint-shell hypothesis is part of the clean statement. If the same exact shell is reused at nonadjacent vertices, elimination can create coincidence strata and repeated poles; that repeated-shell case is not claimed here and belongs with the cyclic/repeated-edge boundary.

## 10. Consequence for the surviving preimage-tube cocycle clue

`CLUE-preimage-tube-fiber-sector-cocycle` remained accepted after PC-093 partly because **genuinely multileg branching tensors** were still outside the two-leg theorem. PC-094 closes the finite acyclic canonical-cotangent part of that escape:
...
The clue therefore remains live only beyond this class: residual cyclic/loopy or repeated-edge networks, genuinely distinct intrinsic multioperator couplings, infinite-depth/limit constructions, or a cross-level use of the surviving endpoint profiles that escapes the finite-tree reduction. Hardy/Hankel higher-word constructions and global uniformization/monodromy remain separate branches.
