...
PC-335--PC-338 close a substantially larger part of the scalar Mellin escape. PC-335 proves that a bounded shell-independent indefinite Mellin multiplier cannot retain exact mixed-semiprime nulls while preserving a prime-power response. PC-336 classifies distributions supported at the endpoint as generalized-von-Mangoldt jets, and PC-337 shows that finite point-supported singular selectors away from the endpoint are either shell-invisible or collapse to `c Lambda`. PC-338 removes the finite-support loophole: **any compactly supported distribution in Mellin frequency satisfying exact semiprime nulls has response `0` off `Re(s)=1` and `c Lambda` on `Re(s)=1`**. Thus replacing finitely many singular frequencies by a compact continuum, compact singular measure, or compact distribution does not rescue the scalar shell selector.

PC-339--PC-345 close the previously open noncompact/global scalar Mellin branch much further. PC-339 and PC-340 show that at the source-forced critical Gamma strip, finite measures and finite-order global Mellin distributions collapse under the complete shell constraints. PC-343 identifies the finite-measure full-shell kernel exactly with support on the already explicit zeta-zero set, while PC-344 extends the tempered classification to the corresponding zeta-zero jets. PC-345 then gives the sharp ordinary-function boundary: for residual selectors with `e^{b|t|}S in L^2`, the full-shell map is injective for every `sigma>0` when `b>=pi/2`, and has an infinite-dimensional Hardy/Blaschke ghost kernel for every `sigma>0` when `0<b<pi/2`, including zero-free lines. Thus neither the inherited zeta-zero distributions nor subcritical global ghosts supply a critical-line selector.

PC-346 closes the simplest matrix-valued continuation of that branch. Replacing the residual scalar profile by an arbitrary separable Hilbert-valued or finite-matrix-valued profile while keeping the shell kernel scalar and the Mellin readout linear merely tensors the scalar map with the identity. The exact `pi/2` threshold and the entire subcritical kernel are inherited coordinatewise. Pairwise noncommuting matrix values do not help because no matrix product or commutator occurs before integration. Therefore a surviving matrix route must make noncommutativity operative **before** scalar Mellin reduction, for example through a source-forced shell-dependent matrix symbol, multiplication/commutators/orderings, or genuinely nonseparable cross-shell structure. This is persisted in [[research/prime_circle/findings/PC-346-linear-hilbert-matrix-mellin-lifts-inherit-gamma-threshold]].

PC-347 tests the simplest genuinely ordered continuation rather than merely attaching passive matrix values. For the source-native Chen hierarchy of the exact radial one-forms `d log Phi_n(z)`, the complete symmetrization at every finite depth is exactly the tensor product of the Mangoldt endpoint values, so every completely symmetric finite-depth scalar contraction again collapses to `Lambda`. The nonsymmetric word coefficients do retain interior information, but each fixed word is a convergent cyclotomic hyperlogarithmic period; every bounded-depth or nilpotent path ordering therefore stays inside the classical root-of-unity iterated-integral algebra. This is persisted in [[research/prime_circle/findings/PC-347-finite-radial-flux-chen-hierarchy-is-cyclotomic]]. It leaves only source-forced nonnilpotent/infinite-depth holonomy, genuinely changing cross-level state spaces, or other operations not reducible to bounded Chen depth as possible noncommutative continuations of this particular radial hierarchy.

The clue therefore remains accepted only beyond PC-179--PC-183 and PC-335--PC-347. A surviving candidate must introduce **additional geometry-forced information before scalar/positive self-adjoint collapse**: a shell-dependent selector or matrix symbol, an indefinite/sign-changing genuinely two-depth kernel whose sign structure is forced by the roots/refinement geometry, a source-forced nonnilpotent/infinite-depth noncommutative law rather than a bounded Chen word, a second independent ordered/skew carrier, a genuinely changing cross-level operation, or an all-shell/cross-level invariant that is not reducible to the scalar Mellin carriers, positive functional calculus, or finite cyclotomic path-ordering hierarchy already classified. Merely enlarging scalar Mellin support, passing from functions to distributions, weakening the Gamma envelope, replacing scalar residual values by passive matrices, or taking finitely many formal radial commutators is now classified rather than open. The candidate must retain the signed cancellation of at least one non-prime-power control, produce a sign or coercivity margin unavailable in the uncoupled `n=2`/prime-power controls, and avoid reducing to the Möbius/divisor/Lambert/Tornheim/cyclotomic-hyperlogarithm or common-zeta Mellin algebra already exposed by PC-001, PC-027, PC-029, PC-102, PC-179, and PC-347. No such coupling, positivity theorem, or RH implication is established by this acceptance; clue acceptance remains a research disposition, not evidence of proof or novelty.
