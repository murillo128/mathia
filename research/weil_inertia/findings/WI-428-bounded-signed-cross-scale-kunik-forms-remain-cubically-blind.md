...
Now let `nu` be any finite complex or signed Borel measure on `[a_0,\infty)^2` with finite weighted total-variation budget
...
and every fixed bounded linear preprocessing map `T: mathcal H -> K` into any Hilbert space `K` satisfies
