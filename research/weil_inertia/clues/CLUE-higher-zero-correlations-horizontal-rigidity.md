...
`WI-117` closes a second cheap repair: kernel optimization **inside the BGSTB/Tsang positive-definite support-one class** cannot activate a boundary alias. Their admissibility condition `widehat j>=0` together with compact support forces the continuous support-edge trace `j(+-1)=0`; hence every such generalized Tsang profile satisfies exactly the endpoint taper used by the WI-115 Poisson cancellation. Fejer and Montgomery--Taylor are therefore not accidentally screenable choices. A replacement pair kernel must leave that positivity architecture, control the signed cross-height reservoir directly, or cross support one.
...
The established zero-density theorems are no longer a live **black-box** way to discard the Tsang bad-pair reservoir: `WI-116` identifies the missing microscopic density slope and little-`o` saving. Ordinary kernel optimization within the compactly supported positive-definite Tsang class is also closed by `WI-117`. A pair-level route remains live only if it estimates the actual weighted reservoir more efficiently than zero counting, uses a different support-one positivity/signed-cancellation mechanism whose horizontal signal survives the screening lattice, or justifies a support-`>1` alias. If the search escalates to mixed moments or genuine finite `k`-point statistics, it must use information beyond the already-known band-limited **vertical** correlations and must be checked explicitly against the same screening configuration.
...
`WI-116` and `WI-117` make the required information gain more precise. Merely improving an ordinary zero-count exponent, importing the established higher vertical-correlation package, or selecting a different compactly supported positive-definite Tsang profile is not enough. A successful controller must either beat the `e^(2a)` horizontal growth in the actual weighted tail, preserve horizontal information through a genuinely different kernel/alias channel, or supply a statistic whose arithmetic evaluation contains new horizontally coupled information.

A successful controller would attack the exceptional complement directly rather than squeezing the same first-two trace moments harder. A rigorous impossibility result would also be valuable if it shows that a still broader support-one pair-level class compatible with the unconditional Montgomery form factor is screenable, thereby justifying escalation to supercritical support or genuinely new higher correlations.

## Decisive test

Start with the exact Tsang interface in `WI-115`, the density-slope ledger in `WI-116`, and the admissibility no-go in `WI-117`. Derive a decomposition of the evaluated pair sum into the same-height horizontal-defect term and the complementary cross-height contribution. Using only established unconditional inputs, either prove a direct bound on the **weighted** complement strong enough that the horizontal defect survives at order `N`, or construct a zeta-count-compatible screened configuration showing that the proposed controller remains `o(N)`-blind. A route that merely substitutes a known zero-density count into the BGSTB partial-summation argument has already failed this test unless it supplies the missing slope-two little-`o` interface. A route that merely changes `j` while retaining compact support and `widehat j>=0` has also failed: `WI-117` forces the same endpoint taper and screening identity.
...
`WI-115` establishes local Tsang horizontal sensitivity, exact support-one lattice cancellation for endpoint-tapered kernels, `O(log M)` finite-block blindness, and `O(1)` stability of that block comparison under the Montgomery weight. `WI-116` establishes an interface mismatch for the direct published zero-density substitution and a prior-art barrier to generic anti-lattice conclusions from the known band-limited higher vertical correlations. `WI-117` proves that the endpoint taper is automatic throughout the BGSTB/Tsang compactly supported positive-definite admissible class; it does **not** prove that every support-one pair kernel belongs to that class or that every alternative signed pair argument must fail.
...
Accepted and narrowed by `WI-115`--`WI-117`. Pair-level horizontal detectability is established, but the raw support-one Tsang/Montgomery pair statistic is screened at density scale. The direct replacement of the box by **currently established zero-density counts** is closed at the BGSTB weighted-tail interface, the existing band-limited higher **vertical** correlations cannot be invoked as a generic anti-lattice principle, and kernel optimization inside the BGSTB/Tsang compactly supported positive-definite class cannot activate the support-edge alias because that class forces endpoint taper. The live question is now whether one can unconditionally control the weighted cross-height reservoir by a sharper source-specific estimate, construct a support-one observable with a genuinely different positivity/signed-cancellation mechanism that survives the canonical screening extremizer, or obtain a justified supercritical/higher-order horizontally coupled statistic with an established arithmetic evaluation.
