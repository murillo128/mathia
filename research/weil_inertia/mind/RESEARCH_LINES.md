...
The live signed route therefore needs **determining information together with a controlled source budget in both height and depth scale**, but exact constant centered response is now closed inside the full tempered linear scale category. Finite-TV translation mixtures cannot cover expanding ordinate ranges cheaply; finite-TV scale mixtures cannot deliver a positive count quantum uniformly down to arbitrarily shallow defects; and no alternative tempered distribution realizes that exact count quantum with a different budget. A surviving mechanism must relax exact constancy to an approximate or one-sided coercive profile with a quantitatively useful inverse/stability theorem, restrict to finite depth windows and pay their scale length, introduce ordinate-dependent or nontranslation-invariant scale coupling, supply a source-compatible renormalized infinite-budget identity, or add genuinely new arithmetic/spectral information that quantizes horizontal displacement independently.

## Keep budget, margin, defect response and representation dimension distinct

A candidate inertia mechanism must state whether it changes the source budget, reference spectral margin, individual defect response, collective zero density, height-sampling resolution, horizontal-depth coverage, endpoint-amplitude precision, cross-height weight, summability/renormalization law, moment determinacy, or genuinely adds independent information. Matrix enlargement, finer sampling, scalar-profile inversion, critical height weighting, analytic continuation, exponent differentiation, vertical translation and scale mixing affect different entries in that ledger. WI-406--WI-413 sharpen the distinction: a canonical regularized value is not a defect-preserving matched subtraction; a stronger positive local tariff is not enough if the source controls only a finite moment budget; even an infinite list of separately finite moments is weaker than a determining identity; a height-unweighted positive charge is not useful by itself when achieving it forfeits the finite source budget needed for exclusion; moving a probe to the target height does not make uniform coverage free because the conserved single-defect area forces a linear height budget; removing shallow-depth attenuation by Haar-scale mixing does not make a count quantum free because the required scale measure has infinite TV and creates a nonintegrable height tail; and exact invertibility of the centered scale convolution does not improve that budget because the desired constant target has Haar as its unique tempered preimage.