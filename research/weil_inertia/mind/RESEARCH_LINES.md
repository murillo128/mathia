...
WI-336 supplies the complementary high-order obstruction and squeezes the generic escape to a one-order transition. For an interval of length `L` and energy `E` comparable to `L log N`, diagonal zero-increment cubes give

`||f||_(U^s(I)) >= sqrt(E/(2L)) L^(-1/2^s)`.

With `R_s(L,N)=2^s loglog N/log L`, matched-energy Rademacher countermodels remain `U^s`-small when `R_s<=1-eta`, whereas any comparable-energy source is forced to have non-small `U^s` once `R_s>=2+eta`. Since one increment in `s` doubles `R_s`, ordinary normalized Gowers smallness has at most one unresolved binary order window. Below it the statistic is too weak; above it the diagonal energy itself makes smallness impossible.

The live theorem below `theta<3/16` is therefore genuinely **signed two-point control**, unless the single critical-order window admits a new quantitative or diagonally renormalized statistic that demonstrably couples to the same covariance. A viable route is an arithmetic covariance identity, source-fixed diagonal subtraction before sign information is lost, a diagonal-free/renormalized higher-order statistic, or another theorem that directly forces a macroscopic negative off-diagonal contribution at the WI-195 scale. Above `3/16`, the same covariance problem remains together with the location/exceptional-start quantifier.

The accepted local clue `CLUE-bow-distinguished-twist-offdiagonal-cancellation` already carries the destination test: expand the exact distinguished-twist variance with `Lambda^sharp` retained in the same norm and prove either the required negative off-diagonal cancellation or a pointwise obstruction. WI-336 now makes the generic-norm alternative sharply conditional: any Gowers-based contribution must come from the single transition order or a statistic whose diagonal contribution has been explicitly removed; simply increasing the ordinary norm order is no longer an open-ended escape.

## Keep source cancellation, start quantifiers, uniformity complexity, diagonal energy and covariance sign separate

Density-one source cancellation, all-start source cancellation, one-point phase pseudorandomness, fixed-order Gowers uniformity, growing-order Gowers smallness, diagonal-cube energy and pointwise Weil covariance are different statements. WI-331 settles the start quantifier only in the low-theta regime. WI-332--WI-336 show that increasingly rich generic one-point/uniformity information first fails to determine the needed signed covariance and then becomes impossible to keep small because the source's own diagonal energy enters the norm.
