...
The live signed route therefore needs **determining information together with a controlled source budget**. A source-compatible matched actual-versus-criticalized identity must preserve the positive defect under a common counterterm, and its source side must then force exact zero of a positive total, provide transform values rich enough to determine the positive defect measure, couple derivative order to height strongly enough to obtain a determinate moment problem, or add an independent lower/quantization law for horizontal depth. On the half-plane branch, no fixed finite-total-variation scale/translation mixture can supply height-uniform linear-depth sensitivity across expanding intervals; a growing `Theta(T)` translated budget is only useful if an actual source norm can afford it. A non-circular mechanism that targets a sparse exceptional set without uniform height coverage remains outside WI-411, as do infinite-TV/distributional limits, nonlinear translated functionals and genuinely different explicit-formula observables. A finite collection of exponent derivatives, separate finiteness of all fixed derivatives, a tail budget that merely vanishes, or an unweighted defect identity with no finite source bound are not sufficient coercivity statements.

## Keep budget, margin, defect response and representation dimension distinct

A candidate inertia mechanism must state whether it changes the source budget, reference spectral margin, individual defect response, collective zero density, height-sampling resolution, endpoint-amplitude precision, cross-height weight, summability/renormalization law, moment determinacy, or genuinely adds independent information. Matrix enlargement, finer sampling, scalar-profile inversion, critical height weighting, analytic continuation, exponent differentiation and vertical translation affect different entries in that ledger. WI-406--WI-411 sharpen the last distinction: a canonical regularized value is not a defect-preserving matched subtraction; a stronger positive local tariff is not enough if the source controls only a finite moment budget; even an infinite list of separately finite moments is weaker than a determining identity; a height-unweighted positive charge is not useful by itself when achieving it forfeits the finite source budget needed for exclusion; and moving a probe to the target height does not make uniform coverage free because the conserved single-defect area forces a linear coverage budget.