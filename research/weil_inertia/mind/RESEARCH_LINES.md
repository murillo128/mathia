...
With `R_s(L,N)=2^s loglog N/log L`, any comparable-energy source is forced to have non-small ordinary `U^s` once `R_s>=2+eta`. Its all-start Rademacher construction keeps every interval small only below `R_s<=1-eta`, leaving at most one binary-order window for an all-interval theorem.

WI-337 now closes that gap for **almost-all/density-one** information. For every fixed margin below `R_s=2`, one matched-energy Rademacher realization is simultaneously `U^s=o(1)` for all retained orders on a set of starts of density one, and those same good starts retain asymptotically the entire diagonal sliding variance. Hence an almost-all theorem cannot be rescued by allowing the ordinary Gowers order to grow: below the diagonal threshold the black-box output is compatible with full variance, while above it ordinary smallness is deterministically incompatible with the source energy.

The live theorem is therefore quantifier-specific. In the low-bow regime `theta<3/16`, genuinely all-start arithmetic information remains stronger and the narrow all-interval critical window left by WI-336 is still logically available. For an almost-all source theorem, however, WI-337 removes the ordinary-Gowers hierarchy as an open-ended escape. A viable route must instead prove source-specific signed two-point covariance, obtain a stronger all-start input, subtract/renormalize the diagonal cubes before measuring higher-order structure, or define another statistic that demonstrably couples to the same distinguished-bow covariance.

The accepted local clue `CLUE-bow-distinguished-twist-offdiagonal-cancellation` carries the destination test: expand the exact distinguished-twist variance with `Lambda^sharp` retained in the same norm and prove either the required negative off-diagonal cancellation or a pointwise obstruction. WI-337 makes the generic-norm alternative even sharper: for density-one input there is no remaining interval of ordinary Gowers orders to explore; for all-start input only the critical transition window or a genuinely diagonal-free statistic remains.

## Keep source cancellation, start quantifiers, uniformity complexity, diagonal energy and covariance sign separate

Density-one source cancellation, all-start source cancellation, one-point phase pseudorandomness, fixed-order Gowers uniformity, growing-order Gowers smallness, diagonal-cube energy and pointwise Weil covariance are different statements. WI-331 settles the start quantifier only in the low-theta regime. WI-332--WI-336 show that increasingly rich generic one-point/uniformity information first fails to determine the needed signed covariance and then becomes impossible to keep small because the source's own diagonal energy enters the norm. WI-337 shows that weakening the start quantifier to density one lets the generic countermodel reach all the way to the diagonal threshold rather than opening an escape.
