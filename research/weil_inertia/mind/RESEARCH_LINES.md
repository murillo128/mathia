...
The live signed route therefore needs **determining information together with a controlled source budget in both height and depth scale**. Finite-TV translation mixtures cannot cover expanding ordinate ranges cheaply, and finite-TV scale mixtures cannot deliver a positive count quantum uniformly down to arbitrarily shallow defects. The canonical Haar scale-invariant limit attains the desired local quantum only by paying infinite coefficient mass and losing height integrability. A surviving mechanism must supply a source-compatible renormalized identity whose cancellations control that infinite budget, a non-circular way to target unknown ordinates and depths with acceptable total cost, a transform family rich enough to determine the positive defect measure, or genuinely new arithmetic/spectral information that quantizes horizontal displacement. A finite collection of exponent derivatives, separate finiteness of all fixed derivatives, a tail budget that merely vanishes, or an unweighted defect identity with no finite source bound are not sufficient coercivity statements.

## Keep budget, margin, defect response and representation dimension distinct

A candidate inertia mechanism must state whether it changes the source budget, reference spectral margin, individual defect response, collective zero density, height-sampling resolution, horizontal-depth coverage, endpoint-amplitude precision, cross-height weight, summability/renormalization law, moment determinacy, or genuinely adds independent information. Matrix enlargement, finer sampling, scalar-profile inversion, critical height weighting, analytic continuation, exponent differentiation, vertical translation and scale mixing affect different entries in that ledger. WI-406--WI-412 sharpen the distinction: a canonical regularized value is not a defect-preserving matched subtraction; a stronger positive local tariff is not enough if the source controls only a finite moment budget; even an infinite list of separately finite moments is weaker than a determining identity; a height-unweighted positive charge is not useful by itself when achieving it forfeits the finite source budget needed for exclusion; moving a probe to the target height does not make uniform coverage free because the conserved single-defect area forces a linear height budget; and removing shallow-depth attenuation by Haar-scale mixing does not make a count quantum free because the required scale measure has infinite TV and creates a nonintegrable height tail.