...
WI-419 supplies the complementary separated regime for the same bounded-density model. For `N` same-depth targets of depth `d`, minimum ordinate separation `S`, one-target capacity `C_d(Q)` and optimizer radius `r_d(Q)=sqrt(C_d(Q)/(pi B))`, if the one-target optimizer disks are disjoint and `Q-N C_d(Q) arsinh(2d/S)>0`, then

`N C_d(Q-N C_d(Q) arsinh(2d/S)) <= M_B(E,Q) <= N C_d(Q)`.

In particular, if `d->0` and `N/(dS)->0`, then

`M_B(E,Q) ~ N C_d(Q) ~ N Q^2/(4 pi B d^2)`.

For fixed `N`, `dS->infinity` is sufficient. Thus the reciprocal-depth scale `1/d` is a genuine budget-reuse threshold in the proved model: clusters inside `o(1/d)` can asymptotically share one one-target budget by WI-418, while targets separated by `omega(1/d)` have asymptotically additive costs by WI-419. The critical crossover `S~c/d`, varying-depth geometry and accumulated-tail interaction for denser growing target sets remain unresolved.

The live Blaschke route is therefore no longer merely “prove separation.” It must derive **source-forced non-oracular regularity and enough reciprocal-depth-separated target occupation** from independent arithmetic information. Once that is available, WI-419 already converts the local `d^(-2)` tariff into an additive budget in the super-reciprocal regime. What is still missing is a zeta-side theorem placing hypothetical defects in that regime, a varying-depth analogue when depths differ, or a different invariant for the critical/clustered regime. A formal sum of one-target costs remains invalid outside these separation hypotheses.

## Keep budget, margin, defect response and representation dimension distinct

A candidate inertia mechanism must state whether it changes the source budget, reference spectral margin, individual defect response, collective zero density, height-sampling resolution, horizontal-depth coverage, endpoint-amplitude precision, cross-height weight, summability/renormalization law, moment determinacy, probe-placement regularity, cluster separation or genuinely adds independent information. Matrix enlargement, finer sampling, scalar-profile inversion, critical height weighting, analytic continuation, exponent differentiation, vertical translation, scale mixing and source-density regularization affect different entries in that ledger. WI-414--WI-419 sharpen the last distinction: global one-sided scale coercivity still pays Haar-rate local density; finite-window count coercivity pays `T/delta`; passing to a discrete set without non-oracular regularity destroys every TV lower bound; bounded two-dimensional probe density restores an exact one-target capacity asymptotic to `Q^2/(4 pi B d^2)`; same-target clusters can reuse that cost; and super-reciprocal target separation restores asymptotic additivity. None of these by itself supplies the arithmetic theorem that makes regularity and separated target occupation source-forced or turns the resulting tariff into a global RH exclusion.