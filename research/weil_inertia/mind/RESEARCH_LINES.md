...
## Approach the boundary with a source-controlled probe family; fixed-safe-strip processing is intrinsically blind
...
The live question is now to derive **source-side control for a boundary-approaching probe family fixed independently of the hypothetical zero depths**. The full zeta logarithmic derivative has poles and is not globally an ordinary `L^2` function on these lines, so WI-429's finite-product identity does not by itself furnish the required source theorem. A successful route must localize or interpret the full source distributionally, price the approach `a_n downarrow0`, and prove the needed bound from accessible arithmetic data without choosing `a_n` after inspecting an off-line zero. Fixed-safe-strip signed or non-diagonal processing remains excluded; a target-adaptive moving line would merely hide the unknown defect in the probe choice.
