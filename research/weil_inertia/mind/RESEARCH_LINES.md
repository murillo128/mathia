...
The live question is now deliberately localized: choose predetermined probe depths `a_n downarrow0` and height windows `chi_T`, derive a full-zeta localized version of the Hardy identity with every boundary/commutator term explicit, and obtain an independent arithmetic upper bound strong enough to compare with WI-430's finite-height defect quantum. A theorem which simply projects the unlocalized prime Dirichlet series cannot work because that projection is identically zero on the absolutely convergent side; a theorem which drops the localization commutator loses exactly the mechanism by which prime data can enter the positive Hardy half-space. Fixed-safe-strip signed or non-diagonal processing remains excluded, and a target-adaptive moving line would merely hide the unknown defect in the probe choice.
