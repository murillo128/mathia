...
The live theorem must therefore add information rather than recycle the same scalar premise: a genuinely stronger localized arithmetic estimate, a mode/prime-dependent or vector/matrix invariant, an infinite/adaptive architecture whose extra degrees of freedom carry new source information, or a direct attack on the exact comparison sign. A fixed nonzero defect remains mathematically intermediate, but **finite source-exact scalar feedback alone cannot contract it**.

## Track comparison coercivity, compact complexity and feedback information separately

WI-309 placed uniform-tail cost in endpoint singularity; WI-311 moves it into high carrier frequencies; WI-312 proves an anti-concentration/compact-complexity tradeoff. WI-313--WI-316 identify what that complexity ultimately buys: a source-preserving comparison whose fixed threshold and variational bottom converge to the actual compact-support sign problem. WI-317 adds a separate gate: localization can redistribute a scalar lower bound without strengthening it.

Future estimates should keep endpoint regularity, modulation scale, frequency concentration, compact correction size, cap defect, fixed comparison defect, localization-portfolio size, the actual information fed into each localized piece, and any defect-contraction rule as separate resources. A shrinking source-negative margin is no longer independent, a shrinking comparison-floor error is no longer intermediate, and a larger finite localization portfolio is not new arithmetic information when every piece is controlled only by the same scalar semibound.

## Keep compact certificate design separate from full Weil positivity

Finite-dimensional trial-subspace optimization remains downstream of the source-valid information-retention question. The capped source-exact decomposition supplies bounded positive tail margin, while WI-315--WI-316 make its vanishing-cap cofinal semantics exact. Neither fact supplies a nontrivial fixed-defect contraction or the exact lower bound `Dhat >= -(1/2)I`; WI-317 further shows that a fixed finite portfolio of source-exact localizations cannot manufacture that contraction from the same scalar premise.
