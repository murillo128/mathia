...
Thus exact cross-scale identity, common-source membership, orientation and even full generalized event adjacency do **not** by themselves improve the `173/200` frontier. The control is not an ordinary-prime source and not a globally selected CA fan, so the live theorem in the intermediate band must now use information absent from that control: ordinary primality/multiplicative arithmetic at the sampled points `x_q`, the standard/reciprocal prime-race coupling, accumulated CA height, skipped-span effects, or rightmost-maximizer compatibility. A particularly concrete target is a sampled-gap theorem for the actual ordinary-prime sequence conditioned on `q` prime and evaluated near `eta_1^{-1}(eta_2(q))`.

Below `469/600`, the finite mixed-depth reduction from RE-088--RE-090 remains the correct weaker frontier.

## Keep event mass, selected-boundary depth, gap moments, incidence geometry and genuine source arithmetic separate
...
RE-093 sharpens the separation: even the deterministic pairing `q -> eta_1^{-1}(eta_2(q))`, same-source integer labels, orientation and complete generalized staircase adjacency can coexist with the same moment budget up to `173/200`. The remaining obstruction is therefore not merely “which gap is paired with which host” as an abstract incidence relation. It is which **ordinary-prime** gap occurs at those analytic sample points under the actual CA selector, together with arithmetic/selection information that an integer PNT mesh cannot reproduce. The `469/600` and `173/200` numbers remain information-class frontiers for the present host-count/gap-moment method, not intrinsic phase transitions of the CA mechanism.