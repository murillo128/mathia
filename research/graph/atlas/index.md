---
id: RA-index
type: riemann-atlas
atlas_version: 1
status: bootstrap
source_revision: 6baea0d026985c90fcf1446bfa33215af994e2d1
derived: true
---

# Riemann Atlas v1

This is Mathia's versioned map of the **currently identifiable Riemann-hypothesis approach space**. It is a denominator for research coverage, not a percentage of RH solved and not a claim to enumerate every possible future idea.

## Bootstrap dashboard

| Metric | v1 bootstrap |
| --- | ---: |
| Atlas coverage | **69.5%** |
| Live frontier | **41.8%** |
| Hard-pruned | **11.2%** |
| Soft-pruned | **3.0%** |
| Reproduced / prior-art redirect | **13.5%** |
| Unvisited | **30.5%** |
| Macro-families | **10** |
| Leaf territories | **37** |

**Reading:** Mathia has traversed a large fraction of Atlas v1, but the program is **not close to saturation**: about 60.1% of the already-visited mass remains active/open, while only 20.4% of visited mass is pruned.

The current source delta materially narrows several already-mapped frontiers without changing the v1 denominator or any leaf state: Prime Flute now reaches compact relative resolvent against its explicit all-composite control; Prime Lattice pushes its completed-Weil collapse barrier to zero-free sampling scales; Weil Inertia removes the full deterministic four-prime local main and the polylog coefficient regime; Prime Circle closes finite algebraic Hardy-shell essential interference; and Weil Positivity carries its category-changing audit through Prym integral data to a torsion-positivity obstruction. The new Arithmetic Fidelity line is transversal methodology and is therefore not counted as a new RH atlas territory.

The central coverage is robust to moderate weighting choices: independently perturbing each macro-family weight by ±25% in the direction most favorable/unfavorable to coverage yields a **62.7%–74.6% weight-sensitivity band**. This is not a statistical confidence interval.

## Families

- [[research/graph/atlas/families/RA-A-classical-analytic-localization|A — Classical analytic localization / zero density / mollifiers]]
- [[research/graph/atlas/families/RA-B-moments-correlations-inertia-rmt|B — Moments, correlations, inertia, RMT]]
- [[research/graph/atlas/families/RA-C-explicit-formula-positivity|C — Explicit formula and positivity]]
- [[research/graph/atlas/families/RA-D-spectral-operator-hilbert-polya|D — Spectral/operator/Hilbert–Pólya]]
- [[research/graph/atlas/families/RA-E-automorphic-trace-scattering-dynamical|E — Automorphic/trace/scattering/dynamical zeta]]
- [[research/graph/atlas/families/RA-F-equivalent-criteria-deformations|F — Equivalent criteria and deformations]]
- [[research/graph/atlas/families/RA-G-arithmetic-harmonic-cyclotomic-bohr|G — Arithmetic/harmonic/cyclotomic/Bohr]]
- [[research/graph/atlas/families/RA-H-geometric-cohomological-adelic|H — Geometric/cohomological/adelic]]
- [[research/graph/atlas/families/RA-I-noncommutative-qsm|I — Noncommutative geometry / QSM]]
- [[research/graph/atlas/families/RA-J-computational-numerical|J — Computational/numerical]]

See [[research/graph/atlas/v1|v1 model and calibration]] and [[research/graph/atlas/metrics|metrics and saturation reading]].