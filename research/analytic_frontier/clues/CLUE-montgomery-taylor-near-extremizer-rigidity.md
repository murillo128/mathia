...
This predicts two boundary families for the disappearing pair: `t_l→0` when `σ=η`, and `t_l→±2τ` when `σ=−η`. It also predicts the rates `S=O(E)`, `r=O(E)`, and horizontal deviations `O(√E)`. `ANF-064` establishes the sharp infimum and radial coercivity but does not classify all near-minimizers or establish this stability law.

## Why it may matter

The question would turn a sharp infimum into a complete description of how it can be approached. It isolates a one-dimensional curvature-landscape test governing the remaining five-point geometric degeneracy, including a second boundary family hidden by the separate moment bounds.
...
If the curvature test succeeds, combine the displayed exact quadratic excess with the certified quartic remainder of `ANF-064` to prove the lower comparison. Obtain the upper comparison from a uniform local height expansion near each branch, and construct approaching sequences showing the stated rates are sharp. Check the `t_l→±2τ` branches explicitly. No new five-point sign certificate is needed.

## Evidence boundary

The all-order moment inequalities and quartic remainder are now canonical evidence in `ANF-063`--`ANF-064`. They do not identify curvature minimizers, establish their uniqueness or nondegeneracy, or prove the proposed complete stability law. The annulus becomes a constraint on global minimizers only after verifying that the minimum lies below its stated threshold. The clue concerns the fixed five-point Montgomery--Taylor profile and makes no assertion about another spectrum, larger multisets, or RH.

## Research disposition

Accepted as a **rigidity/stability strengthening** of the now-resolved five-point coercivity problem. Research Watch reconstructed the quadratic excess directly from the `n=1` coefficient and checked that the proposed two-family mechanism is compatible with the exact relation `d=t_h−t_l`: if both curvature arguments approach the unique signed minimizers `±τ`, the disappearing pair is forced toward `t_l=0` or `t_l=±2τ`. The `ANF-059` annulus prevents a third simultaneously low-curvature additive configuration because it contains no triple `u,v,u−v` with all three entries in the signed annulus.

A targeted literature check recovered the established Montgomery--Taylor extremal framework and recent work improving the zeta-zero proportion by retaining richer finite-compression information, but no classification of the global minimizers of this exact curvature transform or a two-branch stability theorem for this five-point defect. The live question is therefore precise: certify uniqueness and nondegeneracy of the positive curvature minimizer, then prove or refute the stated two-sided stability law using the exact quadratic linkage plus `ANF-064`'s quartic remainder. Failure of uniqueness or nondegeneracy rejects the law as stated without reopening five-point positivity.