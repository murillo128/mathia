...
**Linked intuitions:** `MI-001-target-aware-gram-geometry-must-carry-absolute-scale` through `MI-043-square-summable-euler-spectrum-caps-canonical-complexity-certificate`.
...
Hence the average singular value decays at least like `X_T N^(-1/4)sqrt(log N)`, and the full NB-176 certificate obeys
