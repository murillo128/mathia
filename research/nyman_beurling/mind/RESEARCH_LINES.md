...
with `G_(jk)=1/(rho_j+conj(rho_k)-1)` and `v_j=1/rho_j`. Optimizing holomorphic contour weights yields the same finite zero-sampling dual problem, not an additional coercive channel. A single high zero contributes only a decaying one-point floor; any Ford-scale order-one obstruction must come from quantitative inverse conditioning of the multi-zero evaluation Gram.

The live source-native question is therefore no longer whether a legal finite contour can see the divisor: it can, exactly. The question is whether the actual Ford-scale zero geometry forces `v^*G^(-1)v` to stay large enough, or whether some genuinely different boundary/nonlinear mechanism supplies information not reducible to the same Hardy point-evaluation span. Zero count, contour residue and target-side evaluation conditioning are distinct resources.

This research line does not promote NB-273--NB-275 and does not use their pending claims as premises. NB-276--NB-279 do not establish `H^2` convergence to the singular current, an order-one Hardy lower bound, or failure of Möbius cancellation; they isolate the exact continued current, close Abel damping and finite interior approximation, and identify the finite boundary route with classical zero-sampling geometry.
...
NB-276--NB-279 add a different source-native route rather than repairing that quadratic channel. The exact Ford current is present only after a singular derivative/continuation operation; Abel smoothing cannot preserve Ford depth while removing the derivative tariff; finite differentiated Nyman sources have the wrong interior divisor-current topology; and the legal finite boundary repair reduces exactly to forced Hardy evaluations at the zeros. Future work must keep finite legal source approximation, singular dual weights, target-side zero-sampling Gram conditioning, meromorphic continuation and Ford-scale localization distinct rather than treating the exact contour identity as an extra Nyman coercivity theorem.
