...
\le
\frac{h}{n^2}
\left(6H_B+8+\pi^2\right).
}
\tag{1}
\]

Thus the adjacent-chaining estimate used in `NB-310`, of order `h^2 log(n)/n^2`, can be replaced by an endpoint estimate of order

\[
\frac{h}{n^2}\left(1+\log\frac Nh\right).
\]
...
and `n=m(q+1)`, equation (3) becomes
...
\[
\boxed{
\frac hm\left(1+H_{\lfloor N/h\rfloor}\right)
\longrightarrow0
}
\tag{5}
\]
...
In the first post-core strip `N/2<m<=N`, one has `q=1`, `s_post(N,m)=beta_(N+1)(m)`, `n=2m=N+h`, and `sigma_2^2=(log 2)/4`. Hence
...
## 1. Exact reciprocal-shell formula for the endpoint comparison
...
\[
\operatorname{dist}(g_n,U_N)\le\|e_{N,n}\|_2.
\tag{11}
\]
...
This identity is the cancellation lost by the adjacent triangle inequality in `NB-310`. Indeed, for

\[
d_j:=g_j-\frac{j-1}{j}g_{j-1},
\]

the adjacent defects still telescope to

\[
e_{N,n}=\sum_{j=N+1}^{n}\frac jn d_j,
\tag{13}
\]

but (12) estimates their sum as one remainder pattern rather than paying separately for all `h=n-N` terms. The Hilbert norm is

\[
\|e_{N,n}\|_2^2
=\sum_{k\ge1}\frac{|e_{N,n}(k)|^2}{k(k+1)}.
\tag{14}
\]

## 2. Block energy of the endpoint remainder mismatch
...
\[
k\bmod N=(s+t_b)\bmod N,
\]

where the representative is taken in `{0,...,N-1}`. Let

\[
E_b:=\sum_{s=0}^{n-1}|e_{N,n}(bn+s)|^2.
\tag{16}
\]
...
For `0<=s<N-t_b`, equation (12) gives numerator `-t_b`; for the remaining `h+t_b` values of `s` it gives numerator `N-t_b`. Hence

\[
E_b
=\frac{(N-t_b)t_b^2+(h+t_b)(N-t_b)^2}{n^2}.
\tag{17}
\]
...
The numerator in (12) has exactly three plateau values: `-(N-u)` on `u` positions, `u` on `N` positions, and `N+u` on `h-u` positions. Consequently

\[
E_b
=\frac{u(N-u)^2+Nu^2+(h-u)(N+u)^2}{n^2}.
\tag{20}
\]

Because `u<h<=N`, the three numerator terms are bounded respectively by `hN^2`, `hN^2`, and `4hN^2`. Since `n>=N`,
...
Equations (19) and (21) imply uniformly for all `b>=1`
...
For `1<=b<=B` use (22), while for `b>B` use (23). Then
...
\frac{|e_{N,n}(k)|^2}{k(k+1)}
&\le
\frac{6h}{n^2}\sum_{b=1}^{B}\frac{b+1}{b^2}
+\frac{2N}{n^2}\sum_{b>B}\frac1{b^2}\\
&\le
\frac{h}{n^2}\left(6H_B+\pi^2+4\right).
\end{aligned}
\tag{25}
\]
...
It remains to handle `b=0`. Formula (12) vanishes for `1<=k<N`. Only the `h` shells `N<=k<n` can contribute. Since `|e_(N,n)(k)|<=1` and `k>=N`,

\[
\sum_{k=1}^{n-1}\frac{|e_{N,n}(k)|^2}{k(k+1)}
\le\frac h{N^2}
\le\frac{4h}{n^2},
\tag{28}
\]

where `n<=2N` was used in the final inequality. Combining (25) and (28) proves (1).
...
The logarithm counts the low-`b` blocks for which the accumulated residue shift `bh` has not yet saturated the trivial block-energy bound.
...
\[
\frac{\|w_{q+1}\|_2^2-\|T_{N,m}w_{q+1}\|_2^2}{\|w_{q+1}\|_2^2}
=
\frac{m\,\operatorname{dist}(g_n,U_N)^2}{\sigma_{q+1}^2}.
\tag{31}
\]

Since `s_post(N,m)` is the supremum of `||T v||/||v||` over the post-core space, (31) and (1) give (3). Now `NB-308` gives

\[
\sigma_{q+1}^2\ge\frac{3}{2\pi^2(q+1)^2}.
\tag{32}
\]
...
Adding the trivial upper bound `1` proves (4), and condition (5) proves (6). This is the central gain over `NB-310`: its adjacent triangle inequality produced a factor `h^2`, whereas the exact endpoint remainder pattern leaves only `h`, with the natural harmonic factor measuring the number of unsaturated residue blocks.
...
More generally, for every fixed core depth `q`, `h=o(m)` is sufficient for (5). The theorem itself retains the exact moving-depth criterion (5), which remains meaningful when `q` also grows.

## 6. Relation to NB-310 and the remaining boundary

`NB-310` established explicit post-core near-isometries close to each staircase edge with the sufficient width

\[
h=o\!\left(\sqrt{\frac{m}{\log n}}\right).
\]

That theorem used the correct first post-core vector, but estimated its escaped product generator by chaining `h` adjacent scaled defects and applying the triangle inequality. The present endpoint identity shows that those adjacent defects cancel strongly in aggregate. The resulting criterion is (5), and in the first strip it includes the full sublinear window `h=o(N)`.
...
\ll
\frac{h}{(N+h)^2}\left(1+\log\frac Nh\right)
\]
...
The proof was checked specifically at the places where a shell argument can lose a case. For `t_b>N-h`, the range `s>=N` produces the third plateau `N+u`; equation (20) includes all `n=N+h` positions. The estimate `N/B<=2h` remains valid when `h>N/2`, because then `B=1`. Block `b=0` is not folded into the `1/b^2` argument and is handled separately in (28). Finally, (3)--(4) use only the exact escape identity from `NB-310`, the endpoint bound proved here, and the pivot floor from `NB-308`; no hidden conditioning estimate is inserted.

The main limitation is directionality: (4) constructs a near-isometric post-core direction when its right side is small, but does not assert that this vector is extremal, that the bound is sharp, or that a comparable lower bound on the defect holds.
...
1-s_{\rm post}(N,m)^2
\ll
\frac hm\left(1+\log\frac Nh\right)
\]
...
Thus the loss of the exact unit-singular core at `m=N/2` is followed not merely by a thin square-root neighborhood of near-isometry, but by a full sublinear staircase boundary layer. Any macroscopic decay mechanism for the projected-dilation norm must occur deeper in the linear-to-quadratic transition, or rely on target occupation rather than source geometry alone.
