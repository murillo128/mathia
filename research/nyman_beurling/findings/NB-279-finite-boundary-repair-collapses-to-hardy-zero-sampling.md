...
If one wants a contour containing `s=1`, the logarithmic derivative contributes the additional residue `-r_P(1)` there. It is canceled exactly by adding `r_P(1)\zeta(s)` to the integrand. Thus the unrestricted version is
...
plus the elementary correction generated by a nonconstant `q` at `s=1`. For the high Ford windows relevant to this line it is cleaner to keep `1` outside `\Omega`, and `(4)` is then the exact identity needed.
