...
`NB-146` and `NB-147` show that positive exterior Poisson sampling has exhausted its leading `Theta(log G)` occupancy budget. A natural next move is therefore to use signed combinations of exterior logarithmic-derivative samples so that the universal `(1/2) log G` term cancels. This finding isolates the exact tax paid by that cancellation: the resulting zero-side Poisson kernel has zero vertical mass, so any positive target lobe must export an equal amount of negative mass elsewhere. In particular, cancelling the Hadamard background by a zero-mass local sampler and retaining global zero-side positivity are incompatible unless the sampler is zero-side trivial.
...
Hence a zero-mass sampler `M_nu=0` removes the common leading `(1/2) log G` term. When

\[
\frac{H_G}{G}\,\|\nu\|_{\mathrm{TV}}=o(\log G),
\tag{9}
\]

it removes that universal budget at leading logarithmic order, leaving `o(log G)` from its variation across the sampling window. Equations `(4)`--`(7)` say that the same zero-mass operation forces zero-side sign loss. A signed scheme can therefore improve on `NB-147` only by supplying genuinely new information that controls where actual zeta zeros meet the negative lobe (or by obtaining compensating cancellation on the prime side). Signed sampling alone does not preserve the positivity mechanism that made `NB-147` unconditional.
...
Fubini is therefore legitimate, and `(11)` yields
...
Integrating `(13)` against the signed measure gives `(8)`, and `(9)` gives the stated `o(log G)` residual when `M_nu=0`.
...
`NB-147` reduced positive exterior sampling to an exact weighted reflection-closure budget. The obvious escape was to cancel that leading budget by taking signed or oscillatory combinations. Equations `(4)`--`(9)` show that this escape necessarily exchanges one resource for another: cancelling the `Theta(log G)` Hadamard background with a controlled zero-mass local sampler creates a zero-mean boundary kernel, and zero-mean localization must carry an equal negative lobe.
...
- The bridge `(8)` assumes the sample ordinates remain in an `o(G)` window around the same large height. To conclude that a zero-mass sampler leaves only `o(log G)` of the universal Hadamard term one additionally needs `(9)`. Without that total-variation control, `(4)`--`(7)` remain exact but `(8)` alone does not make the residual small.
- Samples spread across macroscopically different heights can cancel logarithms by a different weighted identity and are not classified here.
...
- Classical Poisson identity `(11)`: derived directly here.
