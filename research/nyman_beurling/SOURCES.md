...
- Ethan S. Lee and Nicol Leong, *New explicit bounds for Mertens function and the reciprocal of the Riemann zeta-function*, [arXiv:2208.06141v4](https://arxiv.org/abs/2208.06141). Role: explicit Vinogradov--Korobov-scale bound `M(x) << x exp(-c (log x)^(3/5) (log log x)^(-1/5))`; `NB-047` transfers this classical unconditional cancellation through the exact `Omega_M` shell formula to a subexponential dual norm and a doubly logarithmic Burnol-signal localization window.
