...
AF-379 supplies the exact positive regime missing from that obstruction chain. For a continuous translation kernel `f(u-x)`, sampled total nonnegativity is determining whenever the normalized finite row-pattern sets are dense in the full ordered difference cones. An increasing unbounded sampling tail with consecutive mesh tending to zero forces that pattern density at every finite order. In particular the intrinsic logarithmic prime set `E={log p}` has vanishing mesh by the prime number theorem, so all prime-log row minors with arbitrary real columns determine full translation total nonnegativity.

AF-381 closes the remaining obvious sampling asymmetry. If both rows and columns are restricted to the same set `E`, density of the **joint** normalized row/column patterns determines every continuous translation minor; vanishing tail mesh again forces this joint density. Hence all finite prime-ratio minors `det(f(log(p_i/q_j)))>=0` determine global translation total nonnegativity, and under local Hölder regularity any fixed negative target minor has a finite prime-ratio witness at a polynomial inverse-margin horizon. Removing the arbitrary real column coordinate therefore does not restore prime selectivity: the fully prime-indexed observation is still target-complete by topological closure.

The prime-log results relocate the arithmetic question sharply. Once all sampled minors are nonnegative, continuity propagates the certificate to every real row/column pattern and the final Boolean positivity statement has no residual prime specificity. The live question is how an arithmetic construction can intrinsically **produce or control the prime-ratio inequalities** without already importing the full Pólya-frequency target, and what happens in regimes where pattern closure fails but source rigidity may still determine the target. Nonlinear coordinate densification remains invalid unless the translation structure is intrinsic in the new coordinate.
...
Observation horizon, admissible source category, tail class, transform fidelity, inverse conditioning, positivity relation, nominal relational arity, **intrinsic support-scale interaction-component size**, normalized finite-pattern closure, **joint row/column pattern closure**, displayed coordinate geometry, source-category covariance under reparameterization, generated difference subgroup, residual observation gauge and destination slack are distinct resources. AF-375 makes the separable gauge currency exact; AF-376 makes positive row separation an independent obstruction; AF-377 shows that even zero minimum spacing can leave the effective interaction arity uniformly bounded; AF-378 shows that coordinate densification is not a new resource when the source experiment is transported faithfully; AF-379 shows that dense normalized finite row patterns determine every continuous translation minor; AF-381 shows that vanishing mesh also makes fully two-sided prime-ratio sampling target-complete. Future work should state which fibre has actually been removed, which source category is being preserved, whether the observed configurations are merely interacting or actually determining, and whether arithmetic presentation still carries any discrimination after closure.