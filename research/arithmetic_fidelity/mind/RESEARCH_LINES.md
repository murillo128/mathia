...
AF-408 localizes the order-five gate. Since `R_3,R_4>0`, write `A_4(t)=4q(t)-(log R_4(t))''`, so `H_5` has the sign of `A_4`. It proves `A_4(t)=(24/5)e^(2t)(1+o(1))` as `t->-infinity` and `A_4(t)=4e^t(1+o(1))` as `t->+infinity`, hence positivity in both sufficiently remote tails.

AF-409 now makes the right half quantitative. With `x=e^t`, exact first-harmonic matrix factorization plus exponentially small higher-harmonic perturbation gives

`A_4(t) > 4e^t-3 > 0` whenever `e^t>=50`.

Thus the whole right semi-infinite interval `t>=log 50` is certified, including the previously targeted endpoint `t>=4`. The unresolved effective-tail bridge is one-sided: produce a rigorous left cutoff compatible with the compact certificate. The compact-interior sign question remains distinct; if it and the left tail are closed, AF-403/AF-404 globalize strict positivity through order five and move the first possible bad order to at least six.

AF-406 removes one possible misinterpretation of this problem: although coefficient `PF_m` can be manufactured along a zero-preserving exponential gauge, the normalized translation-kernel Toda quantities are invariant under affine exponential profile tilt. AF-407 removes another: the native Laplace transform has no zeta-zero obstruction inside its convergence half-plane, so the finite order-five question cannot be replaced by simply locating continued zeta zeros. AF-409 adds a useful proof pattern: when one harmonic supplies an exactly invertible Hankel reference, tail positivity can be made effective by controlling the logarithmic determinant of the remaining harmonics.

The live locality problem remains layered. First declare the physical wall, sampling regime and same-scale source theorem needed to inspect the profile. Then establish the confluent certificate on that physical source scale. Finally propagate one order at a time through the Toda/log-concavity gate, using effective harmonic dominance where available to remove semi-infinite regions before attacking the finite interaction zone. The `13/30` all-start and `13/15` density-one boundaries remain technology boundaries, not intrinsic detector constants.
...
AF-408--AF-409 add a scale separation inside the finite translation hierarchy itself: the order-five sign is asymptotically settled in both tails, and the right tail is now explicitly certified from `t=log 50` onward without any appeal to zeta zeros. Tail positivity is therefore not evidence that the finite certificate has acquired access to the continued divisor.
