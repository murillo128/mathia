...
AF-399 separates the invariant physical wall from its source-dependent combinatorial coordinate. For any increasing regularly varying scale `a_N` of index `rho>0`, a fixed logarithmic reach `h` has successor depth `B_N(h)/N -> exp(h/rho)-1`. A locally uniform second-order expansion for `log(a_floor(lambda N)/a_N)` transfers directly into the corresponding second-order correction of `B_N(h)`. The prime formula above is the specialization `rho=1`, `h=1`: the exact physical count `pi(e p_N)-N` supplies the prime-density corrections, while AF-397 supplies the one-mesh detector margin.

AF-400 identifies the moving-scale qualification. Ordinary regular variation controls fixed multiplicative ratios but does not determine `B_N(h_N)` for arbitrary shrinking reaches `h_N->0`; a smooth regularly varying counterexample makes `B_N(h_N)/(N h_N)` oscillate. A local source law such as `x f'(x)/f(x)->rho` repairs the conversion and gives `B_N(h_N)~N h_N/rho` when `N h_N->infinity`. For primes, the global PNT error gives the required scale-matched control throughout the polylogarithmically shrinking windows covered there.

AF-401 pushes the prime source-density conversion into a polynomial shrinking regime. Uniform primes-in-short-interval control gives `B_N(h_N)~N h_N` whenever `h_N->0` and `h_N>=p_N^(-13/30+epsilon)` eventually. Thus `h_N=p_N^(-beta)` with `beta<13/30` costs `N p_N^(-beta)` successors, and at the current uniform theorem boundary the breadth is `N^(17/30+epsilon+o(1))`. These exponents price the reach of the available unconditional source theorem; they are not intrinsic `PF_3` locality constants and are not claimed sharp.

The smooth `PF_3` recognition question is therefore no longer missing an analytic boundary criterion or a description of how fixed and polynomially shrinking physical walls are priced on the prime mesh. The live arithmetic question is whether genuine source information forces the sampled local cone together with the required physical relational reach. If a proposed detector needs a wall below the AF-401 short-interval range, it must supply stronger source structure, a justified exceptional-set transfer to prime starts, or a representation whose physical locality is not priced by raw successor breadth. A separate structural frontier is whether higher determinant orders admit any comparably sharp split between local analytic conditions and a finite topological residue.
...
For every proposed destination theorem state separately the observed data, admissible source/tail class, retained analytic layer, determinant/relation order, intrinsic support-scale interaction complexity, placement-generator family, support topology/zero pattern, lower-order zero-minor/interiority pattern, cross-scale compatibility, intrinsic curvature coordinate, normalized pattern closure, coordinate covariance, residual gauge and topology/modulus of inference. AF-394--AF-401 sharpen the multiscale entry: the local analytic order-three cone is completely recognized, connectedness of positive logarithmic curvature is the exact residual global datum, and a physical topology wall must be pulled back through a source-density law that is valid at the wall's own scale before successor depth is interpreted as a resource.