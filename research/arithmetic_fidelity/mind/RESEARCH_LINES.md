...
AF-414 proves that this is a genuine competition scale: inside the double-rank-two Cauchy--Binet sector, every fixed excess partition survives on `x=c/n` with alternating Plancherel/hook weight. AF-415 now closes the uniform partition-summation problem for the full exceptional-`1` double-block sector. After normalization, its complete partition ensemble converges locally uniformly to the positive Cauchy exponential `exp(-c^2/(4pi^2))`. The first negative correction is therefore a Taylor coefficient inside a positive resummed sector, not by itself a diagonal sign obstruction.

The accepted clue `CLUE-left-tail-diagonal-order-drift-scale` is correspondingly narrower. The unresolved diagonal sign can now enter only through exponent families omitting the exceptional exponent `1`, determinant sectors using fewer than two singular-block entries, or interactions between those pieces and the resummed positive sector. The growing partition tail of the exceptional-`1` double-block family is no longer an open gate.
