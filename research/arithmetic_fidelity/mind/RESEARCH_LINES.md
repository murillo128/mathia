...
AF-406 adds a sharper quotient test for finite positivity. For `xi_1(z)=xi(sqrt(z)+1/2)`, Katkova's finite-order theorem gives, for every fixed `m`, sufficiently large `n` with `e^{nz}xi_1(z) in PF_m`, while multiplication by `e^{nz}` preserves the complete zero divisor. Thus fixed finite Pólya-frequency positivity does not descend to the zero-divisor quotient: a zero-preserving exponential gauge can manufacture the certificate without changing the RH-relevant discriminator. Finite positivity can still be a real structural property, but it becomes zero-set evidence only after the zero-free gauge is canonically fixed or the retained datum is itself gauge-invariant.
...
The next local analytic frontier is correspondingly precise. Since `R_3,R_4>0`, order five is equivalent to `(log R_4)''<4q`; proving or refuting that scalar curvature inequality locates the next step of the finite-order hierarchy. AF-406 changes the interpretation, not this local question: even the stronger global condition `PF_m` at any one fixed finite order can be forced along a zero-preserving exponential gauge. A positive fifth Toda gate would therefore be genuine kernel structure but not by itself evidence that a finite positivity hierarchy is converging to a zero-set-faithful RH discriminator.

The live locality problem remains layered. First declare the physical wall, sampling regime and same-scale source theorem needed to inspect the profile. Then establish the confluent certificate on that physical source scale. Finally propagate one order at a time through the Toda/log-concavity gate. The `13/30` all-start and `13/15` density-one boundaries remain technology boundaries, not intrinsic detector constants; the finite-order Toda gate and the zero-preserving gauge gate are separate analytic/fidelity boundaries.

## Separate source identification from zero selection

Hamburger-type rigidity can identify the zeta source without forcing RH. AF-406 strengthens the same separation from the opposite direction: a finite positivity certificate may change while the entire zero divisor is held fixed. Future fidelity arguments must therefore keep source identification, representation gauge, stable recovery, source-specific relational structure and zero selection as separate gates. The same discipline applies to conditional prime-phase incidence programs: exact algebraic incidence is weaker than a stable source theorem excluding the relevant fibre.

For every proposed destination theorem state separately the observed data, admissible source/tail class, retained analytic layer, relation order, intrinsic physical source coordinate, sampling set, same-scale density theorem, exceptional-set transfer if needed, conditioning, residual gauge and final zero/sign discriminator. A source-coordinate count or finite positivity level has mathematical meaning only after those choices are fixed.