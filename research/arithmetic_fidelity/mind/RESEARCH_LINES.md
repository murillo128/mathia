...
AF-372 now closes the most obvious finite reduction of that exact certificate. For every fixed order `r>=2`, the Karlin--Khare family `Omega^(r-3/2)` lies in `TN_r` but not `TN_(r+1)`. Hence testing **all** translation minors at every location up to any fixed maximum size still leaves false-positive fibres on the unrestricted translation-kernel class. The exact Pólya-frequency certificate therefore requires unbounded relational arity unless additional source structure collapses the generic hierarchy.

The live question is no longer whether a sufficiently high fixed determinant order is enough in general. It is whether a materially smaller **structured infinite** family, a growing-order limit with controlled stability, or an arithmetic source restriction can force target-pure fibres without importing the Laguerre--Pólya target itself. Any such proposal must prove its own sufficiency or exhibit the remaining matched controls.
...
Observation horizon, admissible source category, tail class, scalar counting fidelity, smoothing order, principal-part fidelity, functional-equation rigidity, pole residue/normalization, inverse conditioning, destination topology, positivity-signature geometry, **relational arity** and destination slack are distinct resources. AF-369--AF-370 show positively that small retained data can become complete after the admissible fibre is narrowed; AF-366--AF-368 show negatively that apparently accurate observations remain useless while the completion class is too broad; AF-371--AF-372 show that even infinitely many positivity inequalities can remain target-poor when they test the wrong relations, while the correct relational family cannot in general be truncated at any fixed arity. Future work must keep these layers separate until a theorem explicitly connects them.
