...
## Distinguish relational order, placement generators, support topology, lower-order interiority and finite-window curvature transport
...
AF-392 now identifies the exact strictly-positive smooth recognition variable. For adjacent gaps `a,b>0`, let `A` and `B` be the two adjacent integrals of `q`. Continuum `TN_3` is equivalent to

`A+B+B'/B-A'/A >= 0`

for every position and every pair of gaps. Log-concavity is only a sufficient way to force this inequality; small non-log-concave oscillations of positive curvature can still be `TP_3`. In the limit `a,b -> 0`, the criterion reduces to `(log q)''<=2q`, exactly the local condition recovered by the shrinking solid hierarchy. The remaining gap is therefore no longer “non-log-concavity” by itself: it is the difference between an infinitesimal curvature inequality and the complete family of finite adjacent-window comparisons.

The live recognition question is to determine what weaker source/category hypothesis can promote the multiscale solid information to the full AF-392 finite-window criterion, including an appropriate limiting treatment when `q` has infinitely-flat zeros. Quasianalytic continuation and log-concavity are two sufficient mechanisms, but AF-392 shows that neither is necessary in the positive-curvature interior. Any proposed arithmetic application must still derive the relevant solid signs or curvature transport from genuine source information rather than assume target positivity.
...
For every proposed destination theorem state separately the observed data, admissible source/tail class, retained analytic layer, determinant/relation order, intrinsic support-scale interaction complexity, placement-generator family, support topology/zero pattern, lower-order zero-minor/interiority pattern, cross-scale compatibility, finite-window curvature transport, normalized pattern closure, coordinate covariance, residual gauge and topology/modulus of inference. AF-390--AF-392 sharpen the multiscale entry: compatible observations can recover the local boundary jet, but continuum order-three fidelity is governed in the positive interior by a full adjacent-window inequality. A source theorem must bridge that finite-window gap or imply it through a genuine category/shape mechanism.