...
## Decide exact eight-prime incidence after stability and finite exponential-sum separation have both failed
...
AF-320--AF-327 separate exact algebraic recovery, fixed-support conditioning, bounded-support-height geometry, unrestricted support variation and the complete retained `{1,z,z^2,z^4}` Gram margin. AF-328 then reduces the unresolved middle-singular target to one exact common-zero question `F(it)=G(it)=0` and proves that the pair has no common nonunit factor.

AF-329 removes two further escape routes. The dense prime-log orbit approaches an ambient regular-octagon zero of `(e_1,e_4)` with arbitrarily small joint defect, in fact with relatively dense near-hits, so no positive norm margin can separate the prime orbit from the singular target. At the same time `(F,G)` is a proper ideal in the finite exponential-sum ring despite being coprime, so there is no Bézout certificate `AF+BG=1` either.

The remaining question is therefore genuinely **pointwise**: does the specific one-parameter prime-log subgroup enter `V(e_1,e_4)`? A viable proof must use arithmetic/transcendence information about that orbit, an exact identity forcing a forbidden multiplicative relation, or construct an actual hit. Stable conditioning, ambient continuity, common-factor theory and finite exponential-sum ideal separation are exhausted for this predicate.
...
The eight-point branch now separates exact algebraic recovery, fixed-support Diophantine conditioning, bounded-height support-union geometry, unrestricted support-union topology, coordinate-level moment transfer, whole-Gram boundary separation, common-factor exclusion, unit-ideal separation and exact pointwise incidence. Future claims must state which layer they consume rather than treating “finite prime support”, “stable recovery”, “coprime exponential sums” or “zero-free” as one qualitative hypothesis.