...
AF-395 turns that qualitative residue into a quantitative sampling obstruction. It constructs a positive smooth profile with two separated positive-curvature components that satisfies the local order-three analytic conditions on each component. On any vanishing mesh, every sampled `3x3` minor whose combined row/column index span is bounded by a fixed radius is eventually nonnegative, while negative witnesses still exist only at span exceeding the inverse mesh scale. For prime logarithms the mesh tends to zero, so a fixed successor-radius stencil becomes *more* physically local and cannot recover curvature-component membership.

The smooth `PF_3` recognition question is therefore no longer missing an analytic boundary criterion. The live arithmetic question is whether genuine source information forces both the sampled local cone and enough **nonlocal relational reach** to see connected positive curvature, or instead supplies connectedness through an independent source theorem. Increasing resolution without letting combinatorial reach grow is not a substitute. A separate structural frontier is whether higher determinant orders admit any comparably sharp split between local analytic conditions and a finite topological residue.
...
For every proposed destination theorem state separately the observed data, admissible source/tail class, retained analytic layer, determinant/relation order, intrinsic support-scale interaction complexity, placement-generator family, support topology/zero pattern, lower-order zero-minor/interiority pattern, cross-scale compatibility, intrinsic curvature coordinate, normalized pattern closure, coordinate covariance, residual gauge and topology/modulus of inference. AF-390--AF-395 sharpen the multiscale entry: the local analytic order-three cone is completely recognized, connectedness of positive logarithmic curvature is the exact residual global datum, and bounded combinatorial reach cannot recover that datum merely by refining the mesh. Any arithmetic application must derive both the local signs and a source-faithful route to the nonlocal component relation rather than assume target positivity.