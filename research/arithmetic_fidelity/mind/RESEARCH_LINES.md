...
## Classify prime-source incidence and quantitative separation on the eight-point singular stratum
...
AF-316 gives the generic post-hit information law. AF-317--AF-318 show that the eight-point middle singularity `e_4=0` is neither forced into six-point-style antipodal geometry nor excluded by the strongest elementary prime-phase character-rank test. AF-320 then repairs exact representation on the actual eight-prime source class: at a centered common nonzero time, `e_4=0` forces `e_3!=0`, and the fifth harmonic reconstructs the product phase by

`E = (6 s_5 - 5 s_2 s_3)/(10 conjugate(s_3))`.

AF-321 closes the next tempting inference. Rank-one character arithmetic supplies **no quantitative conditioning margin**: exact rank-one middle-singular controls approach a regular octagon with `|s_3|->0`, and the low-harmonic inverse from `(s_2,s_3,s_4,s_5)` to `E` is not uniformly continuous on that class. At the symmetry-enhanced closure fiber the first seven power sums are blind to the product phase, while `s_8=-8E` is the first harmonic separator there.

The live theorem is therefore source-specific. Can the common-time rational-prime orbit actually meet `s_1=0,e_4=0`? If it can, does the full prime-log source give quantitative Diophantine separation from the deeper regular-octagon/torsion stratum, or is an additional retained mark needed for stable transport? Character-rank alone cannot answer this. Exact incidence, exact recoverability and stable conditioning must remain separate.
...
## Keep incidence, exact recovery, stable conditioning and equicoercivity separate

Generic low-harmonic identifiability is no longer the larger-support problem. AF-320 repairs the exact eight-prime singular representation, while AF-321 proves that the currently extracted rank-one source invariant has zero stability margin near a symmetry-enhanced closure fiber. The remaining currencies are first prime-log incidence, quantitative source separation finer than relation rank, target-stable retained marks, and source-derived non-escape conditions for the quartic Euler objective.