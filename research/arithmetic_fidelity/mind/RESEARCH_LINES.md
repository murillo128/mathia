...
The live question is therefore no longer whether **some** nonlinear curvature correction exists for the exact prime-prefix model. It is to classify when the reference data needed by such a correction are themselves source-natural, observable and stable under the broader nuisance/observation classes that the final destination permits. AF-545 makes the target-relative test sharper: prove that the source supplies a shared calibration whose inverse-sensitivity variation is below the destination scale, rather than merely proving small branchwise reconstruction error. A successful extension must also distinguish genuinely shared from differential nuisance directions and survive matched generalized sources. AF-543--AF-545 establish a calibrated normal form, its precision threshold and the general common-mode quotient criterion; they do not yet turn that calibration into a general arithmetic discriminator or an RH mechanism.
