...
AF-330 shows that any exact eight-prime hit forces `z=e^{it}` into the algebraic locus `e^{it}\in\overline{Q}`. AF-331 closes the tempting stability/genericity use of that restriction: the corresponding real exceptional-time set is countable but dense, contains `2\pi\mathbb Q`, has dense complement, and has zero distance from every real time. Every neighborhood may contain both exponentially algebraic and exponentially transcendental times.

The remaining question is therefore strictly pointwise: **can the prime-log one-parameter subgroup hit `V(e_1,e_4)` at one of these algebraically constrained times?** A viable proof must exploit internal arithmetic/transcendence structure of such a time together with the prime phases, derive a forbidden multiplicative relation, or construct an actual hit. Stable conditioning, ambient topology, measure/category genericity, proximity to the exceptional set, near-zero numerics, common-factor theory and finite exponential-sum ideal separation are exhausted for this predicate.

If exact hits exist, the separate downstream question remains whether the finite-height Gram/eigenvalue margin is strong enough in the metric consumed by the final theorem, or which richer provenance than maximum-prime height must be retained.
