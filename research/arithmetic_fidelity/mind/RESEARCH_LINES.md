...
AF-373 shows that unbounded arity does **not** force exhaustive location sampling. On the exponentially decaying class `L`, Ganzburg's `E`-Pólya-frequency theorems prove that all-order minors with one row-coordinate restricted to an infinite set `E` already imply full Pólya-frequency when `E` has an accumulation point, and also for symmetric unbounded sequences `u_s=o(sqrt(s))`. The integer lattice `E=Z` is an exact counterexample: compactly supported `Z`-PF functions need not be PF. Therefore **relational arity and sampling geometry are independent fidelity resources**. A structured infinite certificate can be exact only relative to a declared source class and a determining-set theorem.

AF-374 identifies the hidden quotient behind the fixed-lattice failure. Multiplying a Pólya-frequency profile `f_0` by any positive `a`-periodic gauge `p` leaves every Toeplitz minor whose row locations lie in `a Z` unchanged up to a positive column factor. The ambiguity survives positivity, entireness and Gaussian decay: `exp(-t^2+epsilon cos(2 pi t))` passes every integer-row minor test while failing order-two total positivity for sufficiently large `epsilon`. Thus ordinary regularity does not make one arithmetic lattice determining; the source class must break the periodic gauge or the observation must retain additional lattice phase information.

The live question is now sharper: can an arithmetic source class justify a determining skeleton whose all-order minors are materially more accessible than the full Schoenberg family **and** prove that every observational periodic gauge is trivial? A second phase, an incommensurate sampling geometry, or a source normalization is useful only if a theorem shows that it actually kills the gauge. Reducing location complexity does not remove the unbounded-order bill, and smoothness/analyticity alone does not supply sampling uniqueness.
...
Observation horizon, admissible source category, tail class, scalar counting fidelity, smoothing order, principal-part fidelity, functional-equation rigidity, pole residue/normalization, inverse conditioning, destination topology, positivity-signature geometry, **relational arity, sampling geometry, observational gauge**, and destination slack are distinct resources. AF-369--AF-370 show positively that small retained data can become complete after the admissible fibre is narrowed; AF-366--AF-368 show negatively that apparently accurate observations remain useless while the completion class is too broad; AF-371--AF-372 show that even infinitely many positivity inequalities can remain target-poor when they test the wrong relations or stop at bounded arity; AF-373 shows that location compression can be exact when the source class makes the retained sampling skeleton determining; AF-374 shows why a fixed lattice can remain nonfaithful even inside an entire rapidly decaying class when an exact periodic gauge survives. Future work must keep these layers separate until a theorem explicitly connects them.
