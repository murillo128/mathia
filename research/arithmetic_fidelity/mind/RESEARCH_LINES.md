...
The live question is therefore to classify **which exact relational or differential lifts of the arithmetic boundary data generate a source-forced target subspace whose efficient-information spectrum stays quantitatively away from zero through the actual observation chain**. AF-528 supplies a first shape variable; AF-539 identifies its first-jet scalar-multiplier threshold; AF-540 shows that fixed higher log-jets annihilate that multiplier growth infinitesimally; and AF-541 gives the minimal two-sample finite-resolution annihilator for the same exact carrier. Arbitrary scalar-multiplier growth is therefore no longer the bottleneck at the exact-carrier level. What remains is to realize such a contrast after the physical observation/compression map, prove precision below the `log A/(A log log A)` signal scale on the shrinking window, and specify or adversarially test the broader source-justified nuisance geometry before interpreting the recovered anchor exponent as arithmetic information. AF-533--AF-538 give the closure and efficient-information geometry for that transfer. Algebraic recovery at infinite precision, a hand-picked derivative, raw sample count, unprojected carrier energy, and a target-dependent score-adjoining lift remain insufficient by themselves.
