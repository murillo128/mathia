...
AF-379 supplies the exact positive regime missing from that obstruction chain. For a continuous translation kernel `f(u-x)`, sampled total nonnegativity is determining whenever the normalized finite row-pattern sets are dense in the full ordered difference cones. An increasing unbounded sampling tail with consecutive mesh tending to zero forces that pattern density at every finite order. In particular the intrinsic logarithmic prime set `E={log p}` has vanishing mesh by the prime number theorem, so all prime-log row minors with arbitrary real columns determine full translation total nonnegativity. This is stronger than dense generated differences or merely unbounded interaction components: every target row configuration is approximated, modulo the common translation already invisible to the columns.

The prime-log result also relocates the arithmetic question. Once all sampled minors are nonnegative, continuity propagates the certificate to every row pattern and the final Boolean positivity statement has no residual prime specificity. The live question is therefore how an arithmetic construction can intrinsically **produce or control the sampled inequalities** without already importing the full Pólya-frequency target, and what happens in the middle regime where pattern closure fails but source rigidity may still determine the target. Nonlinear coordinate densification remains invalid unless the translation structure is intrinsic in the new coordinate.
...
Observation horizon, admissible source category, tail class, transform fidelity, inverse conditioning, positivity relation, nominal relational arity, **intrinsic support-scale interaction-component size**, normalized finite-pattern closure, displayed coordinate geometry, source-category covariance under reparameterization, generated difference subgroup, residual observation gauge and destination slack are distinct resources. AF-375 makes the separable gauge currency exact; AF-376 makes positive row separation an independent obstruction; AF-377 shows that even zero minimum spacing can leave the effective interaction arity uniformly bounded; AF-378 shows that coordinate densification is not a new resource when the source experiment is transported faithfully; AF-379 shows that dense normalized finite patterns are sufficient to determine every continuous translation minor. Future work should state which fibre has actually been removed, which source category is being preserved, whether the observed row configurations are merely interacting or actually determining, and which target-changing fibres remain.
