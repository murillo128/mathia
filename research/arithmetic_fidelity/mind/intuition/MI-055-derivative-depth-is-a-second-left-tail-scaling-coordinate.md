...
**Evidence level:** exact synthesis from [AF-416](../../findings/AF-416-full-left-tail-diagonal-has-positive-cauchy-limit.md) through [AF-426](../../findings/AF-426-exact-square-amplitude-has-irreducible-integer-source-packet-gap.md).
...
AF-425 pushes the hierarchy one order deeper and exposes a source-realizability gate. Its formal second-order cancellation target is continuously solvable, but `d_n` is an integer source coordinate. Increasing `d_n` by one changes the exact corrected target by `log(n/m)+m/n+O(n^-2)`, hence by `~log n`, while the required residual window shrinks like inverse powers of `n` times `1/log n`.

AF-426 then closes the exact square-amplitude fibers without an equidistribution hypothesis. If `a=m^2` and the complete adjacent-packet ratio stays bounded, the integer depth must satisfy `d_n=-1` eventually. At that only surviving depth the packet ratio tends to `m K_m>1`, so it cannot approach one; the `o(n^-2)` shrinking target has no solutions. Thus the square transition is not merely hard to tune on the source lattice: on exact square amplitude it has an irreducible integer packet gap.

The reusable lesson is a **nested asymptotic-coordinate hierarchy with an exact source-realizability gate**. Equality at one scale does not control a collision fiber at the next, and solving a continuous cancellation equation is not enough when the physical parameter lives on a discrete lattice. After each tuning, pull the required accuracy back to the admissible source set; an exact fiber may turn out to contain no admissible cancellation path at all.

**Boundary.** AF-426 closes the exact square-amplitude packet-balancing route, not the whole left-tail problem. For `a!=m^2` the balancing depth can move on the `n/log n` scale and remains a genuine inhomogeneous Diophantine problem. Lower saddles, tall-column families, beyond-all-orders effects, the genuinely supercritical regime `s_n/n>2`, other determinant sectors and zero selection remain separate.