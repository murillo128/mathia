...
1. the translation kernel `K_f(u,x)=f(u-x)` is globally `TN_2`;
2. `K_f` is not `TN_3`;
...
For the profile constructed below, every negative sampled order-three minor using tail indices at least `N` must satisfy
...
Nevertheless negative sampled minors exist arbitrarily far out. Hence the missing resource is not sampling density but **nonlocal combinatorial reach**: as the physical mesh shrinks, a witness to the global curvature-component obstruction must span more and more sample indices.
...
The profile is integrable. Since `q` is even and compactly supported, `g'` is a positive constant on the far left and the opposite negative constant on the far right, so `g(t)\to-\infty` linearly at both ends.
...
The failure of `TN_3` can also be exhibited directly. Take column gaps
...
For every `t\in[0,1/4]`, the `A`-window lies inside the zero gap while the `B`-window overlaps the left bump, so
...
Row normalization divides the original translation matrix by positive factors `f(t_i)`, so the corresponding raw `3\times3` translation minor is strictly negative. Thus
...
Since the two open components in `(12)` are separated by distance two, `I` cannot meet both of them.

If `I` meets only the right component, let `q_*(t)=\beta(t-2)`. If it meets only the left component, let `q_*(t)=\beta(t+2)`. If it meets neither component, take `q_*\equiv0`. In every case,
...
On `I`, equations `(14)`, `(30)`, and `(31)` give `(g-g_*)''=0`, so for constants `\alpha,\gamma`,
...
The row and column factors in `(34)` are positive and cannot change the determinant sign. By `(32)`, the determinant is nonnegative. Thus
...
The global negative sign is therefore invisible to every order-three observation whose full argument hull is no longer than the zero gap separating the two curvature components.
...
Then `(38)` is strictly below two, and the locality lemma `(35)` proves every local sampled order-three minor `(4)` nonnegative.
...
Conversely, suppose a sampled tail minor is negative. By `(35)`, its argument interval must have length strictly greater than two. Therefore
...
Combining `(40)` and `(41)` gives `(5)` and `(6)`.

Negative sampled minors do not disappear. The explicit continuum minor `(25)` has a fixed negative margin. Translate all six row/column coordinates by a common large `T`. Because the tail mesh tends to zero, sampled points can approximate the translated row and column configurations simultaneously and arbitrarily well. Translation cancels in all differences, and determinant continuity preserves the strict negative sign. Thus negative sampled `3\times3` minors occur arbitrarily far out.
...
even though `K_f\notin TN_3` and full prime-ratio sampling contains negative order-three minors arbitrarily far out.
...
Since `p_n\ge p_N` for `n\ge N`, equation `(46)` gives `(8)`. Substitution into `(6)` yields `(9)`: the index reach of a negative witness grows at least polynomially along the prime tail.
...
AF-382 found an exact order-two locality mechanism: positivity lets logarithms convert adjacent `2\times2` inequalities into additive mixed differences, and those local generators telescope to every rectangle.
...
A successful source-side argument must provide at least one additional resource: nonlocal prime interactions whose index reach grows with the sampling scale; an independent theorem forcing the relevant positive-curvature set to be connected; or a stronger source rigidity statement that rules out the two-component fibre represented by `(11)`.
...
The prime symbols in `(45)` are not themselves the missing arithmetic discriminator. A nonarithmetic vanishing-mesh sequence has the identical blind spot. Prime specificity would have to enter through a theorem that forces the missing global resource, not through the local sampling geometry alone.
