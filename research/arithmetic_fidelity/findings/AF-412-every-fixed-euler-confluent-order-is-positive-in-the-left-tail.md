...
Combined with AF-411, every bounded confluent order is positive outside an order-dependent compact interval. Therefore any sequence satisfying `|t_j|\to\infty` and `H_{m_j}(t_j)\le0` must have
...
Then `f(t)=u+h(x)`. As in AF-410, `h` is analytic for `|x|<2\pi` and
...
Let `E:=x\,d/dx` and `h_j:=E^j h`. Since `d/dt=E-\partial_u`,
...
Let `\mathcal A=\{1,2,4,6,8,\ldots\}`. Equation `(4)` has the form
...
For every fixed finite derivative window, `(10)` is an absolutely convergent rank-one expansion for sufficiently small `x`. Cauchy--Binet then implies that a nonzero `r x r` minor of `B_m` requires at least `r` distinct exponents from `\mathcal A`.
...
For `m>=2`, put `r=m-2`. Choosing both rows and columns of the block `(7)` in the determinant expansion gives
...
\[
\det M_m=-L_r(x)+O\!\big((u+1)x^{S_{r+1}}\big)+O(x^{S_{r+2}}).
\tag{14}
\]
...
The `m=5` coefficient agrees independently with AF-408/AF-410. Those findings give `H_4\sim x^3/3` and `A_4:=-(\log H_4)''\sim(24/5)x^2`, while AF-404 gives `H_5H_3=H_4^2A_4`. With `H_3\sim x/2`, this yields `H_5\sim(16/15)x^7`, exactly matching `(17)`--`(19)`.
...
- The theorem is fixed-order. Equation `(19)` is not uniform in `m`; the constants and threshold may deteriorate rapidly with order.
...
- The logarithmic coordinate is harmless only because its derivative-Hankel contribution has the exact rank-two form `(7)`. A higher-rank singular germ changes the leading minor size and exponent.
