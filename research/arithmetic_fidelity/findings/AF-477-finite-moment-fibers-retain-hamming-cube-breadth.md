...
when `X` has enough atoms gives the worst-fiber consequence

\[
\boxed{
\sup_u\ \operatorname{dist}_{H,\delta}\bigl(\mathcal F_{r,u}(\Psi)\bigr)
\ge
...
where the left side denotes the best Hilbert bi-Lipschitz condition number at resolution `delta` for the worst nonempty fiber.
...
Every `mu_epsilon` is a probability measure. Equations `(12)` and `(14)` give the same source law for every orientation:
...
- Johann Radon, **“Mengen konvexer Körper, die einen gemeinsamen Punkt enthalten,”** *Mathematische Annalen* 83 (1921), 113–115. Radon's theorem is exactly the finite-dimensional convex-geometric step behind `(12)`: `d+2` feature points admit two disjoint convex representations of one barycenter.
...
The required literature anchors were already present in the Arithmetic Fidelity source ledger through AF-451/AF-460 and AF-464, so no new source entry is needed for this finding.
...
- **Full moment-fiber model is substantive.** The theorem assumes every finitely supported positive law satisfying the declared moment and support constraints is admissible. A thinner arithmetic subclass may forbid the convex mixtures in `(14)` through ordering, integrality, provenance, multiplicative compatibility, or another non-moment relation.
- **Finite affine rank is substantive.** If the retained source law has dimension growing with `r`, or is genuinely infinite-dimensional, `(21)` does not force growing cube dimension.
...
- **TV / `ell^1` geometry is substantive.** The exact Hamming metric in `(17)` uses mutual singularity of the Radon sides and disjoint latent blocks. Another source metric needs its own cube calculation.
