...
Resolved by:
- [[research/weil_positivity/findings/WP-091-schiffer-finite-certificate-is-generic-compact-positivity-not-pole-structure.md]]

The fifth-order certificate can be reconstructed exactly without the Mittag--Leffler expansion. More generally, after a finite-order zero is factored, strict positivity on a compact interval has finite polynomial surrogate certificates by classical approximation theory, with exact interval-polynomial positivity controlled by the one-dimensional Lukács theorem and constructive remainder bounds supplied by standard Taylor-model methods. Conversely, a positive symmetric pole/tail family can have arbitrarily high order of contact at the origin, so the pole representation alone gives no uniform bounded jet order. The two proof styles therefore do not encode a special pole-to-finite equivalence; the Schiffer clue classicalizes and does not create a new Weil-positive geometric mechanism.