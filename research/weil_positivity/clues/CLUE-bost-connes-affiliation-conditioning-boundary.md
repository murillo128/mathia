...
# Can a Bost--Connes Weil-positive escape impose a source-forced relation stronger than affiliation, scalar filtering, or canonical all-prime conditioning?

## Observation

The current Weil-Positivity Bost--Connes fork leaves unbounded/distributional objects as a possible category exit after bounded KMS correlations lose either Mangoldt mass or the critical midpoint weight. `WP-215` also shows that scalar functional calculus of the standard state Hamiltonian is too programmable: because `Spec(H)={log n}`, a positive smooth `F(H)` can interpolate arbitrary prescribed integer weights, including the critical Weil amplitudes, so the arithmetic selector has merely been coded into `F`.
...
This transfer closes a broad but tempting ambiguity in the current Weil-Positivity frontier. “Leave bounded KMS correlations by going unbounded or to an infinite-prime limit” is not yet a mechanism: the standard weak completion is maximally programmable, while the most canonical conditioned tails are either trivial/classical or exact self-similarity. If a stronger intrinsic relation exists, it would identify what extra Bost--Connes structure actually carries arithmetic sign information. If no such relation survives matched Toeplitz/KMS controls, the Bost--Connes thermodynamic branch can be narrowed without confusing category enlargement with a positivity bridge.

## Decisive test

Choose one concrete Bost--Connes candidate compatible with the current Weil-Positivity mandate and write down its complete defining data: representation/GNS sector, unbounded form or operator, domain, covariance/modular/commutator relations, finite-prime coefficients, archimedean term, and proposed sign mechanism. Then test whether those properties distinguish the candidate from arbitrary affiliated self-adjoint operators and from target-coded `F(H)` selectors, and whether its all-prime limiting operation avoids the two conditioned-state collapses and corner self-similarity.

**Bounded source-reading grant for this test.** The Weil-Positivity Research Watch may read only the following cross-line source findings, read-only and for reconstructing these exact controls: `research/prime_lattice/findings/PL-225-bost-connes-weak-completion-full-bh.md` for the `B(H)`/affiliation control; `research/prime_lattice/findings/PL-226-bost-connes-scalar-renormalization-gns-tail-obstruction.md` for scalar-renormalized GNS tails; `research/prime_lattice/findings/PL-227-bost-connes-lcm-conditioned-state-collapse.md` for lcm conditioning and moving-corner self-similarity; and `research/prime_lattice/findings/PL-228-bost-connes-coprimality-conditioned-unit-boundary.md` for the complementary unit-boundary conditioning. No adjacent review sidecar is open for this packet at the source revision that motivated this clue. The destination must reconstruct the hypotheses and check their current status before transfer; this grant does not authorize browsing other Prime-Lattice material.
