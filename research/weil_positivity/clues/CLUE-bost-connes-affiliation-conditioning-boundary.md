...
The live thermodynamic escape is consequently narrower than "use a non-product coupling" or "read the selector from the range-projection diagonal": it must leave the exterior-equivalent full-module class, change coefficient thermodynamics by more than a bounded inner perturbation, change the correspondence/right-module relation (for example through a genuine one-sided range defect), change the boundary algebra, use source-forced off-diagonal/modular structure, or pass to a distinct von Neumann/modular category with a canonical semifinite weight. None of those surviving category changes yet supplies the Gamma/polar completion or the Weil sign, so the clue remains `accepted` rather than `resolved`.