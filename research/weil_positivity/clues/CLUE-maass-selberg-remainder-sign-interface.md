...
Continued investigation is now restricted to a source-defined operator that genuinely couples distinct primitive Eisenstein oldclasses before positivity/scalarization; non-spherical/vector-valued inducing data whose normalized operator retains relevant noncentral arithmetic; or a higher-rank truncation/boundary form with an independent sign theorem that combines the rootwise scalar normalizers before the odd bare-Hessian obstruction applies. Choosing off-diagonal character/Weyl kernels, chamber orientations, tangent directions, absolute values, squares, or root combinations after inspecting unwanted `L`-, Gamma-, polar, or curvature signs remains a hand-picked recombination, not an answer to the clue.