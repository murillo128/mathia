...
Continued investigation is now restricted to a source-defined operator that genuinely couples distinct primitive Eisenstein oldclasses before positivity/scalarization; non-spherical/vector-valued inducing data whose normalized operator retains relevant noncentral arithmetic; or a higher-rank truncation/parameter-space form with an independent sign theorem that combines the rootwise scalar normalizers into the exact Weil functional without target-chosen directions. Choosing off-diagonal character/Weyl kernels or root combinations to cancel unwanted `L`-, Gamma-, or polar terms after inspecting their divisors remains a hand-picked recombination, not an answer to the clue.