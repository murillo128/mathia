...
The direction survives triage, but two natural internal mechanisms are now closed. `WP-237` proves from Wong's exact zeta Maaß–Selberg decomposition that relative subtraction, averaging, differentiation, or any other linear operation acting only on the Arthur height cannot isolate the zero divisor: the entire `T`-independent explicit-formula block cancels or survives intact. `WP-242` further shows that passing to the canonical squarefree `Gamma_0(N)` many-cusp scattering family does not supply the missing matrix direction: after standard normalization its scattering matrix is the same completed-zeta scalar common to every cusp channel times an `s`-independent-Hadamard-diagonalizable tensor of local factors at `p|N`. Fixed channel combinations therefore cancel or retain the global zeta/Gamma/polar package as a whole, while direct unitary-axis Gram positivity is trivial. Continued investigation is restricted to genuinely non-simultaneously-diagonalizable/operator-valued arithmetic before scalarization, or to a source-defined pre-scalar finite--archimedean coupling with an independent sign theorem.