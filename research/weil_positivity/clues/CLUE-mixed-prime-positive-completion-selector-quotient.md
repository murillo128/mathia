...
`WP-100` further narrows the option of retaining the explicit `WP-097` mixed sector unchanged through the global construction. The natural radial family of those affine product factors is equivalent to product Haar only for `sigma>1/2` and is Kakutani-singular to product Haar for `0<sigma<=1/2`; in particular the exact critical carrier at `sigma=1/2` is singular for every fixed finite admissible diagonal `C`. Its finite-cylinder Haar densities also have divergent `L^2` norm at the boundary. Thus the `WP-097` product completion cannot be treated as a regular density perturbation inside the canonical Haar Prime-Torus Hilbert geometry at the Weil exponent.

The remaining question is therefore genuinely global rather than a hidden positive quotient. A surviving construction must either retain the mixed-prime sector in a deliberately singular finite-place representation, replace the independent product completion by a structurally forced correlated completion, or change the finite observable/domain structure before any passive reduction, and then make the exact Weil decomposition emerge only after a single global sign theorem. `WP-100` does not rule out correlated Haar-equivalent completions or singular states. Non-passive relative/off-diagonal or nonlinear reductions remain logically possible, but they must prove their sign independently and cannot inherit it merely from positivity of the `WP-097` carrier.