...
created: 2026-09-19
related: [WP-071, WP-155, WP-157, WP-158, WP-161, WP-162, WP-374, WP-377]
...
The result itself is deliberately narrower than a no-go for analytic or two-dimensional geometry: it classifies the **canonical unweighted `H^2` Gram pairing of the cyclotomic logarithmic potentials**, and blocks any attempt to obtain the exact Mangoldt coefficient from a fixed bounded `H^2` background cross term. However, the broader rotation-invariant weighted-Hilbert escape is already closed by `WP-071`. That finding proves that any positive rotation-invariant Hilbert completion of analytic polynomials with nonzero monomials and bounded ambient evaluation at `z=1` cannot contain even one `F_n=log Phi_n`. Therefore merely replacing `H^2` by a weighted Hardy/Bergman/Dirichlet coefficient norm cannot make the Mangoldt boundary anchor finite-energy while retaining the cyclotomic shells.

What remains outside the combined `WP-071`/`WP-378` obstruction is a genuinely pointed or non-rotation-invariant positive metric, a canonical shell-only quotient whose arithmetic functional is not ambient point evaluation, a singular/nonclosable boundary functional justified by source geometry, an indefinite intermediate pairing, a genuinely finite--archimedean boundary condition, or a construction that couples the finite and archimedean sectors before such an ambient Hilbert completion is formed. Any such variant must be source-forced rather than chosen to insert `Lambda`.
...
A bounded literature search for the structural combination used here did not locate a source presenting the exact pairwise kernel (1)--(2) as a Weil-positivity mechanism. No novelty claim is made for the underlying identities: (1)--(2) are an elementary consequence of classical Ramanujan-sum formulas and Parseval. Relative to `WP-071`, which had already established the general rotation-invariant Hilbert boundary-anchor no-go, the substantive new content of `WP-378` is the exact canonical `H^2` Gram calculation: its nonzero cross-prime terms still Euler-factorize, its diagonal loses prime-power exponent information, mixed composites retain positive energy, and its intrinsic positive geometry is therefore the wrong finite Weil pairing even before the more general topology obstruction is invoked.
...
A surviving analytic-disk route cannot be obtained merely by choosing a different rotation-invariant positive coefficient weight: `WP-071` already rules out that repair once bounded ambient evaluation at `1` and retention of the cyclotomic shell are both required. The source geometry must instead force a topology or coupling outside those hypotheses—most naturally a pointed/non-homogeneous metric, a canonical non-ambient boundary object, or a finite--archimedean construction formed before scalar Hilbert completion—and the same construction must still produce the archimedean/polar terms and an independent global sign theorem.