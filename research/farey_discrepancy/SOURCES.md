...
- Ethan Simpson Lee and Nicol Leong, *New explicit bounds for Mertens function and the reciprocal of the Riemann zeta-function*, arXiv `2208.06141v5` (revised 9 September 2026). Role: primary current source for an unconditional explicit Korobov--Vinogradov-scale estimate of the form `M(x) << x log x exp(-eta (log x)^(3/5)(log log x)^(-1/5))`. `FD-223` uses only this stretched-exponential decay to make the growing switched-block repair fit its shrinking prime reservoirs.
- Kaisa Matomäki and Maksym Radziwiłł, *Multiplicative functions in short intervals II*, arXiv `2007.04290` (2020). Role: prior-art boundary for power-saving exceptional-set estimates in quantitative almost-all short-interval theory. `FD-056` does not claim such estimates are new; it derives the source-specific exceptional exponent required for global cardinality alone to locate a good interval inside the zero-frontier adaptive transfer corridor.
...
- Kaisa Matomäki, Maksym Radziwiłł, Xuancheng Shao, Terence Tao and Joni Teräväinen, *Higher uniformity of arithmetic functions in short intervals II. Almost all intervals*, Inventiones Mathematicae **244** (2026), 967--1091, DOI `10.1007/s00222-026-01408-6`. Role: primary source for Theorem 1.1(i), which gives arbitrary fixed log-power Möbius cancellation for `H>=X^(1/3+epsilon)` outside a global exceptional set of measure `O_A(X/log^A X)`. `FD-056` shows that this global exceptional budget is asymptotically larger than the entire adaptive zero-frontier transfer corridor for every fixed log-power saving when `1/2<Theta<1`, so the theorem alone cannot certify a good transfer start there.
...
- Michael J. Mossinghoff, Timothy S. Trudgian and Andrew Yang, *Explicit zero-free regions for the Riemann zeta-function*, Research in Number Theory **10** (2024), article 11, DOI `10.1007/s40993-023-00498-y`, arXiv `2212.06867`. Role: primary source for the explicit Korobov--Vinogradov zero-free region `sigma >= 1-1/(55.241 (log|t|)^(2/3)(log log|t|)^(1/3))` used in the unconditional prime-density input for `FD-223`.
- Daniel R. Johnston, *Zero-density estimates and the optimality of the error term in the prime number theorem*, arXiv `2411.13791` (current version 22 March 2026). Role: primary source converting a Korobov--Vinogradov zero-free region into an essentially optimal prime-number-theorem remainder `|vartheta(x)-x|/x <= exp(-c (log x)^(3/5)(log log x)^(-1/5))` up to fixed polylogarithmic factors. `FD-223` uses only the resulting qualitative stretched-exponential error after absorbing those factors into the exponential constant.

## Fixed-rank almost-prime counting
...
## Exact-rank prime-factor local laws

- Gérald Tenenbaum, *Introduction to Analytic and Probabilistic Number Theory*, 3rd ed., Graduate Studies in Mathematics **163**, American Mathematical Society, Providence, 2015, DOI `10.1090/gsm/163`. Role: standard Selberg--Delange/Sathe--Selberg anchor for exact central prime-factor counts, including the squarefree variant obtained from `prod_p(1+z/p^s)`. `FD-139` uses the squarefree central-rank formula and tracks exclusion of one prime by deleting the corresponding Euler factor, yielding a rank-cell lower bound uniform in the excluded prime.
- Gérald Tenenbaum, *Moyennes effectives de fonctions multiplicatives complexes*, The Ramanujan Journal **44**(3) (2017), 641--701, DOI `10.1007/s11139-017-9949-7`; correction, The Ramanujan Journal **53** (2020), DOI `10.1007/s11139-020-00297-4`, arXiv `1709.01315`. Role: primary local-law anchor for joint counts of prime factors in arbitrary disjoint prime sets. `FD-139` applies the two-set central local bound to primes `1 mod 4` and `3 mod 4`, obtaining one additional `1/sqrt(log log T)` codimension for the fixed color boundary inside an exact `omega` rank.
...
- Hung M. Bui, Alexandra Florea and Micah B. Milinovich, *Negative discrete moments of the derivative of the Riemann zeta-function*, Bulletin of the London Mathematical Society **56**(8) (2024), 2680--2703, DOI `10.1112/blms.13092`. Role: primary source for Theorem 1.1, which under RH bounds the reciprocal-square derivative moment over zeros separated from every other ordinate by `gg 1/log T` by `O_delta(T^(3/2+delta))`; `FD-273` transfers this well-spaced-zero bound through the discrete consecutive-difference/divisor-replication kernel to force any threshold-sized finite zero-packet coherence onto the close-zero complement. Theorem 1.2 gives the corresponding enlarged-family refinement under the Farmer--Gonek--Hughes conjectural bound for `S(t)`.
