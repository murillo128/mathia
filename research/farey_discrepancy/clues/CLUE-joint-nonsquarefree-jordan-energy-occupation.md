...
[[research/farey_discrepancy/findings/FD-039-one-lipschitz-divergent-shell-profiles-can-evade-nonsquarefree-occupation.md]] shows that this hyperbola-sampling escape is genuinely possible for the generic source properties used so far. A fixed nonnegative one-Lipschitz profile with divergent critical square norm can place asymptotically all selected-horizon energy on one fixed squarefree row while preserving the exact Jordan/hyperbola sampling geometry. Therefore bounded increments, terminal nonsquarefree density, and critical-energy divergence cannot by themselves prove the physical occupation theorem; the exact arithmetic coefficient law must enter.

[[research/farey_discrepancy/findings/FD-040-nonsquarefree-jordan-rows-frame-finite-supercritical-mellin-packets.md]] supplies the first source-spectral restriction on that escape. For every fixed `beta>1/2` and every finite set of logarithmic Mellin frequencies, the nonsquarefree Jordan rows form a coercive sampling frame with a strictly positive generalized Gram eigenvalue. Hence any shell source with a finite dominant asymptotic `n^beta (log n)^q P(log n)` has `liminf U/E>0`; the `FD-039` triangular concentration cannot be generated by a fixed finite supercritical Mellin packet.

The unresolved physical question is therefore no longer whether generic hyperbola sampling alone forbids amplification—it does not. It is whether the actual source, whose coefficient Dirichlet series is `zeta(s+1)/zeta(s)`, can realize an escape through genuinely infinite or non-separated spectral content. A positive solution may come from a finite or quantitatively compact spectral approximation with a uniform nonsquarefree frame constant, or directly from divisibility/multiplicative consequences of `Delta mathcal H(n)=mu(rad n)phi(rad n)/n`. A negative solution would need an actual physical mechanism in which growing Mellin complexity defeats every fixed finite-frame bound. The clue remains accepted because neither alternative is currently established.