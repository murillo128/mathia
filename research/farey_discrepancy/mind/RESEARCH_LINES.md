...
The live question has therefore narrowed further. Positive diagonal weights at or above half-Sobolev all inherit the same shellwide overcoercion on the RH-facing line, while weights below half-Sobolev pay a growing rank-one tariff merely to remove `B_H(1)`. A plausible route should explicitly exploit signed projection, cross-mode/cross-horizon coherence, or another Möbius-specific destination quantity that removes common rows without then requiring uniform positive suppression of the centered reciprocal-prime shell itself.

## Apply information budgets only after fixing source class, observation family and destination norm

The same Farey representation can be complete, target-rigid or permanently noninjective depending on the source class, observation family and destination norm. Above the geometric square-root constant, stable cumulative inversion is available; below it, exact null directions remain. FD-195--FD-199 show that even above threshold the contraction visible in the raw coordinate inverse need not survive compression to quadratic Farey energy, and that the RH-facing positive weighted budgets from half-Sobolev upward simultaneously overcontrol a reciprocal-prime Mertens shell.
