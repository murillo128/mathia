...
FD-252 closes the idea that the special support shape `D subset E union (E+1)` might by itself improve that exponent. Every arbitrary finite divisor support embeds by parity dilation into a genuine exact-omission increment support `2D union (2D+1)` while preserving all even-node incidence-Möbius intervals, hence `kappa(2D union (2D+1))>=kappa(D)`. Support combinatorics alone are therefore as bad, at power-law scale, as unrestricted induced divisor supports.

The live response question is now coefficient-level: define and bound the weighted inverse after composing the incidence inverse with the actual omission-to-increment map `Y|_E -> Delta Y`, whose coordinates occur in adjacent opposite pairs and telescope when omissions touch. Does this paired operator admit a strictly smaller polynomial exponent than the free-coordinate norm `kappa(D)`, or can a polynomial-size exact-omission family realize polynomial amplification even under the pairing constraint? The comparison must still be made against the `N/K` scale needed for macroscopic hidden coefficient mass.
...
The exact hierarchy now separates two response norms: `kappa(D)` prices free increment coordinates on a support and is universally capped by ordered-factorization entropy, while FD-252 proves that exact-omission support geometry cannot improve its extremal growth. Any new response-side coercivity must therefore keep the paired amplitude map rather than relax it away; any source-side theorem must then additionally justify positivity and physical arithmetic admissibility.