...
FD-262 rules out the canonical FD-225 full-grid divisor inverse as a hidden source of that superlinear depth exponent. Its exact identity

`2 c_d = sum_(q|d)(Delta R_N(q)-B_q(N))`

shows that, after maximum block-Mertens forcing and full-grid residual increments are factored out, a dyadic band has only `O(D log D)` total repair mass. For rounded full-grid flattening the residual increment is bounded, so repeated divisor inversion cannot by itself generate the `D^(2-beta)` threshold.

FD-263 sharpens that conclusion by retaining the location of each forcing block. The exact band ledger weights coordinate `q` by `m_x(q)=floor(2x/q)-floor(x/q)`, the number of its multiples in the coefficient band. Deep coordinates `x<q<=2x` have `m_x(q)=1`, so an isolated large deep block is copied only once. In a shell `Q=D^(rho+o(1))` with `K=D^(kappa+o(1))` blocks of relative amplitude `D^(delta+o(1))`, the total contribution is at most `A_N D^(1-rho+kappa+delta+o(1))`. Crossing the general positive-capacity threshold therefore requires `kappa+delta-rho>1-beta`; at `Q~D`, `kappa+delta>2-beta`.

FD-264 then separates that absolute/one-sided ledger from the signed band mean. The same replication count satisfies `m_x(q)=floor(x/q+1/2)`, so its jumps `w_x(q)=m_x(q)-m_x(q+1)` are nonnegative and have total mass `x`. Abel summation turns the signed drift into `x/2` times a **probability average** of `R_N(q)-M(qN)`. In particular `w_x(q)=0` for `x<=q<2x`: a dense population of deep blocks can survive in `sum |c_d|` or `sum(-c_d)_+` while telescoping out of `sum c_d`. At fixed arithmetic amplitude the signed mean has only linear depth amplification, irrespective of block density.

The live source question has consequently narrowed again. In the general signed-increment positive cone, a successful canonical full-grid repair must force enough **one-sided within-band variation** to cross the FD-263 amplitude-density threshold; a merely negative signed band mean cannot use population to do so. Otherwise it must leave full-grid flattening and exploit switched-only freedom whose intermediate residual increments change the source demand, or prove source structure strong enough to move the physical repair into a sharper cone such as `mu*b>=0`. Small improvements of the FD-260 kernel exponent remain secondary unless a concrete source theorem crosses the resulting phase boundary.

FD-258 remains an independent polynomial-depth ceiling. The first switched row forces `liminf log D_N/log N <= 1-Theta <= 1/2` for all-large-`N` realizations with subpolynomial first-row tolerance. This does not touch the present `D=N^{o(1)}` regime, and FD-261--FD-264 show why neither canonical full-grid inversion, isolated deep Mertens spikes, nor a signed deep-band average automatically improves it in the general positive cone.
