...
especially near the balanced calibration `lambda=1`, where the forced coefficient is roughly `X^(0.65--0.67)` depending on shell depth. FD-309--FD-310 show both that classical GRH pointwise theory leaves a fixed-power gap and exactly how much phase-growth improvement would be needed inside the same pointwise template.

The live spectral question is now precise: **prove a principal-frequency estimate whose effective phase-growth exponent crosses the FD-310 barrier, or exploit structure outside that pointwise template**. To beat the source ceiling uniformly one needs `theta<c`; to close the balanced route by this mechanism alone one needs `theta<0.2739244180...`. Alternatively, a theorem may strengthen the source reduction so the forced amplitude/localization improves, or use additional arithmetic structure of the carrying shell that is invisible to a generic bound in `X||alpha||`. Merely re-optimizing major arcs, making a small improvement over the Baker--Harman exponent `1/2`, or invoking classical GRH cancellation is insufficient. Once the Farey divisor-replication geometry has been discharged and only the fixed-block low-frequency Möbius problem remains, ownership naturally moves toward `mobius_cancellation` unless new Farey structure constrains the witness.
...
Inside that rigidity regime FD-298--FD-310 expose the fixed baseline as the block-Mertens source field and then, under a near-capacity premise, as increasingly rigid correlation and low-frequency spectral carriers with an explicit principal-frequency theorem threshold. Outside it, but still in the short-complete-packet range, the unresolved theorem remains near-linear safe occupation or a stronger source-side path-control statement. The current Montgomery--Vaughan phase selector is intrinsically aggregate at zero-gap scale and cannot simply be rerun independently on each RH zero gap.
