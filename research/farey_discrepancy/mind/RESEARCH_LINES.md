...
Equivalently the shell RMS must reach `N^(1/2)x^(c-o(1))`. A uniform fixed-power saving below that threshold rules out the near-capacity route. The same argument forces positive aggregate masked two-point Möbius correlation inside aligned length-`N` blocks, but this is an aligned same-block statement rather than an ordinary fixed-shift Chowla correlation.

FD-301 demasks that aligned correlation exactly. The periodic same-block interval mask has discrete Fourier `l^1` norm at most `2+log N`, so a near-capacity correlation forces some rational additive mode

`sum_(NQ<n<=2NQ) mu(n)mu(n+h)e(an/N)`

of size `Qx^(2c-o(1))` after logarithmic losses. The frequency may be zero, a nonzero reduced denominator need only divide `N`, and the shift may vary with the source parameters. Under RH the same premise also forces a power-sized population of nearly extremal block increments rather than a few isolated intervals.

FD-302 localizes the genuinely twisted alternative. With the FD-301 correlation scale `Y=x^(2c-o(1))`, frequencies above `K_0=ceil(2(N/Y)^2)` cannot carry more than half of the required same-block correlation. Hence either the untwisted mode is already large or there is a nonzero residue with `k(a)=min(a,N-a)<=K_0` carrying the anomaly up to a logarithmic cost. In a polynomial-depth regime `x=N^(lambda+o(1))`, this becomes a shrinking block-Fourier arc once `lambda>1/(4c)`; the near-capacity route itself is already impossible past `lambda>1/(2c)`. In the twisted branch both `h` and `N-h` are forced to be power-sized.

The live deterministic source question has therefore sharpened from a generic block-energy problem to an **untwisted or low-block-frequency two-point Möbius anomaly forced by near-capacity mass**. One route to exclusion is a uniform cancellation theorem for the exact admissible family of shifts and rational frequencies produced by FD-301--FD-302, strong enough throughout the relevant `Q,h,a` ranges; another is to show that the arithmetic source genuinely forces such an anomaly and then prove that enough of it survives divisor replication into `L_N(x)`. These implications remain necessary rather than sufficient: a large additive-twist correlation or shell variance does not by itself produce the required post-convolution `l^1` field. The sign-balance reduction remains conditional on the Mertens growth input used in FD-299, in particular RH for the stated half-power bound.
...
The low-height gate is therefore static rather than a return-frequency problem. Below the FD-296/FD-297 rigidity threshold, the decisive source question is to estimate one fixed baseline `R_(T_bullet,N)` at the physical band scale. FD-298--FD-302 sharpen that baseline question in the regime where the hard complement is `l^1`-close to the block-Mertens field: under RH, one-sided sign polarization cannot decide the near-capacity classification, and any near-capacity `L_N(x)` forces both the aligned block-energy threshold and the untwisted/low-frequency correlation anomaly above. Uniform cancellation for the exact FD-301--FD-302 family would therefore close the low-height near-capacity route before one has to prove a matching `l^1` lower bound.
...
This sharpening remains conditional where stated. FD-297 turns the low-height search into a fixed-baseline dichotomy only under its global background control and zero-safe complete-cluster geometry; FD-298--FD-302 identify a more explicit low-height source field, remove sign polarization under the stated Mertens/RH input, expose a necessary aligned variance tariff, and demask its correlation consequence to a low block-Fourier family, but they do not prove the required lower bound for `L_N(x)` or show that those necessary anomalies are sufficient. FD-294 still requires the occupation and complete-packet hypotheses it consumes outside the rigidity regime, and FD-295 only removes capacity for short complete packets in its stated range. Any attempt to replace the physical grouped remainder by a generic Möbius/Mellin estimate must re-prove that it controls the exact gapwise destination quantity; if it loses the Farey divisor-replication geometry, the natural ownership moves to `mobius_cancellation`.
