...
The hard-Perron frontier has consequently moved from finite-window transference to **admissible-height intersection**. FD-289 does not prevent an externally prescribed sparse sequence of individual cutoffs from landing entirely in the exceptional set, and it does not show that independent remainder, zero-cluster or contour-admissibility constraints intersect the phase-good set. Nor does it control cancellation from `Re(s)<=1`. The next useful theorem must show that the actual admissible Perron cutoffs meet the `1-O(L^(-2))` phase-good heights, or else exhibit a genuine cross-collar/remainder mechanism that cancels the protected witness. If the needed estimate loses the physical divisor-replication geometry and becomes only a generic Möbius Mellin theorem, ownership naturally transfers to `mobius_cancellation`; any Farey-specific gain must retain the grouped physical kernel or cross-collar geometry.