...
FD-184 now iterates that mechanism through **every fixed multiplicative rank**. For every fixed integer `r>=1`,
...
The two source-frontier questions are now sharply different. With moving prime coordinates, determine whether the `x/log x` fan scale is genuinely sharp by constructing a near-threshold low-energy control or proving a stronger obstruction. With fixed prime coordinates, the open problem is no longer another fixed generation but **growing-rank uniformity**: can the FD-184 path/convolution bounds be made quantitative for `r=r(x)` far enough to force near-linear support, or can deterioration in the path threshold and fixed-rank constants be exploited? Dense tiny-amplitude perturbations require a weighted replacement for support sparsity and remain a separate escape.
...
The same midpoint coordinate can encode the full source when observed through changing consecutive horizons, become sparse-target-rigid under multiplicativity, and retain a permanent kernel under deletion-only sources that are coordinatewise indistinguishable from Möbius outside an `o(first-correction)` extremal cap. FD-180--FD-184 add a sharper separation: exact dyadic-null data can coexist with critical prime-extension defect, while the support cost of making that defect subcritical changes from a prime-fan scale on the moving-prime fibre to **every fixed almost-prime layer** when prime coordinates are frozen. Hence “approximately multiplicative” is not inferable from midpoint accuracy, and even a valid relation tariff is meaningless without its source-fibre and rank-uniformity quantifiers.
