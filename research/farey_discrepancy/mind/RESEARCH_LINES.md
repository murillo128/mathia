...
FD-288 shows that this necessary phase event is globally rare in height but does not yet rule it out on the source-selected polynomial windows. After discarding only an `L^(-2)` boundary slice, the maximal strict-inner phase derivative has bounded Besicovitch mean-square energy, whereas the escape requires height `Theta(L)`, so the exceptional set has upper Besicovitch density `O(L^(-2))`. This is not enough to transfer to `[H,2H]` with `H=x^(tau+o(1))`: the effective Dirichlet-series length at the inner boundary is much larger than any polynomial in `x`, and the source may in principle select only exceptional heights.

The hard-Perron frontier is therefore a **finite-window transference problem** rather than a generic right-collar cancellation problem. One route is a local maximal estimate for the near-one phase envelope on the admissible polynomial height windows, strong enough to show that source-selected heights cannot concentrate in the Besicovitch-exceptional set. The alternative is genuine cancellation from `Re(s)<=1` or another contour component, or a changed boundary law from smoothing/modified Perron architecture. If the needed estimate loses the grouped divisor-replication geometry and becomes only a generic Möbius Mellin theorem, ownership naturally transfers to `mobius_cancellation`; any Farey-specific gain must retain the physical grouped kernel or cross-collar geometry.