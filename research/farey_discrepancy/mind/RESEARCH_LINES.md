...
FD-286 removes internal right-collar self-cancellation throughout the sub-Korobov--Vinogradov height regime. The complete strict-right collar keeps an `Nx^(1-o(1))` signed prime plateau unconditionally; the common `q=1` divisor mode is a coordinate-independent real translation and can be absorbed by selecting the reinforcing prime phase sector rather than estimated away. Thus at those heights compensation must come from `Re(s)<=1` or another contour component.

FD-287 pushes the same protection into a genuine polynomial-height range under RH and localizes what failure would require beyond it. Splitting the collar into an exactly phase-controlled outer piece and a near-one inner slice shows that destroying the prime witness by internal right-collar cancellation forces an `Im zeta'/zeta` spike of order `L` inside a strip of width `Theta(1/L)` near `Re(s)=1`. At balanced depth the explicit worst-case bound already excludes fixed height exponents below `0.1456343439...`; outside that window the escape is no longer an unspecified rotation mechanism but a concrete near-one phase event.

FD-288 shows that this necessary phase event is globally rare in height. After discarding only an `L^(-2)` boundary slice, the maximal strict-inner phase derivative has bounded Besicovitch mean-square energy, whereas the escape requires height `Theta(L)`, so the exceptional set has upper Besicovitch density `O(L^(-2))`. Its remaining loophole was transference: a moving polynomial window could in principle have concentrated on that globally sparse exceptional set.

FD-289 closes that continuous-window loophole with the classical Montgomery--Vaughan mean-value theorem. Uniformly for every translated interval `J=[H,H+R]`, the strict-inner maximal envelope satisfies

`||M_str||_(L^2(J)/R) <= sqrt(C_0)+O_eta(1/L+L^3/sqrt(R))`.

Thus whenever `R/L^6 -> infinity`, the proportion of heights in `J` with an order-`L` phase spike is `O(L^(-2))`, uniformly in `H`. The exponent `6` is only a sufficient artifact of the crude maximal estimate, but every polynomial dyadic Perron window automatically lies far beyond that threshold. Under RH and below the inherited sub-square-root height ceiling, a proportion `1-O(L^(-2))` of every such window therefore retains the protected right-collar prime witness.

FD-290 closes the phase-good versus cluster-complete compatibility issue for continuous polynomial windows. If `x=N^(lambda+o(1))` and `J=[H,2H]` with `H=x^(tau+o(1))`, `0<tau<1/2`, then the zero-unsafe set has relative measure at most `tau/[pi(1+1/lambda)]+o(1)`, while the FD-289 phase-bad set has relative measure `O(L^(-2))`. Their complements therefore intersect with positive uniform reserve; at balanced depth the reserve is greater than about `0.9204` throughout the allowed `tau`-range. The zero-safe choice is also cluster-complete: it does not split any reciprocal-log cluster of the FD-279 scale. No statistical independence is required, only the exceptional-measure budget.

FD-291 exploits a structural property of the **full hard-truncation complement** that FD-290 did not use. Between consecutive distinct zero ordinates the hard residue packet is constant, hence so are `E_T`, its physical divisor-replicated remainder and every band norm built from that remainder. Remainder-goodness is therefore a property of whole zero gaps. If a good gap is trimmed by `1/L` at each zero boundary, every point in its safe core is automatically zero-safe while retaining exactly the same remainder value.

Let `Omega_B(J)` be the total length inside `J` of these reciprocal-log-safe cores of remainder-good gaps. The only remaining universal set to avoid is then the FD-289 phase-bad set. Consequently the sufficient intersection criterion is only

`Omega_B(J) >= (C_phi(kappa)+delta) H/L^2`,

not a fixed positive fraction of the window. Equivalently, among gaps of length at least `(2+c)/L`, it is enough for about `H/(cL)` good gaps, an `O(L^(-2))` fraction of the `Theta(HL)` total population in a polynomial window. This is a sufficient weighted occupation criterion, not a zero-spacing theorem.

The hard-Perron frontier has therefore moved from generic remainder-goodness on a positive-measure set to a **good-zero-gap occupation theorem at the physical remainder budget**. The missing theorem must show that safe cores of gaps on which the complete grouped remainder is small have total mass slightly exceeding the `H/L^2` phase-exceptional budget, or exhibit a genuine cross-collar/remainder cancellation mechanism. FD-291 does not prove that any such gap exists, and its exact constancy belongs to the complete hard remainder rather than to individual contour pieces or smoothed cutoffs. If the needed estimate loses the physical divisor-replication geometry and becomes only a generic Möbius Mellin theorem, ownership naturally transfers to `mobius_cancellation`; any Farey-specific gain must retain the grouped physical kernel or the zero-gap/cross-collar geometry.