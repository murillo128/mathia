...
FD-283 now narrows the sign-preserving escape on every strict right subcollar `a<=Re(s)<=c` with `a>1`. After exact physical grouping and the absolutely convergent expansion `1/zeta(s)=sum mu(n)n^(-s)`, integrating in `sigma` multiplies each coefficient mode by the strictly positive Mellin weight

`W_(a,c)(r)=int_a^c r^sigma dsigma`.

The phase `exp(iT log(Nu/n))` is independent of `sigma`, and pairing the conjugate upper/lower edges merely extracts its sine quadrature. Thus neither `sigma`-integration nor edge pairing supplies an independent `1/T` mechanism on the strict-right collar. Any saving there is exactly a Möbius Mellin cancellation statement in the grouped kernel, with local phase scale `Delta n~Nu/T`; the other surviving possibility is cancellation against the adjoining horizontal portion where absolute convergence of the reciprocal-zeta Dirichlet series is unavailable.

The spectral frontier is consequently **cluster-complete, repair-aware and sign-preserving**, but the surviving hard-Perron question is sharper. Tiny cutoff motion cannot control the remainder; pointwise Perron majorization is quantitatively too late; an already-grouped physical kernel retains a depth plateau if made absolute; and on the strict-right collar, delayed `sigma` integration does not create new oscillation mode by mode. A viable continuation must prove a genuine coefficient-space Möbius Mellin cancellation estimate after physical grouping, exploit cancellation with the near/left-of-one collar, or change the collar law through smoothing/modified Perron architecture. If the physical divisor kernel becomes inessential and the needed estimate reduces to a generic Möbius Mellin theorem, that subproblem belongs naturally to `mobius_cancellation`; any Farey-specific gain must retain the replication kernel or cross-collar geometry.