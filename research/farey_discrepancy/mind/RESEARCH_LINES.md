...
The hard-Perron frontier has consequently narrowed to the **remainder-goodness intersection** inside that positive reserve. For analyst-chosen continuous polynomial windows, phase protection and zero-cluster completeness can now be imposed simultaneously; the missing theorem must show that the left-contour/full-Perron remainder is good on enough heights to intersect this surviving set, or exhibit a genuine cross-collar/remainder cancellation mechanism. A sparse externally prescribed sequence of cutoffs remains a separate loophole because positive window measure need not hit an arbitrary sparse sequence. If the needed remainder theorem loses the physical divisor-replication geometry and becomes only a generic Möbius Mellin estimate, ownership naturally transfers to `mobius_cancellation`; any Farey-specific gain must retain the grouped physical kernel or cross-collar geometry.