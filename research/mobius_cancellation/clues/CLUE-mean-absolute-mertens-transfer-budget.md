...
based_on:
  ...
  - research/mobius_cancellation/findings/MC-135-dyadic-star-nullhomotopy.md
---
...
`MC-134` audits the first inter-scale escape. Over every field, the reduced persistent barcode of the actual filtration `Delta_1 subset Delta_2 subset ...` consists exactly of bars `[m,2m)` in degree `Omega(m)-1`, one for each odd squarefree `m>1`. Hence every reduced homology map across a factor-two scale gap is zero and every logarithmic lifetime is exactly `log 2`.

`MC-135` strengthens that obstruction from homology to the spaces themselves. For every `b>=2a`, the whole earlier complex `Delta_a` lies inside the prime-`2` star of `Delta_b`: an odd squarefree simplex `P(m)` extends to `P(2m)`, while an even simplex already contains `2`. The star is a cone, so the actual inclusion `Delta_a -> Delta_b` is null-homotopic. The factor `2` is sharp because an odd prime vertex `p` remains isolated throughout `p<=b<2p`. Thus ordinary homotopy-invariant transport of this one-parameter filtration is already trivial after one dyadic step, not merely its persistent homology.
...
After `MC-135`, the divisor-topology residual can no longer be ordinary single-parameter persistent homology **or any other carrier that factors only through the ordinary homotopy types and inclusion maps of Björner's filtration**. Especially interesting possibilities are raw occurrence order; essentially full-resolution or genuinely non-polynomial/adaptive temporal information; nested or noncommutative multiscale relations that cannot be flattened into a longer finite context or bounded moment family; critical-or-larger effective memory; genuinely non-additive state; arithmetic labels or chain-level divisor relations not invariant under the prime-`2` homotopy contraction; higher coherent data not determined by the pairwise homotopy-category maps; a genuinely different/multiparameter arithmetic filtration; exact multiplicative constraints; or another arithmetic law whose information budget can be bounded explicitly.
...
The negative findings now calibrate both generic and exact arithmetic representations. A candidate cannot justify itself merely by combining local memory, modular occupation, coarse time labels, finite-horizon empirical paths, periodic clocks, polynomial timestamp moments, static divisor homology, persistent homology, or another ordinary homotopy invariant of the same divisor filtration. `MC-129`--`MC-132` match the generic summaries in explicit subcritical regimes; `MC-133` shows static homology has linear unsigned mass and an alternating projection equal to `M`; `MC-134` classifies the one-parameter barcode; and `MC-135` shows that the actual inclusion maps themselves factor through a contractible prime-`2` star across a factor-two gap. Any positive mechanism must identify extra structure not exhausted by these collapses without merely storing the endpoint.
...
Kill a candidate if its input already contains an RH-equivalent Mertens/Riesz/coarse mode; if inversion reintroduces the original partial-sum burden; if its information factors through the local-histogram, additive-state, modular-transition, time-binned, cross-coupled one-step, finite-horizon empirical path, periodic-clock, or polynomial/quasi-polynomial timestamp-moment summaries covered by `MC-122`--`MC-132`; if a static divisor-topology invariant reduces to the Betti/Euler package in `MC-133`; if an inter-scale proposal factors through the ordinary one-parameter persistence module in `MC-134`; **or if it is merely another homotopy-invariant functor of the same inclusions, whose factor-two maps are null-homotopic by `MC-135`**. Also kill it if the apparent gain disappears after exceptional-set, smoothing, weighting, and scale losses are restored.

A candidate survives this obstruction surface only if it uses information the current controls do not match: genuine occurrence order or near-full timing resolution; a multiscale relation not reducible to one longer empirical context or bounded polynomial-moment family; effective memory at or above the unresolved critical scale; non-additive arithmetic state; divisor/prime structure beyond static Betti data, the dyadic barcode, and ordinary homotopy-level transport; exact multiplicative constraints; or another proved Möbius-specific relation excluding both the matched reorderings and exact reconstruction collapses.
...
`MC-133`--`MC-135` are exact arithmetic/topological audits with deliberately limited scope. `MC-133` closes single-scale ordinary homology/homotopy; `MC-134` closes ordinary one-parameter persistent homology by determining its full barcode; `MC-135` closes ordinary long-range homotopy-category transport by showing that every inclusion across a factor-two gap factors through a contractible prime-`2` star. None analyzes arithmetic labels attached to chains, non-homotopy-invariant chain-level statistics, higher coherent choices beyond the individual maps, genuinely different or multiparameter filtrations, or another Möbius-specific relation not determined by this topology. Those remain possible escape surfaces, not evidence that any one of them works.
...
**Accepted, further narrowed by `MC-135`.** The live residual is a source-to-amplitude theorem using genuinely occurrence-ordered or essentially full-resolution timing information, non-flattenable multiscale structure, non-additive or critical-memory state, or Möbius-specific arithmetic structure that survives the generic order-forgetting controls and is not exhausted by the static, persistent-homological, or ordinary homotopy-level topology of the exact divisor filtration.