...
The next decisive test is therefore no longer to swap in another finite zero-mean local linear statistic or merely to choose a wavelet/frame decomposition with an explicit top scale. It is to produce the **coarse Riesz mode from independently weaker arithmetic information**: for example a nonlinear or multiplicative coupling in which controlled detail data force the coarse coefficient, or a source-natural global statistic whose relation to the Riesz mode has a proved polynomial gain without importing zero location. A matched multiplicative exact-support comparator satisfying the proposed detail/local hypotheses while its Riesz coarse mode remains too large would kill that candidate. The clue remains accepted rather than resolved because no such arithmetic production mechanism is currently established.
