...
`MC-119` removes the immediate finite-scale-density repair. The same balanced-block construction can be chosen so that, for a fixed `theta>1`, the RH-scale fractional bound holds **uniformly at every cutoff `Q<=X<=Q^theta`**, up to a polylogarithmic factor, while `D_1(Q)` still has exponent strictly above `1/2`. The admissible polynomial horizon is explicit: for each `p<1`, every `theta<(5-3p)/(4-2p)` is achievable with some super-square-root excursion exponent. Thus continuity of the weak statistic across all intermediate scales in a substantial finite window is still not the missing recovery principle.

`MC-120` now removes the moving-comparator escape on an **infinite but lacunary** family of windows. One fixed exact-support multiplicative function can carry balanced prime-block excursions at infinitely many source scales while its fractional `p`-moment is `X^(1/2+o(1))` uniformly throughout each associated polynomial window. The construction deliberately sends consecutive source scales extremely far apart to absorb the finite deterministic cost of all older blocks. It therefore does not touch the `MC-116` subpower-dense checkpoint condition. What survives is not mere one-function consistency over an unbounded sequence, but dense/overlapping-scale coherence or genuinely Möbius-specific joint structure.
...
After `MC-120`, a viable source condition must use information absent even from one fixed exact-support multiplicative comparator that satisfies the weak statistic on infinitely many separated polynomial windows. Exact agreement with `mu(p)=-1` at every prime, together with exact support and multiplicativity, is not a useful intermediate condition because `MC-005` already notes that it reconstructs Möbius tautologically. The live candidates are therefore **quantitative intermediate** conditions: one-function coherence on a subpower-dense or overlapping family of scales, a genuinely joint correlation-plus-multiplicativity constraint, a quantitative prime-local law weaker than exact identification, or another exact Möbius identity that forbids balanced prime-block transport without encoding the target itself.
...
The checkpoint reduction makes the output surface smaller: a source-natural argument that only produces estimates at selected resolutions can still be sufficient if consecutive checkpoint logarithms have ratio tending to one. Conversely, matched controls `MC-117`--`MC-120` show progressively that weak sub-`L^1` information can coexist with excessive first-absolute mass after exact support, qualitative correlation information, multiplicativity, continuous finite-window scale coverage, and finally one-function recurrence across infinitely many **lacunary** windows are added separately. The remaining question is whether arithmetic coherence at the scale density actually needed by `MC-116`, or a genuinely joint Möbius-specific constraint, prevents the rare balanced excursion itself.
...
2. construct a **single fixed-function or equally source-faithful** matched control satisfying the proposed local/multiscale hypothesis on a qualifying subpower-dense or overlapping scale family while its mean-absolute partial-sum process retains exponent `>1/2`.

A candidate fails if the proposed transfer input already contains an RH-equivalent fixed Riesz/coarse mode, if smoothing makes the desired scale vacuous, if inversion reintroduces the original partial-sum burden, if the only gain comes from a triangle inequality that spends the polynomial saving on exceptional mass, if a sub-`L^1` compression discards rare coherent amplitude without a source-specific recovery mechanism, if it relies only on generic multiplicativity or exact square-free support, if merely demanding the same weak bound throughout finite polynomial windows is its recovery mechanism, if merely requiring one fixed function along an arbitrarily lacunary infinite sequence is its recovery mechanism, or if the controlled checkpoints are power-lacunary and no additional source-specific interpolation pays the resulting exponent gap.
...
`MC-117` is a sharp generic transfer obstruction plus a nonmultiplicative exact-support qualitative-Chowla control. `MC-118` restores exact support and multiplicativity and transports the weak statistic to a future subpower scale. `MC-119` strengthens that same multiplicative control to every cutoff throughout a fixed polynomial window. `MC-120` places infinitely many such bad windows inside one fixed multiplicative function, but only by choosing the source scales very lacunarly. The `MC-118`--`MC-120` comparators still do not preserve Möbius's quantitative Chowla information or exact prime law. These findings therefore do **not** rule out a theorem for the actual Möbius function that exploits subpower-dense one-function coherence, a genuinely joint quantitative correlation/multiplicativity condition, or another intermediate arithmetic constraint unavailable to the comparator.
...
**Accepted, narrowed by `MC-115`--`MC-120`.** Earlier work eliminated several tempting but over-strong or information-losing carriers. The Tanaka/excursion branch showed that long excursions can coexist with diffusive mean-absolute behavior; the path-energy branch exposed an RH-equivalent first Riesz coarse mode; fixed-order Riesz smoothing remains RH-equivalent, while growing-order smoothing either becomes normalization-vacuous or delays endpoint visibility and requires a nontrivial inversion carrier. `MC-117` ruled out sub-`L^1` time moments as a free relaxation even with exact support, qualitative Chowla, and subpower-dense scale coverage. `MC-118` showed that generic multiplicativity does not repair that loss. `MC-119` showed that making the weak multiplicative control uniform over every scale in a polynomial window does not repair it. `MC-120` now shows that **using one fixed multiplicative source on infinitely many such windows still does not repair it when the windows may be lacunary**.

The active residual is consequently more specific than one-function consistency. A surviving statistic must preserve first-moment sensitivity to rare coherent amplitude at a scale family dense enough to close the `MC-116` interpolation gap, or impose genuinely joint correlation/prime-local structure that prevents the balanced-block mechanism. The clue remains accepted rather than resolved because no polynomial transfer from such source-faithful information to `D_M` has yet been established.