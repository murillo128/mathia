...
based_on:
  ...
  - research/mobius_cancellation/findings/MC-162-absolute-commutator-stability-power-equivalence.md
  - research/README.md
---
...
MC-162 closes the converse temptation. For the explicit absolute contraction factors of MC-160/MC-161, `eta<1` implies that the forward transform is bounded on the same `x^theta` power space as the inverse, so `F_w(x)=O(x^theta)` (or the raw `T_a` analogue) is equivalent to `M(x)=O(x^theta)`. The same inequality also forces the transfer factor to be zero-free on `Re(s)>=theta`. Therefore **tuning another absolutely contractive weight is not itself an intermediate quantitative gain**; it only supplies a well-conditioned equivalent representation.

The live scalar question is correspondingly narrower. A fixed transformed input can still justify retaining the scalar route only if there is a genuinely independent source-side proof mechanism for its target-range estimate, not merely the existence of an `eta<1` decoder. Alternatively, one must prove a signed Möbius-specific inverse-tail estimate that transfers cancellation even where the absolute `eta` condition fails. A candidate that supplies only another exact encoding, only conditioning, only analytic nonmasking, or only an exponent-one/logarithmic source asymptotic does not answer the clue.