...
`MC-446` tests the most naive collective version of that remaining family. For a primitive character, the exact full `(u,a)`-box second moment is

`sum_(u,a) |S_H(u,a)|^2 = sum_(k,l<=H) |c_q(k-l)|^2`.

In the squarefree rough range `H <= P^-(q)`, this is `H phi(q)^2 + H(H-1)`. Thus ambient incomplete correlations really do have square-root-scale RMS cancellation, but Cauchy can transfer that cancellation to a weighted source family only if its effective occupancy exceeds approximately `q^2/H` when `phi(q) ~ q`. Generic full-box averaging therefore cannot rescue a sparse CRT/source-frame image.

`MC-447` now classifies the first proposed pieces of arithmetic structure inside that image. In the separated-modulus range `d,e<=D`, `D^2<q`, with `(h,q)=1`, the frame `(u,a)` recovers the exact lcm `L=[d,e]` and the canonical CRT residue `r`. Same-frame ambiguity is bounded by `3^omega((L,h))` and is exactly that size when `L<=D`; it comes only from the three local assignments `d`-only/`e`-only/both at primes common to `L` and `h`. If `(h,P(z))=1`, the divisor-pair-to-frame map is injective. Thus fixing `a=hL^-1` is merely fixing `L`, and collisions/fibres provide no hidden averaging in the coprime-shift separated regime. The surviving question is a genuinely **restricted weighted moment or dispersion estimate on distinct CRT-idempotent frames**.
...
Second, can the well-factorable upper-sieve CRT image beat the `MC-446` occupancy barrier by using structure that the ambient `(u,a)` box discards **after** the `MC-447` fibre classification? In the coprime-shift separated regime, pair collisions and grouping by `a` are no longer candidate mechanisms. The live possibilities are well-factorable cancellation in the outer coefficients, restricted Fourier/dispersion geometry of the distinct CRT-idempotent `u` set, variable-length structure, modulus factorization that reduces the conductor before scalarization, or a quantitatively useful wraparound/nonunit-shift regime outside the hypotheses of `MC-447`. The target remains a restricted second moment or bilinear form whose post-Cauchy diagonal is parametrically smaller than the full-box `H phi(q)^2` tariff after the actual divisor weights are included.

It remains insufficient for a variable merely to survive notation. The desired analogue of Stadlmann must change usable diagonal/off-diagonal information before scalarization, rather than provide a reversible, Abel-equivalent, Fourier-equivalent, residue-permutation-equivalent, complete-period-equivalent, ambient-L2-equivalent, CRT-fibre-equivalent, or RH-equivalent coordinate system.

## Why it may matter

`MC-446` separates a false negative from a real obstruction. The incomplete shifted character family is not devoid of collective cancellation: the full box has an exact favorable second moment. What fails is the transfer of that cancellation to a sparse source family by a generic Cauchy step. `MC-447` removes another possible source of free gain: the source map does not secretly identify many divisor pairs into a favorable smaller frame family when the shift is coprime to the sieve support, and its constrained `a` coordinate is not an independent averaging variable.

If a restricted-frame moment on the distinct CRT-idempotent set beats the ambient occupancy threshold, it would supply precisely the kind of retained pre-quadratic information the current source-depth argument lacks. If every natural restricted moment inherits the same diagonal tariff, the well-factorable/q-vdC branch can be closed much more decisively than by individual Burgess or Pólya--Vinogradov estimates.
...
For the factorable upper-sieve branch, start from the exact CRT map of `MC-413`, the occupancy no-go of `MC-446`, and the fibre classification of `MC-447`. In the clean regime `(h,P(z))=1`, `D^2<q`, fix an lcm `L` and compute a **restricted weighted second moment or dispersion form on its distinct CRT-idempotent `u`-set**, inserting the well-factorable coefficients before Cauchy. Count the diagonal and the first nontrivial off-diagonal exactly or with a theorem whose hypotheses match this set. The branch survives only if the resulting threshold is parametrically smaller than the full-box tariff in a parameter range that remains useful after source-depth bookkeeping.

Reject a continuation if it applies an individual incomplete character-sum bound to each divisor pair, applies Cauchy against the full box and fails `MC-446`, completes first and falls back to `c_q(h)` as in `MC-413`, treats fixed `a` as an independent averaging coordinate despite `MC-447`, or attributes a gain to frame collisions in the injective coprime-shift separated regime.
...
`MC-446` is an exact finite harmonic-analysis result, not an improved character-sum or Mertens estimate. It proves that full ambient averaging has an effective-occupancy requirement of order `q^2/H` in the squarefree rough regime. `MC-447` is exact finite CRT algebra: it proves that, when `(h,q)=1` and `D^2<q`, `a` recovers the lcm and all same-frame multiplicity comes from primes common to `L` and `h`; for `(h,P(z))=1` the frame map is injective. Neither result proves that the distinct arithmetic frame set fails a sharper restricted estimate.

Variable interval lengths, weighted cancellation across the distinct frames, restricted Fourier geometry, modulus factorization, and the wraparound/nonunit-shift regimes outside `MC-447` remain unexploited. Stadlmann's theorem concerns a different Type I averaged-prime setting and does not transfer automatically. Heath-Brown/Irving q-vdC theory and sliding-sum/trace-function methods show that incomplete oscillatory families can be exploitable, but they do not by themselves establish the required weighted restricted-frame estimate. The scalar Möbius--PNT-error branch remains classified by `MC-408` as RH-equivalent at the target exponent.

## Research disposition

The clue remains `accepted` and is **materially narrowed by `MC-446` and `MC-447`**. Generic collective averaging over all translation/start pairs is blocked by a quadratic effective-occupancy threshold; in the coprime-shift separated regime, hidden frame collisions and grouping by the constrained shift are also closed as free mechanisms. The live well-factorable question is now specifically whether the distinct CRT-idempotent frame set, with its actual well-factorable weights, has a restricted moment or dispersion structure whose diagonal/off-diagonal cost is parametrically below the ambient threshold. The independent truncated-lcm/source-specific branch remains open under its previous stronger-than-generic requirement.