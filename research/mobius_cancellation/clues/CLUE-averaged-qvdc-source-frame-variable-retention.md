...
`MC-402` resolves the generic representation question at an incomplete endpoint. For `X<B^2`, `A_I(X)=sum_(n<=X) M(floor(X/n)) E_I(n;B)`, where each `E_I(n;B)` has the same signed GCD/meet factorization as the complete endpoint, restricted to divisors of `n`. This exact Möbius-inversion identity does **not** make `A_I(X)` Mertens-equivalent and does not rule out a direct truncated-lcm theorem, but it shows that the cutoff mask itself is not a free retained variable.
...
`MC-412` closes the obvious progression-theorem transplant. Rewriting the divisor coefficient as a progression gives only `n=0 (mod d)`; for a fixed source character this collapses exactly under `n=dm` to `chi(d) M_chi(N/d)`. Resolving the source character into classes modulo its conductor merely makes `d` permute those fixed source residues. By contrast, the BFI/Maynard/Pascadi/Yang well-factorable distribution machinery places the factorable variable in the modulus of a **nonzero reduced congruence**; the recent Yang convolution proof then produces a reciprocal/Kloosterman phase carrying that affine datum. The upper-sieve coefficient is therefore genuinely well-factorable but currently occupies the wrong arithmetic slot.

`MC-413` tests the first obvious way to create the missing affine datum rather than merely postulating it: additive van der Corput differencing of the actual weighted character sequence. This does generate a genuine translated phase `chi(n+h) overline(chi(n))` and couples the divisor pair through `(d,e)|h` and `L=[d,e]`. But the gain survives only on **incomplete source-conductor blocks**. For every compatible pair with `(de,q)=1`, a full block of `q` progression points has exact correlation `c_q(h)`, independent of `d`, `e`, `L`, the CRT residue, and the primitive character. The divisor-dependent source phase therefore disappears under complete-period averaging. Any Stadlmann-style retained-variable gain must act collectively on the incomplete translated sums before completion or pairwise absolute values.
...
The useful target is therefore no longer “truncation prevents factorization.” It is a source-specific theorem such as a nontrivial estimate for `A_I(X)=sum_(ell<=X) C_g(ell) chi_I(ell)` or for the corresponding joint form `sum_(ell m<=N) C_g(ell) chi_I(ell) chi_I(m)` whose proof exploits actual `g`/signature structure and, after complete bookkeeping, avoids both the sequential `2^{-Omega(A)}` source-depth tariff and a fixed positive common-radical cost `P^theta`.

Second, can the **incomplete translated correlations produced by additive differencing of the well-factorable upper-sieve frame** be controlled collectively before their divisor/source geometry is erased? `MC-409` supplies the positive factorable frame; `MC-410`--`MC-412` rule out the bare product, ordinary completion, and zero-residue progression interpretations; `MC-413` shows that a real translation appears after differencing but that every complete conductor block collapses to the universal Ramanujan scalar `c_q(h)`. The remaining object is therefore the family of sub-conductor translated sums indexed jointly by the divisor pair/CRT data, schematically `sum_(k in I_de) chi(k+u_de+h[lcm(d,e)]^-1) overline(chi(k+u_de))` with `|I_de|<q` and `(d,e)|h`.

A viable continuation must exploit that family **jointly**, using factorability, dispersion, modulus factorization, reciprocal structure, or another exact relation before Cauchy/triangle inequalities destroy the dependence on `(u_de,hL^-1)`. An individual Burgess/Pólya--Vinogradov estimate for each remainder is not enough unless, after summing the divisor pairs and propagating all source costs, it gives a strict improvement over the existing source-depth tariff.

It is still not sufficient that a variable merely avoid lcm factorization. The Mangoldt quotient example shows that a visible source coordinate can scalarize after exact summation into a Mertens self-coupling, while `MC-408` shows that the surviving scalar signed prime-error convolution is itself an RH-equivalent target at square-root scale. The desired analogue of Stadlmann must therefore change usable diagonal/off-diagonal information **before scalarization**, rather than only provide a reversible, Abel-equivalent, Fourier-equivalent, residue-permutation-equivalent, complete-period-equivalent, or RH-equivalent coordinate system.

## Why it may matter

The representation audit now separates several increasingly strong notions that had previously been easy to conflate: a coordinate may be visible, may survive lcm compression, may change a quotient argument, may survive as a coefficient convolution, and may even display an actual translated character phase, yet still fail to provide independent analytic information after exact reindexing, completion, or aggregation.

`MC-413` is useful because it does not merely kill another representation. It locates the remaining information spatially: the complete conductor-period part is source-phase-free after exact Ramanujan collapse, while the incomplete boundary retains the divisor-dependent translation. This gives the next dispersion calculation a sharply bounded target rather than the whole shifted correlation.
...
For the factorable upper-sieve route, start from the actual shifted correlation after one additive differencing step, expand both divisor weights, and keep the incomplete translated family intact. `MC-410` forbids credit for the raw `chi(uvm)` tensor product, `MC-411` forbids credit for an additive completion whose only apparent coupling is `e(h u v m/q)`, `MC-412` forbids credit for rebranding `d|n` as a progression modulus, and `MC-413` forbids credit for the **complete** part of the translated correlation: every full source-conductor block is exactly `c_q(h)` and carries no divisor-dependent character phase.

The branch now survives only if the sub-conductor remainder family is estimated collectively before that collapse. Write the post-differencing kernel with its exact CRT starting point and shift, identify which factor of the conductor is reduced by an `A`-process or which reciprocal relation appears under dispersion, and count the post-Cauchy diagonal. Then propagate the estimate through the well-factorable coefficient sum and the source-frame argument. Reject the candidate if it merely applies an individual short-character-sum bound to each divisor pair and recovers the existing conductor/source-depth cost after summation.

For any other pre-quadratic alternative, exhibit the kernel before estimating it. It must not factor through the lcm as in `MC-398`; after source expansion, its retained variable must remain nonseparable through the relevant first correlation rather than cancelling as in `MC-399`; and after exact aggregation it must not reduce to a scalar Mertens self-coupling, an Abel/Fourier-equivalent coordinate system, a complete-period Ramanujan scalar, or the RH-equivalent scalar criterion classified by `MC-405`--`MC-408`. Then count the post-Cauchy diagonal exactly and compare the mechanism with Stadlmann's retained-variable gain.

## Evidence boundary

No theorem using the actual Selberg lcm coefficient has yet changed the source-cost scaling, no collective source-signature estimate has escaped the global-modulus horizon, and no factorable-upper-sieve estimate has yet beaten the existing source-character bound. `MC-409` establishes only that positivity, shell-prime normalization, and the correct diagonal scale survive a switch to a well-factorable upper linear-sieve representation. `MC-410`--`MC-412` are negative structural classifications of the bare multiplicative/factorable representation and its obvious completions/progression rewrite.

`MC-413` adds an exact boundary rather than an estimate: additive differencing does create a nonseparable translated character correlation, but its complete-period component is the classical Ramanujan sum `c_q(h)`. It supplies no bound for the incomplete divisor-coupled family, no improved character-sum exponent, and no improved bound for `M(x)`. Heath-Brown/Irving q-van der Corput theory establishes that incomplete shifted correlations can be exploitable when the modulus factorization is arranged correctly; it does not automatically provide the collective weighted estimate required here.
...
Stadlmann's theorem concerns averaged prime distribution to smooth moduli in a different Type I setting and does not imply an endpoint source-cost theorem or improved Möbius cancellation. Likewise, the well-factorable distribution theorems of Bombieri--Friedlander--Iwaniec, Maynard, Pascadi, and Yang concern a different nonzero-residue modulus/progression geometry and do not by themselves estimate the incomplete `MC-413` divisor-coupled character family.
...
For the well-factorable frame, the next gate is now **the incomplete translated-correlation family**. A continuation must keep the CRT starting point and shift dependence alive across divisor pairs, apply q-van der Corput/dispersion or another exact collective mechanism before complete-period or absolute-value collapse, and show a strict source-cost gain after the full coefficient and source-frame bookkeeping. Failure of that collective incomplete-sector estimate would close a substantially more precise class of positive pre-quadratic sieve representations.

The scalar Möbius--PNT-error branch remains classified by `MC-408`: at the square-root target it is RH-equivalent, so it is not an easier retained-variable intermediate. Any further alternative must remain analytically independent before exact aggregation rather than collapsing to the Möbius--Mangoldt self-coupling, an Abel/Fourier-equivalent form, a residue-permutation-equivalent form, a complete-period Ramanujan scalar, or an RH-equivalent scalar target.