...
The unresolved question is therefore no longer whether a support-supercritical complement can be made recursively cheap, whether adding more common-prime/gcd layers makes the retained carrier termwise lower-scale, whether independent exclusive-prime recursion contracts, whether prime labels provide a free square-function norm, whether fixed-gap interpolation spends the gain, whether continuum control is needed, or whether the already available full Walsh variance can simply be pushed through Hamming interpolation. Those routes are settled, quotiented, or calibrated by `MC-089`--`MC-095`. The live residual is a **genuinely joint deterministic signed estimate after the correct quotient**: prove cancellation inside the `omega(a)` shells strongly enough to bound their weighted square function, find an intermediate source-forced quotient that simultaneously retains useful orthogonality and has `N^{o(1)}` reconstruction cost, or identify another source coupling outside these mask/radialization classes. Any survivor must preserve a strict power margin, avoid reconstructing the improved Mertens target as an input, and close the remaining coarse/small-prime plus `MC-027` iteration ledger.