...
The unresolved question is therefore no longer whether a support-supercritical complement can be made recursively cheap, whether adding more common-prime/gcd layers makes the retained carrier termwise lower-scale, whether independent exclusive-prime recursion contracts, whether the p labels themselves provide a free square-function norm, or whether fixed-gap polynomial interpolation necessarily spends the gain. Those routes are settled, quotiented, or calibrated by `MC-089`--`MC-093`. The live residual is a **genuinely joint deterministic signed estimate**: control the normalized degree-weighted product-fiber parity correlation from `MC-092`, obtain a source-natural uniform interior estimate for the deformation, find cancellation among surviving source terms before triangle inequalities, or identify another source-forced coupling outside both mask classes. Any survivor must preserve a strict power margin, avoid reconstructing the improved Mertens target as an input, and close the remaining coarse/small-prime plus `MC-027` iteration ledger.