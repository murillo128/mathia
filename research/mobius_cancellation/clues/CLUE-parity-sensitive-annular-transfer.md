...
based_on:
  ...
  - research/mobius_cancellation/findings/MC-091-prime-symmetric-difference-rectangular-variation.md
  - research/mobius_cancellation/findings/MC-092-exclusive-prime-variation-product-fiber-collapse.md
  - research/mobius_cancellation/findings/MC-093-fixed-gap-hamming-deformation-endpoint-power.md
  - research/mobius_cancellation/findings/MC-094-chebyshev-bias-node-endpoint-transfer.md
  - research/mobius_cancellation/findings/MC-095-noise-radialization-orthogonality-tradeoff.md
  - research/mobius_cancellation/findings/MC-110-diagonal-shell-norm-central-energy-floor.md
...
`MC-091` supplies a concrete non-gcd continuation rather than another mask. Weighting square-free pairs by a parameter according to the number of primes dividing exactly one coordinate interpolates from a diagonal form to the Möbius Huxley--Watt block. Its derivative at the Möbius endpoint is exactly `-2 sum_{p<=N} Q_p(N/p,N)` with p-sifted **rectangular** source blocks. Under an old exponent `beta`, termwise bounds give the power-expansive ell-1 ledger `N^(1+beta)`, so independent prime recursion is dead. But the large-prime block scales have square-sum tail `N^(2 beta) P^(1/2-beta)`; for `P=N^delta` this is a strict power gain whenever `beta>1/2`. No orthogonality theorem realizing that benchmark is known here, and endpoint differentiation alone does not reconstruct the target.

`MC-092` identifies the exact quotient hidden by that prime-indexed representation. For any threshold `P`, the entire large-prime sum is exactly an `omega_{>P}(a)`-weighted product-fiber parity correlation: all exclusive primes belonging to the same square-free kernel `a` contribute with the same Möbius sign and sawtooth phase. Prime labels therefore do not supply independent block signs. A matched random multiplicative calculation nevertheless puts the whole weighted variation at `O_delta(N log^2 N)` RMS for `P=N^delta`, so the route is not generically power-impossible; the missing theorem is specifically deterministic control of the all-minus parity point against those degree-weighted Huxley--Watt weights.

`MC-093` removes a separate interpolation ambiguity. The full Hamming deformation has degree only `O(log N/log log N)` because its product-fiber powers are `omega(a)` with `a<=N^2`. Classical Chebyshev/Remez extrapolation therefore transfers a uniform bound on any fixed interval `0<=t<=1-eta` to the Möbius endpoint `t=1` with only an `N^{o_eta(1)}` loss. Thus the deformation cannot hide a different polynomial power only at the endpoint: a uniform fixed-gap interior power gain is already an endpoint power gain up to subpolynomial cost. A single endpoint derivative remains insufficient, but interpolation itself is no longer a separate power barrier once genuinely uniform interior arithmetic control exists.

`MC-094` lowers the reconstruction burden further. Because the source degree is only `K_N=O(log N/log log N)`, values at `K_N+1` deterministic Chebyshev bias nodes already determine the deformation, and even a fixed-`ell_p` mean bound across those nodes transfers to the Möbius endpoint with only `N^{o(1)}` loss. Continuum-uniform control is therefore unnecessary; a sublogarithmic bias family is enough.

`MC-095` closes a tempting shortcut opened by that observation. The whole deformation is exactly the Boolean noise operator applied to the product-fiber Walsh polynomial and evaluated at the all-minus vertex, but every value of the one-parameter deformation depends on the fiber vector only through the degree-shell sums `C_k=sum_{omega(a)=k} W_N(a)`. Full Walsh Parseval gives the matched critical-power `L^2` scale before this quotient, whereas generic point evaluation from that `L^2` input alone has an ambient norm `N^{1+o(1)}` even at fixed interior bias. A fixed-radius discrete Fourier family does give an exact `L^2` norm of the degree-shell sums and transfers it to the endpoint with subpolynomial cost, but it no longer contains orthogonality between different kernels in the same shell. Exact full-fiber Parseval cannot be reproduced for all coefficient vectors by only the shell-sized sample family by a rank argument.

`MC-110` closes the remaining **absolute shell-square-function** version of that shortcut at the actual source level. The critical Sathe--Selberg window contains `Theta(sqrt(log log N))` positive shells of size `N^2/sqrt(log log N)`, and for arbitrary positive diagonal shell weights every Hölder primal/dual certificate is bounded below by any one retained shell because the inverse endpoint weight cancels the attempted preconditioning. The ordinary radial `L^2` energy is consequently at least `N^4/sqrt(log log N)` up to its explicit constant. Thus the previously proposed option of proving enough cancellation *inside the already-collapsed `omega(a)` shells* to make a positive weighted square function critical-scale is not live: the coherent shell sums themselves are provably almost-square. What remains possible is cancellation **between** degrees before absoluteization, or a genuinely finer non-radial/source-coupled quotient.

The unresolved question is therefore no longer whether a support-supercritical complement can be made recursively cheap, whether adding more common-prime/gcd layers makes the retained carrier termwise lower-scale, whether independent exclusive-prime recursion contracts, whether prime labels provide a free square-function norm, whether fixed-gap interpolation spends the gain, whether continuum control is needed, whether the already available full Walsh variance can simply be pushed through Hamming interpolation, or whether a positive diagonal shell norm can recover the endpoint. Those routes are settled, quotiented, or calibrated by `MC-089`--`MC-095` and `MC-110`. The live residual is a **genuinely joint deterministic signed estimate after the correct quotient**: find a non-diagonal signed cross-degree transform or recurrence that keeps cancellation before taking absolute values, find an intermediate source-forced quotient that simultaneously retains useful orthogonality and has `N^{o(1)}` reconstruction cost, or identify another source coupling outside these mask/radialization classes. Any survivor must preserve a strict power margin, avoid reconstructing the improved Mertens target as an input, and close the remaining coarse/small-prime plus `MC-027` iteration ledger.