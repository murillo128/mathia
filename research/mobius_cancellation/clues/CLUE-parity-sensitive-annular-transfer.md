...
The unresolved question is therefore no longer whether a support-supercritical complement can be made recursively cheap, whether adding more common-prime/gcd layers makes the retained carrier termwise lower-scale, whether independent exclusive-prime recursion contracts, or whether the p labels themselves provide a free square-function norm. Those routes are settled or quotiented by `MC-089`--`MC-092`. The live residual is a **genuinely joint deterministic signed estimate**: control the normalized degree-weighted product-fiber parity correlation from `MC-092`, cancellation among surviving source terms before triangle inequalities, or another source-forced coupling outside both mask classes. Any survivor must preserve a strict power margin, avoid reconstructing the improved Mertens target as an input, and close the full small-prime plus interpolation/`MC-027` iteration ledger.