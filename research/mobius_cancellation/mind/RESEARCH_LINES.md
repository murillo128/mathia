...
The live arithmetic question is now to estimate or structurally constrain the **physical anti-diagonal pushforward while keeping the distinct localized root states in the same source average**. A viable theorem cannot rely on broad marginals, finite local congruence rigidity, or many source labels sharing one exact growing profile. It must relate the structured evolution of the localized CRT state, moving cutoff and archimedean row geometry to displacement or cancellation in the observed coordinate `x+j`, or introduce a genuinely signed/complex pre-aggregation relation before the nonnegative table is formed. If the excess collision statistic is `o(1)`, MC-506 closes the current quadratic hard-mask bias. If it has fixed excess, collisions remain only necessary and one must test signed alignment with the centered quadratic phase.

A genuinely `q`-dependent signed or complex coefficient introduced **before** the nonnegative aggregation remains a distinct escape and requires its own normed-measure analysis; the probability/collision formulation above must not be applied to it without that extra work. The accepted localized-full-profile clue already records this surviving branch, but MC-509 removes exact-profile recurrence itself from its possible mechanism.
