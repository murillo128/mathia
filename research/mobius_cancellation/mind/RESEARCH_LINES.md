...
## Work inside the forced half-loaded source band with genuinely high-degree or arithmetic structure
...
MC-330 closes the generic signed-postprocessing loophole left after MC-329. Once the character family has been compressed to the `ell^2` bias-energy estimate, every signed linear statistic satisfies Cauchy--Schwarz and every quadratic kernel satisfies the operator-norm bound `|beta^*K beta|<=||K||_op ||beta||_2^2`. At the reciprocal endpoint the normalized optimum in either class is exactly `||x||_2^2`, already attained by the original energy (and by the endpoint-matched rank-one projector). Signs, off-diagonal couplings, low rank and endpoint-matched linear/quadratic kernels applied **after** the energy estimate therefore cannot move the quartic horizon.

The matching exponent is thus a method boundary seen from two independent positive compressions, and MC-330 identifies where a genuinely signed escape would have to enter. A fixed-power gain beyond four must improve the prime-character theorem in genuinely new exponent geometry, prove a source-natural structured estimate for a transform `BT` **before** generic `ell^2` compression while retaining a large endpoint response, use a higher-order/nonlinear invariant whose control does not factor through the same bias energy, or change the endpoint/source representation. Inside the incidence architecture, the remaining arithmetic discriminator still lives in the forced central Hamming band and requires Fourier degree comparable to that band.
...
## Treat observation category, conductor cost, source loading, Fourier degree and analytic family horizon as separate currencies

A theorem about many characters helps only when it controls the exact population and scale demanded by the endpoint. MC-326 makes universal source loading minimax-sharp; MC-327 adds equality-case rigidity for actual economical packages; MC-328 shows that low Fourier degree in the endpoint coefficient cannot resolve the resulting central source band. MC-329 adds an independent ceiling: even the full endpoint character family at one common modulus is controlled only up to the quartic fixed-power horizon by the present bias-energy input, and optimizing its Burgess order does not move that horizon beyond four. MC-330 adds a placement gate: arbitrary signed linear or quadratic processing of the already-compressed bias vector has no operator-normalized advantage over total energy.

Future proposals must expose the analytic bill per mode, the logarithmic occupancy distribution of the source columns actually realized, the Walsh/Fourier complexity of any proposed weighting, the common-modulus range and exponent geometry of the character theorem being invoked, **where any signed/joint structure is used relative to the energy compression**, and any arithmetic structure remaining inside the forced half-loaded band before interpreting family size, order growth, mode design or postprocessing as leverage.
