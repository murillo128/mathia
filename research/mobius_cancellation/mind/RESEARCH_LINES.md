...
MC-508 then rules out fixed finite local mask data as the missing coupling. At fixed conductor phase, any prescribed finite set of lower-prime roots `-h q^(-1) mod ell` can be realized by infinitely many primes once the realizing first-exit prime may escape the active source scale. Finite congruence coordinates therefore do not force source/profile rigidity; any useful dependence must use localization, a growing mask, the moving cutoff/order, archimedean row geometry or another source resource absent from that CRT control.

MC-509 shows that simply retaining more exact local roots does not create free repeated-profile averaging either. If a localized source interval has diameter `H`, a fixed conductor residue contributes modulus `p`, and roots are retained for lower primes up to `z`, equality of root profiles forces equality modulo `pM_z`, where `M_z` is the corresponding primorial product. Every exact-profile fibre has size at most `1+floor(H/(pM_z))`; once `pM_z>H`, the truncated root profile is injective on the localized source. Because `log M_z~z`, only logarithmic mask depth is needed in a dyadic block. Localization-induced rigidity is therefore a **source-addressing effect**, not automatically a cancellation or averaging resource.

MC-510 shows that quotienting the one-row affine gauge does not remove this addressing barrier; it only raises its reconstruction tariff. For an oriented pair of localized source primes `(q,r)`, the gauge-invariant lower-prime state is `lambda_ell(q,r)=q r^(-1) mod ell`. Equality of the retained relative tuples implies

`p M_z | q r' - q' r`.

On an interval of diameter `H` inside `[Q,Q+H]`, the determinant obeys `|q r'-q'r|<=2H(Q+H)`. Hence `pM_z>2H(Q+H)` makes the relative tuple injective on ordered source pairs; in a dyadic block the sufficient modulus is `pM_z>4Q^2`. Thus even after the physically necessary gauge quotient, an exact growing relative mask eventually becomes a rational-reconstruction address for the source pair rather than a recurrent coarse class.

The live arithmetic question is now to estimate or structurally constrain the **physical anti-diagonal pushforward while keeping distinct localized absolute and relative mask states in the same source average**. A viable theorem cannot rely on broad marginals, finite local congruence rigidity, many source labels sharing one exact growing profile, or gauge quotienting alone. It must exploit the quantitative map from many distinct pair states to the observed coordinate `x+j`, use a deliberately coarser state below the integer/rational reconstruction thresholds, exploit endpoint/cutoff geometry before exact conditioning, or introduce a genuinely signed/complex pre-aggregation relation. If the excess collision statistic is `o(1)`, MC-506 closes the current quadratic hard-mask bias. If it has fixed excess, collisions remain only necessary and one must test signed alignment with the centered quadratic phase.
