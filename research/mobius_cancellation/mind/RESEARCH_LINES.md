...
## Force the remaining near-negative spectrum into an arithmetically impossible high-cost quotient
...
MC-294 closes the simplest collective reuse escape. Let `W<=H*` be uniformly Page-cheap, so every nonzero `w in W` has `Q(w)<=D_y`. For independent near-negative directions with defect at most `tau`, every quotient relation of length at most `t` in `H*/W` creates a strongly biased cheap character whenever `2t tau+epsilon_y<1`. Page theory allows at most the single exceptional relation. After deleting at most one direction, the quotient vectors therefore have minimum dependence length greater than `t`, and Hamming sphere packing forces

`2^(r-dim W) >= sum_(j=0)^(floor(t/2)) binom(R-1,j)`.

So high individual source cost is no longer enough: a large near-negative family must occupy genuine **high-source-cost quotient dimension** rather than reusing a few expensive directions modulo cheap corrections.

The unresolved escape is now one of four concrete regimes: effective occupancy is extremely small; the shell encoding is nearly injective; one classical Landau--Page mode is genuinely load-bearing; or the shell possesses enough high-source-cost quotient dimension/code structure to host the required near-negative family. The next theorem must rule out or exploit the singleton exceptional mode, bound the available high-cost quotient dimension, link that dimension to multiplicity/participation, or prove stronger arithmetic restrictions than the generic Hamming bound.

## Keep parity, occupancy, entropy complexity, source cost and quotient dimension as different currencies

Even weights are killed by equal-cell pairing. Odd weights require an odd nucleus plus pair capacity. The affine branch has a character with bias exactly `-1`; MC-289--MC-292 constrain non-affine escape using odd moments, common-cell geometry and entropy. MC-293 constrains the minimum lower-prime product cost of individual generated shell functionals. MC-294 adds yet another resource: dimension and short-dependency structure **after quotienting by an entire uniformly cheap subspace**.

Raw support density and `eta` are not interchangeable: multiplicity concentration can make `eta` much smaller than `K/2^r`. Exceptional cardinality, independent rank, defect profile, entropy cost `1-h_2(d_lambda)`, source cost `Q(lambda)`, primitive conductor, family conductor, cheap-subspace dimension and high-cost quotient girth are likewise distinct. MC-294 prevents many independent biased modes from hiding behind cheap corrections, but it does not upper-bound the expensive quotient itself. A closing theorem must state which arithmetic currency it controls and how that control reaches the blind endpoint.
...
MC-294 combines that same Page input with a generic binary coding argument: short quotient dependencies become codewords and the Hamming sphere-packing bound charges them to quotient dimension. This is a necessary tax, not an arithmetic impossibility theorem. Further progress must exploit properties of the actual Legendre shell not already contained in generic coding geometry: an upper bound or structural restriction on the high-cost quotient, a link from source cost to shell multiplicity/occupancy, or a proof that the unique exceptional mode cannot support the endpoint mechanism.