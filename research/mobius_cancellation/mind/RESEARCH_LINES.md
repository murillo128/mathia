...
The endpoint audit is therefore two-layered. For the endpoint-only branch, first apply the MC-500 Q-free source-location gate and the MC-501 Q-free source-width gate; only scales passing both reach the MC-499 block-population gate and then the exact MC-498 participation/fibre computation. For the incomplete hard-mask branch, MC-502 removes complete-period conductor-power coherence, while MC-503--MC-504 require any remaining quadratic bias to live in source mass carried by profile classes with `R_start|profile R_profile=O(p)`, or in profile variation that cannot be compressed with a quantitatively small error.

The live arithmetic question is now to classify the **joint source geometry of exact lower-prime profiles and conductor starts**. If material first-exit mass can be grouped into recurrent or controlled approximate profiles with large profile support and large conditional start participation, MC-504 closes the quadratic hard-mask alignment there. If instead the source keeps the product participation small, the burden is to derive the exact arithmetic law responsible for low support, exceptional profile--start alignment, or noncompressible profile diversity. A genuinely `q`-dependent signed or complex coefficient introduced before the nonnegative aggregation of MC-495 remains a separate escape. None of these mechanisms may be inferred from raw label multiplicity or mask irregularity alone.
