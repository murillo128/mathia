...
Continue the exact endpoint partition of `MC-296` and the unequal-defect setup of `MC-368`–`MC-371`. Let `lambda_1,...,lambda_R` be linearly independent shell functionals with endpoint-cell weights

$$
d_i > 0, \qquad \sum_i d_i = 1,
$$

and write

$$
lambda_I = \sum_{i\in I} lambda_i, \qquad d(I)=\sum_{i\in I} d_i.
$$

The exact partition identity is

$$
x_{lambda_I}=(-1)^{|I|}(1-2d(I)). \tag{1}
$$
...
epsilon_i=d_i-1/R,
\qquad
Delta_2=\sum_i epsilon_i^2=\left\|d-R^{-1}{\bf 1}\right\|_2^2. \tag{2}
$$

For every Hamming layer `0 <= k <= R`, let `C(R,k)` denote the binomial coefficient and define

$$
A_k=C(R,k)^{-1}\sum_{|I|=k}|x_{lambda_I}|^2. \tag{3}
$$
...
$$
C_R=\{I\subseteq[R]:|I|=R/2\},
\qquad
m_R=|C_R|=C(R,R/2). \tag{6}
$$

For each nonempty `I`, let `Q(lambda_I)` be the minimum primitive lower-prime source conductor from the source-character dictionary of `MC-301`–`MC-323`, and put

$$
theta_{delta_0}
=
\frac{1}{m_R}
\#\{I\in C_R:Q(lambda_I)>y^{2-delta_0}\}. \tag{7}
$$
...
a_p=\#\{i:p\in V_i\},
\qquad
w_p=\frac{\log p}{\log P},
\qquad
B_R=\sum_{p\in U}w_p\left(\frac{2a_p-R}{R}\right)^2. \tag{11}
$$
...
\qquad
m_I=\prod_{p\in V_I}p. \tag{12}
$$

Then the average logarithmic package cost satisfies

$$
\boxed{
\left|
\frac{1}{m_R\log P}\sum_{I\in C_R}\log m_I
-\frac12
\right|
\le
\frac{1}{2(R-1)}+\frac{B_R}{2}.
} \tag{13}
$$
...
$$
\boxed{
\frac1{m_R}\sum_{I\in C_R}\log m_I
=(2+o(1))\log y.
} \tag{15}
$$
...
$$
E_I=\sum_{i\in I}epsilon_i.
$$

Because `sum_i epsilon_i=0`, one has `E[E_I]=0`. The inclusion probabilities are

$$
P(i\in I)=k/R,
\qquad
P(i,j\in I)=k(k-1)/(R(R-1))
$$

for distinct `i,j`. Also

$$
\sum_{i<j}epsilon_i epsilon_j=-Delta_2/2.
$$
...
\frac{k}{R}Delta_2
-
\frac{k(k-1)}{R(R-1)}Delta_2
=
\frac{k(R-k)}{R(R-1)}Delta_2. \tag{17}
$$
...
$$
|x_{lambda_I}|^2=(1-2k/R-2E_I)^2.
$$

Averaging and using `E[E_I]=0` plus `(17)` proves `(4)`. At `k=R/2`, the deterministic first term vanishes and `(5)` follows.

The mechanism is the standard finite-population correction for sampling without replacement, specialized to the endpoint weights. A targeted prior-art check found no reason to treat `(17)` as new probability theory. The durable delta is its composition with the exact endpoint Fourier profile and the source-conductor frontier, not the variance formula itself.

## 2. Prime Halász--Montgomery turns middle-layer energy into a conductor dichotomy

Split the middle layer as `C_R=L union H`, where

$$
L=\{I:Q(lambda_I)\le y^{2-delta_0}\},
\qquad
H=C_R\setminus L. \tag{18}
$$
...
Equation `(5)` of `MC-323` therefore applies to the low-conductor family `L`:

$$
\sum_{I\in L}|x_{lambda_I}|^2
\le
C_{delta_0}\left(1+|L|y^{-c_{delta_0}}\log y\right). \tag{19}
$$

For the high-conductor complement one only needs

$$
\sum_{I\in H}|x_{lambda_I}|^2\le|H|. \tag{20}
$$

Adding `(19)` and `(20)`, using `|L|<=m_R`, dividing by `m_R`, and inserting `(5)` gives `(8)`. Rearrangement gives `(9)`.

The central binomial coefficient satisfies

$$
m_R=2^R\sqrt{2/(pi R)}(1+o(1)). \tag{21}
$$

Hence `m_R^(-1)=o(1/(R^2 log R))` whenever `R->infinity`. For each fixed `delta_0`, the term `y^(-c_delta0) log y` is also `o(1/(R^2 log R))` when `R=y^(o(1))`. This proves `(10)` under the stated high-conductor-fraction hypothesis.
...
A middle-layer package representative contains `p` exactly when `|I intersection S_p|` is odd. Thus the fraction of middle-layer sets containing `p` is

$$
pi_R(a_p)=\frac12(1-kappa_{R/2}(a_p;R)), \tag{22}
$$

where

$$
kappa_{R/2}(a;R)=K_{R/2}(a;R)/C(R,R/2)
$$
...
for odd `a`, and for `a=2j`,

$$
kappa_{R/2}(2j;R)
=(-1)^j C(R/2,j)/C(R,2j). \tag{23}
$$

For `1<=a<=R-1`,

$$
\boxed{|kappa_{R/2}(a;R)|\le1/(R-1).} \tag{24}
$$

Indeed the even ratio in `(23)` equals `1/(R-1)` at `j=1` and `j=R/2-1`; the ratio of consecutive magnitudes is `(2j+1)/(R-2j-1)`, so the extrema on the interior occur at those endpoints. Odd `a` vanish identically.

The only nonzero source column not covered by `(24)` is the universal column `a=R`. Its logarithmic source-mass proportion is at most `B_R`, because such a column contributes exactly `w_p` to `(11)`. Therefore, averaging `(22)` with weights `w_p` gives

$$
\left|
\sum_{p\in U}w_p pi_R(a_p)-\frac12
\right|
\le
\frac1{2(R-1)}+\frac{B_R}{2}. \tag{25}
$$
...
\frac1{m_R}\sum_{I\in C_R}\log m_I
=
\log P\sum_{p\in U}w_p pi_R(a_p). \tag{26}
$$

Equations `(25)`–`(26)` prove `(13)`, and `(14)` then gives `(15)`.
...
An audit can falsify the finding at three concrete points: failure of the finite-population identity `(4)`, failure of the source-character dictionary needed for `(19)`, or a counterexample to the interior central-Krawtchouk bound `(24)`. Each is an exact finite statement apart from the already-canonical analytic estimate inherited from `MC-323`.
