...
The clue is accepted as a line-local investigation because it attacks the exact missing quantity left by PF-296: the `q`-weighted coefficient budget of the complete normalized PF-290 response. The first gate is representation, not estimation: derive the source-row return equation from the physical `P/H`-split DtN/Schur problem and verify that it reconstructs the actual Riesz representative. Only then test a source-adapted Green potential or the equivalent bounded superharmonic-majorant certificate above.
