...
What was not established at proposal time was the Darboux/contour or Green theorem converting that nonzero threshold functional into a sharp large-mode asymptotic for `g_w(L)z_i`, any corresponding obstruction or positive estimate for `B_w`, uniformity in the canonical corridor parameters, preservation under actual `K_a` transport, or the final uniform far-leg elliptic estimate. PF-276 by itself did not claim that a nonzero branch-point coefficient functional automatically determines continuous-Hahn coefficient asymptotics.
...
Resolved by:
- [[research/prime_flute/findings/PF-278-mass-one-branch-forces-critical-normalized-fixed-row-leakage|PF-278]]

PF-278 establishes the missing branch-to-coefficient transfer. The genuine mass-one branch exposed in PF-277 produces a nonzero normalized fixed-row coefficient tail of order `n^-3/2 (log n)^-3/2`, hence a dyadic fixed-row shell norm of order `N^-1 (log N)^-3/2`. The Robin normalization therefore retains a logarithmic gain but cannot produce any fixed supercritical exponent `q>1` required by PF-271, even inside a fixed low-output row.

This negatively resolves the fixed-axis leakage question for this route. Because the obstruction occurs before variable-corridor transport or canonical-tail uniformity, PF-255/PF-256 cannot rescue the same PF-271 operator-norm gate. Any continuation of PF-243 must change the proof currency or factorization rather than sharpen this normalized-source estimate.