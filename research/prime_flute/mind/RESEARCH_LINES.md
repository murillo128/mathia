...
PF-333 rules out a generic operator-norm shortcut; PF-334 makes the relative square-root map dimension-free in Hilbert--Schmidt norm. PF-335 transfers normalized-insertion translation commutators through the square-root step. PF-336 then removes the remaining normalized-insertion envelope from the **completed polar unitary** itself: for physical translations commuting with `Q`,

`||[U_n,tau_t]||_(S2) <= 1/sqrt(2) ||[K_n,tau_t]||_(S2)`,

with `K_n=Q_(L,n)^(-1/2) Lambda_(LL,n) Q_(L,n)^(-1/2)`. No retained-rank, seam-conditioning or `sup ||K_n||` factor is required for this polar transport.

The direct translation theorem is therefore narrower: prove the `o(1)` reciprocal-prime Hilbert--Schmidt translation commutator for the **normalized pant insertion** on the PF-328 critical interval. A separate concentration theorem for one reference profile, or a stronger fixed-window cutoff/off-diagonal estimate, is still needed because spatial cutoffs generally do not commute with `Q`.

Do not reintroduce a generic `S_infinity` perturbation theorem or a uniform operator envelope merely to control the polar rotation; PF-336 shows those are unnecessary on the translation route. A global `S_2` commutator is still not physical concentration.

## Close the global heavy-range ideal counts after the localized S2 decision

For the PF-287 heavy low/high extension-range projection product, determine whether the conditional channel `T_H` and the shear-deleted reference-angle family satisfy the required ideal counts once the completed local block behavior is known. PF-318 supplies endpoint counting; PF-320 supplies the shear-error budget; PF-325 supplies the position-space frame; PF-330 gives exact packet-energy conservation; PF-331 controls polar extraction; PF-332 identifies the insertion source; and PF-336 shows that translation compatibility of the completed polar factor is inherited directly from the normalized insertion.
