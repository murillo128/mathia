...
PF-297 removes one ambiguity from this question. The fixed-axis Robin inverse is `ell^1(q)`-nonexpansive and its complete positive action conserves `q`-mass; finite-section strict contraction comes only from omitted-tail leakage and its gap degenerates to zero with depth. Therefore the needed return estimate cannot come from a uniform Robin contraction. The live gate is to derive the actual `P/H`-split return/reassembly map and prove that the physical forcing is compensated by genuine escape/support loss, a source-adapted excessive potential, a signed cancellation mechanism, or another factor that changes the `q` currency.

The unresolved quantities are the exact coefficient forcing, the return map outside the conservative Robin factors, and any residual physical-high term.
