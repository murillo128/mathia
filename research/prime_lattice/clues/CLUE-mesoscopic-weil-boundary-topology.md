...
Equivalently, if `log log N/L->alpha<1`, survival forces `beta>=1-alpha/2-o(1)`; only when `log log N=o(L)` does this sharpen to `beta->1`. Under RH the same family collapses throughout every band `log N=o(exp L)`, essentially all the way to the unrelated scale at which the normalized archimedean `log N` cost becomes order one. Thus a nonzero natural-scale fixed-depth band limit is not a plausible **RH-compatible** critical-line mechanism. A surviving clue must change amplitude normalization, topology, depth geometry, or arithmetic coupling rather than merely push the same natural-scale Dirichlet cutoff farther.
...
`PL-063`--`PL-065` strengthen that scale barrier from the PNT supremum to direct zero sampling: all bands with `log N (log log N)^2=o(L^(3/2))` collapse, all Sobolev orders with `s_L L^(3/2)/(log L)^2->infinity` collapse, and the concrete coherent Kronecker witnesses are forced beyond `exp(c_R L^(3/2)/(log L)^2)`. `PL-066` then closes the **natural-normalization fixed-depth band** as an RH-compatible nonzero-limit route throughout the larger sub-archimedean regime `log N=o(exp L)`: RH forces every such compression to zero, while any order-one survival certifies a zero satisfying `1-beta<=log(C log N)/(2L)`. At genuinely mesoscopic log-log slope `log log N=o(L)` that means `beta->1`; for `log log N/L->alpha<1` it means `beta>=1-alpha/2-o(1)`. The clue remains accepted only for constructions that change the information geometry — for example a justified second amplitude scale, a different moving form topology, growing-depth/frequency coupling not conjugate to `PL-058`, or an additional rational-prime invariant. Merely enlarging the same natural-scale Dirichlet band is no longer a live RH mechanism.
