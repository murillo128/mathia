...
`PL-058` now closes the most direct amplified spatial-tail repair. Moving the two endpoint layers inward by total depth `D` and multiplying by the compensating factor `exp(D/2)` is exactly unitarily equivalent to the original boundary family with smaller effective half-length `L-D/2`; the identity uses only finite-section translation geometry and survives matched generalized-prime systems. Thus neither raw growing depth nor pure spatial recentering with the natural PNT-scale amplitude compensation creates an intermediate topology.

A useful moving topology would therefore need additional `L`-dependent frequency/regularity structure, a renormalization not reducible to spatial recentering, or coupling to the full Weil terms. Its existence remains a research question, not evidence.

## Research disposition

Accepted for active investigation, but narrowed by `PL-055`--`PL-058`. Every fixed `L`-independent compact Sobolev sandwich suppresses the high-frequency recurrence and upgrades the `PL-051` PNT boundary limit to norm/Schatten convergence. For fixed order `s>1/2`, the trace-class Fredholm determinant converges to the elementary rank-two determinant of the universal PNT mode. `PL-056` closes the static endpoint `s=1/2` as well: the full smoothed family converges in `S_2`, so the canonical `det_2` has the same universal rank-two limit, while the critical full-reflection atom has only the generic `1/k` weak-trace residue `R/pi` (twice that in the two-end absolute channel) and an elementary hyperbolic-sine `det_2` ladder. The apparent smoothing threshold is therefore ordinary one-dimensional ideal geometry, not zeta-critical rigidity.
...
The unresolved question is now specifically whether the localized Weil construction forces a **simultaneously moving frequency/regularity topology or a genuinely non-spatial global renormalization** — for example a boundary-frequency cutoff tending to infinity, a Sobolev order `s(L)` that weakens with `L`, a joint spatial/frequency scaling whose frequency component cannot be conjugated away, or a normalization forced by the archimedean/pole part of the full Weil geometry — for which the PNT layer can be removed while a nontrivial rational-prime-specific residual converges and fails matched Beurling controls. Fixed smoothing, the fixed critical `det_2` endpoint, static weak-trace residues, compact/Schatten counterterms, raw threshold Fredholm flow, raw growing depth, and canonically amplified pure spatial recentering are excluded by `PL-053`--`PL-058`; no surviving moving topology is currently established.