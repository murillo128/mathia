...
Accepted for active investigation, but narrowed by `PL-055` and `PL-056`. Every fixed `L`-independent compact Sobolev sandwich suppresses the high-frequency recurrence and upgrades the `PL-051` PNT boundary limit to norm/Schatten convergence. For fixed order `s>1/2`, the trace-class Fredholm determinant converges to the elementary rank-two determinant of the universal PNT mode. `PL-056` closes the static endpoint `s=1/2` as well: the full smoothed family converges in `S_2`, so the canonical `det_2` has the same universal rank-two limit, while the critical full-reflection atom has only the generic `1/k` weak-trace residue `R/pi` (twice that in the two-end absolute channel) and an elementary hyperbolic-sine `det_2` ladder. The apparent smoothing threshold is therefore ordinary one-dimensional ideal geometry, not zeta-critical rigidity.

The unresolved question is now specifically whether the localized Weil construction forces a **genuinely `L`-dependent mesoscopic decompactification or renormalization** — for example `R(L)->infinity`, a boundary-frequency cutoff tending to infinity, or a Sobolev order `s(L)` that moves with `L` rather than being held at or above the static endpoint — for which the PNT layer can be removed while a nontrivial rational-prime-specific residual converges and fails matched Beurling controls. Fixed smoothing, the fixed critical `det_2` endpoint, static weak-trace residues, compact/Schatten counterterms, and raw threshold Fredholm flow are excluded by `PL-053`--`PL-056`; no such moving scale is currently established.