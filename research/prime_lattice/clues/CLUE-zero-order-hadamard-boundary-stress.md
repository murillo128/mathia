...
`PL-300` now closes the **generic perturbation shortcut** to the remaining BV gate. It constructs a small-order sequence of normalized fractional principal operators and self-adjoint perturbations `B_j` of rank at most two with `||B_j||->0` for which the unique simple ground states still converge uniformly to the pure logarithmic ground state, yet their total variations diverge and the autocorrelation derivative at any preassigned lag `h_0 in (0,2)` can be forced to diverge. The construction exploits the exact scale mismatch between logarithmic graph cost `O(log K)` and variation/shift-derivative cost `O(K)`. Therefore norm-resolvent convergence, a persistent spectral gap, Kato tracking and even uniform `C_0` convergence cannot by themselves supply the compactness demanded here. A successful theorem must use a structural property of the **actual** prime-shift/completion remainder that excludes this high-frequency contamination.

`PL-301`--`PL-305` sharpen that source-structured requirement. The actual Suzuki remainder is BV-stable, but the prime-shift comb is not a BV contraction at the working aperture; the odd spectral gap and generic `L^p`/measure resolvent routes do not repair this. On the positive side, `PL-305` proves that once an actual state lies in `C_0 cap BV_0`, every lower-order aperture term has an exact fixed-state derivative and prime-power activation is first-order transparent. The unresolved compactness gate is therefore specifically state-side: derive enough BV/derivative-measure control from the completed-Weil eigen-equation to pass those explicit lower derivatives through `s->0`.

`PL-306` removes compactness of the **squared** renormalized endpoint stress as an independent task once the finite-`s` stress identity and lower derivative convergence are available. `PL-307` goes one step further: Ros-Oton--Serra's state-level fractional Pohozaev identity, combined with the exact dilation covariance of the lower Weil form, yields that same finite-`s` squared-stress virial identity directly. A new mixed moving-domain Hadamard theorem is therefore **not an independent gate for the scalar endpoint-square quantity**. What remains is to prove that the actual regularized completed-Weil eigenstate satisfies the Pohozaev regularity and lower dilation-pairing hypotheses, and then establish the source-structured convergence of `partial_a b_a(w_{s,a})`. Signed or pointwise trace convergence remains optional extra structure beyond the scalar eigenvalue problem.

The exact unresolved question is now: can the small-`s` regularization of the actual isolated completed-Weil branch produce source-structured regularity strong enough to justify the Ros-Oton--Serra Pohozaev identity and the lower-order dilation pairing on source-static intervals, together with uniform BV/derivative-measure control sufficient to transport `C'_{u_{s,a}}(log n/a)` for the finitely many active prime-power shifts and hence `partial_a b_a(w_{s,a}) -> partial_a b_a(w_{0,a})`, **without** importing `H^{1/2}` or absolute first-frequency-moment differentiability? If that succeeds, `PL-307` forces the squared boundary stress automatically. The route is rejected if the actual branch fails Pohozaev admissibility, if source-structured derivative compactness fails, if the completion/dilation pairing is uncontrolled, or if the resulting virial residual remains unchanged under matched nonarithmetic source weights and supplies no rational-prime-specific first-crossing control.