...
Accordingly, the clue survives only after a fifth strengthening. Further work should **not** ask whether `S_1` membership, a determinant, an ordinary scalar `p,q,pq` cocycle law, a nontrivial scalar projective multiplier, a scalar Hilbert-symbol/product-formula normalization, or the normalized unramified spherical standard intertwiner can supply the missing rigidity. It should ask whether a canonically specified **non-spherical or target-relative** operator family is intrinsic to the ordinary zeta problem, remains nontrivial after L-factor normalization and reciprocity, and provably constrains the spherical zeta scattering scalar rather than merely accompanying it. A Nyman/model-space target coupling or a global positivity/trace relation that forces matrix data outside the spherical line to control the scalar channel would qualify; arbitrary ramification/K-types added by hand would not.
