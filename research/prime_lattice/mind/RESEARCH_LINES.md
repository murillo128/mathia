...
The live arithmetic question is now concrete: estimate the best lag-stationary approximation to the **canonical centered fixed-mod-5 prime covariance**, place its normalized coordinates `(m/d,f/d)` relative to the PL-441 wedge--ellipse, and control the nonstationary remainder in operator norm at the same covariance scale. Exact stationarity is not inherited from the source, `Delta_s=0` alone does not imply stationarity, and a bound on the single quadratic-character probe is insufficient without the remaining covariance geometry. If the stationary component has a robust interior margin, the arithmetic burden reduces to an operator-norm anisotropy estimate; if it lies robustly outside, approximate stationarity cannot prove the local-factor gate.
