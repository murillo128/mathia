...
The exploratory [`riemann_corpus`](riemann_corpus/) work for issue `#42` is separate from that critical path. It inventories and normalizes an external source corpus and runs a small teacher-analysis audit; it does not train a model, use the GPU, or authorize the protected `#32` run.
