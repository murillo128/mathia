...
The exact generator instruction, reviewer instruction, 8+16 evenly spaced
...
read-only review records `CALIBRATION_PASS` in `calibration_review.json`.
...
# Publish and obtain the required fresh CALIBRATION_PASS review, then record calibration_review.json.
